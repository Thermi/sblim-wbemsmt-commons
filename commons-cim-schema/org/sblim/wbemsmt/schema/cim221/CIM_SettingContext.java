/**
 * CIM_SettingContext.java Â© Copyright IBM Corp.  2009,2005 THIS FILE IS PROVIDED UNDER THE TER MS
 ...
 *            of instances of Setting or SettingData. Therefore, the aggregation of Settings into
 *            Configurations is replaced by the ConcreteComponent relationship between instances of
 *            Settings and SettingData. Deprecated description: This relationship associates
 *            Configuration objects with Setting objects. For example, Settings of a NetworkAdapter
 *            could change based on the site or network to which its hosting ComputerSystem is
 *            attached. In this case, the ComputerSystem would have two different Configuration
 *            objects, corresponding to the differences in network configuration for the two network
 *            segments. Configuration A would aggregate a Setting object for the NetworkAdapter when
 *            operating on segment "ANet", whereas Configuration B would aggregate a different
 *            NetworkAdapter Setting object, specific to segment "BNet". Note that many Settings of
 *            the computer are independent of the network Configuration. For example, both
 *            Configurations A and B would aggregate the same Setting object for the
 *            MonitorResolution of the ComputerSystem. generated Class
 */
...
public class CIM_SettingContext extends org.sblim.wbemsmt.bl.fco.AbstractWbemsmtFco {
    ...
    public boolean set_Context_CIM_Configuration(CIM_Configuration newValue) {
        ...
        if (currentProperty != null) {
            setProperty(setPropertyValue_Context_CIM_Configuration(currentProperty, newValue));
            return true;
        }
        else {
            logger.warning("Property " + PROPERTY_CONTEXT_CIM_CONFIGURATION.NAME
