...
public final class CIM_ControlledByHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_ControlledBy(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_ControlledBy.Java_Package_List.size(); i++) {
						if (!((String)(CIM_ControlledBy.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_ControlledBy.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_ControlledBy.Java_Package_List.setElementAt((String)(CIM_ControlledBy.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_ControlledBy.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_ControlledBy.");
						resultArrayList.add(new CIM_ControlledBy(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_ControlledBy.");
						resultArrayList.add(new CIM_ControlledBy(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_ControlledBy.");
						resultArrayList.add(new CIM_ControlledBy(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_ControlledBy getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ControlledByHelper.getInstance(cimClient, new CIM_ControlledBy(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ControlledBy getInstance(CIMClient cimClient, CIM_ControlledBy dataInstance) {
		...
		return CIM_ControlledByHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ControlledBy getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_ControlledBy dataInstance = null;
		try {
			dataInstance = new CIM_ControlledBy(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ControlledBy createInstance(CIMClient cimClient, CIM_ControlledBy dataInstance, boolean notifyDifferencies){ 
		...
		CIM_ControlledBy newDataInstance = null;
		try {
			newDataInstance = CIM_ControlledByHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ControlledBy modifyInstance(CIMClient cimClient, CIM_ControlledBy dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_ControlledByHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_ControlledBy dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_ControlledBy dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_ControlledBy.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_ControlledBy.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_ControlledBy.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_ControlledBy.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_ControlledBy.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AccessMode(UnsignedInt16 AccessMode) {
		return true;
	}

	public static boolean isValid_AccessPriority(UnsignedInt16 AccessPriority) {
		return true;
	}

	public static boolean isValid_AccessState(UnsignedInt16 AccessState) {
		return true;
	}

	public static boolean isValid_DeviceNumber(String DeviceNumber) {
		return true;
	}

	public static boolean isValid_NumberOfHardResets(UnsignedInt32 NumberOfHardResets) {
		return true;
	}

	public static boolean isValid_NumberOfSoftResets(UnsignedInt32 NumberOfSoftResets) {
		return true;
	}

	public static boolean isValid_TimeOfDeviceReset(Calendar TimeOfDeviceReset) {
		return true;
	}

	public static boolean isValid_CIM_Controller(CIM_Controller CIM_Controller) {
		return true;
	}
