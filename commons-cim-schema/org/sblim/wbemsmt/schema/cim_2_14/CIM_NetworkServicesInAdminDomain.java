 ...
 *
 *
 * Description:  This association establishes the Dependency relationships that exist between an
 * administrative domain and the Network Services that it hosts. It is
 * deprecated since AdminDomains can contain any ManagedSystemElements. There is
 * no special relationship needed for NetworkServices. In addition, the Min (1)
 * cardinality on AdminDomain is not valid - since it may not be possible to
 * define every NetworkService in the context of one or more Domains.
 * 
 */
...
/**
 *  This association establishes the Dependency relationships that exist between an
 * administrative domain and the Network Services that it hosts. It is
 * deprecated since AdminDomains can contain any ManagedSystemElements. There is
 * no special relationship needed for NetworkServices. In addition, the Min (1)
 * cardinality on AdminDomain is not valid - since it may not be possible to
...
public class CIM_NetworkServicesInAdminDomain extends CIM_HostedDependency  {
	...
	public final static String CIM_CLASS_NAME = "CIM_NetworkServicesInAdminDomain"; //$NON-NLS-1$
