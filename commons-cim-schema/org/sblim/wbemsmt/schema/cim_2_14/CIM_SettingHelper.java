...
public final class CIM_SettingHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_Setting.Java_Package_List.size(); i++) {
						if (!((String)(CIM_Setting.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_Setting.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_Setting.Java_Package_List.setElementAt((String)(CIM_Setting.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
	...
	*/	
	public final static CIM_Setting getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_SettingHelper.getInstance(cimClient, new CIM_Setting(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Setting getInstance(CIMClient cimClient, CIM_Setting dataInstance) {
		...
		return CIM_SettingHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Setting getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_Setting(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Setting.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Setting.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Setting.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Setting.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
