...
public final class CIM_AlarmDeviceHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_AlarmDevice.Java_Package_List.size(); i++) {
						if (!((String)(CIM_AlarmDevice.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_AlarmDevice.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_AlarmDevice.Java_Package_List.setElementAt((String)(CIM_AlarmDevice.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_AlarmDevice.");
						resultArrayList.add(new CIM_AlarmDevice(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_AlarmDevice.");
						resultArrayList.add(new CIM_AlarmDevice(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_AlarmDevice.");
						resultArrayList.add(new CIM_AlarmDevice(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_AlarmDevice getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_AlarmDeviceHelper.getInstance(cimClient, new CIM_AlarmDevice(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_AlarmDevice getInstance(CIMClient cimClient, CIM_AlarmDevice dataInstance) {
		...
		return CIM_AlarmDeviceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_AlarmDevice getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_AlarmDevice dataInstance = null;
		try {
			dataInstance = new CIM_AlarmDevice(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_AlarmDevice createInstance(CIMClient cimClient, CIM_AlarmDevice dataInstance, boolean notifyDifferencies){ 
		...
		CIM_AlarmDevice newDataInstance = null;
		try {
			newDataInstance = CIM_AlarmDeviceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_AlarmDevice modifyInstance(CIMClient cimClient, CIM_AlarmDevice dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_AlarmDeviceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_AlarmDevice dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_AlarmDevice dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_AlarmDevice.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_AlarmDevice.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_AlarmDevice.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_AlarmDevice.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_AlarmDevice.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AlarmState(UnsignedInt16 AlarmState) {
		return true;
	}
	...
	public static boolean isValid_AudioIndicatorIsDisabled(Boolean AudioIndicatorIsDisabled) {
		return true;
	}

	public static boolean isValid_MotionAlarm(Boolean MotionAlarm) {
		return true;
	}

	public static boolean isValid_MotionIndicatorIsDisabled(Boolean MotionIndicatorIsDisabled) {
		return true;
	}

	public static boolean isValid_Urgency(UnsignedInt16 Urgency) {
		return true;
	}
	...
	public static boolean isValid_VisualIndicatorIsDisabled(Boolean VisualIndicatorIsDisabled) {
		return true;
	}
