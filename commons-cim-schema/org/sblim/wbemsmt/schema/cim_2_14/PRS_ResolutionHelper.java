...
public final class PRS_ResolutionHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Resolution.");
						resultArrayList.add(new PRS_Resolution(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Resolution.");
						resultArrayList.add(new PRS_Resolution(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Resolution.");
						resultArrayList.add(new PRS_Resolution(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Resolution getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_ResolutionHelper.getInstance(cimClient, new PRS_Resolution(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Resolution getInstance(CIMClient cimClient, PRS_Resolution dataInstance) {
		...
		return PRS_ResolutionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Resolution getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Resolution dataInstance = null;
		try {
			dataInstance = new PRS_Resolution(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Resolution createInstance(CIMClient cimClient, PRS_Resolution dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Resolution newDataInstance = null;
		try {
			newDataInstance = PRS_ResolutionHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Resolution modifyInstance(CIMClient cimClient, PRS_Resolution dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_ResolutionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Resolution dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Resolution dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Resolution.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Resolution.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Resolution.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Resolution.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Resolution.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Confidence(UnsignedInt16 Confidence) {
		return true;
	}
