...
public final class CIM_OrgUnitHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_OrgUnit.");
						resultArrayList.add(new CIM_OrgUnit(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_OrgUnit.");
						resultArrayList.add(new CIM_OrgUnit(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_OrgUnit.");
						resultArrayList.add(new CIM_OrgUnit(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_OrgUnit getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_OrgUnitHelper.getInstance(cimClient, new CIM_OrgUnit(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_OrgUnit getInstance(CIMClient cimClient, CIM_OrgUnit dataInstance) {
		...
		return CIM_OrgUnitHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_OrgUnit getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_OrgUnit dataInstance = null;
		try {
			dataInstance = new CIM_OrgUnit(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_OrgUnit createInstance(CIMClient cimClient, CIM_OrgUnit dataInstance, boolean notifyDifferencies){ 
		...
		CIM_OrgUnit newDataInstance = null;
		try {
			newDataInstance = CIM_OrgUnitHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_OrgUnit modifyInstance(CIMClient cimClient, CIM_OrgUnit dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_OrgUnitHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_OrgUnit dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_OrgUnit dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_OrgUnit.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_OrgUnit.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_OrgUnit.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_OrgUnit.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_OrgUnit.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
