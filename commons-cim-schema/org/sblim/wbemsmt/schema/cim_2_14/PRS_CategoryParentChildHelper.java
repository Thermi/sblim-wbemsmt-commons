/** 
 * PRS_CategoryParentChildHelper.java
 *
...
public final class PRS_CategoryParentChildHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new PRS_CategoryParentChild(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<PRS_CategoryParentChild.Java_Package_List.size(); i++) {
						if (!((String)(PRS_CategoryParentChild.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(PRS_CategoryParentChild.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							PRS_CategoryParentChild.Java_Package_List.setElementAt((String)(PRS_CategoryParentChild.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (PRS_CategoryParentChild.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_CategoryParentChild.");
						resultArrayList.add(new PRS_CategoryParentChild(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_CategoryParentChild.");
						resultArrayList.add(new PRS_CategoryParentChild(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_CategoryParentChild.");
						resultArrayList.add(new PRS_CategoryParentChild(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_CategoryParentChild getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_CategoryParentChildHelper.getInstance(cimClient, new PRS_CategoryParentChild(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_CategoryParentChild getInstance(CIMClient cimClient, PRS_CategoryParentChild dataInstance) {
		...
		return PRS_CategoryParentChildHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_CategoryParentChild getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_CategoryParentChild dataInstance = null;
		try {
			dataInstance = new PRS_CategoryParentChild(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_CategoryParentChild createInstance(CIMClient cimClient, PRS_CategoryParentChild dataInstance, boolean notifyDifferencies){ 
		...
		PRS_CategoryParentChild newDataInstance = null;
		try {
			newDataInstance = PRS_CategoryParentChildHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_CategoryParentChild modifyInstance(CIMClient cimClient, PRS_CategoryParentChild dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_CategoryParentChildHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_CategoryParentChild dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_CategoryParentChild dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_CategoryParentChild.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_CategoryParentChild.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_CategoryParentChild.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_CategoryParentChild.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_CategoryParentChild.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_PRS_Category_1(PRS_Category PRS_Category_1) {
		return true;
	}

	public static boolean isValid_PRS_Category_2(PRS_Category PRS_Category_2) {
		return true;
	}
