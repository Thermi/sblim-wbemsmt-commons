 ...
 *
 *
 * Description:  NextHopRouting relates a destination address to the address or interface
 * through which the remote address may be reached. An instance of this class
 * can represent either static or dynamic routing, and may represent a routing
 * table entry (but it is not required that all NextHopRoutes be in a routing
 * table). Static routes are distinguished by setting the IsStatic boolean
 * property to TRUE. Subclasses of NextHopRouting provide specific protocol and
 * Key information. NextHopRoutes are dependent on at least one
 ...
 * the definition of BOTH a next hop address and an interface for transmission
 * of the traffic. Also, it does not mandate the instantiation of a
 * ForwardingService class. The latter is not needed if the sole purpose of the
 * instance is to specify a 'desired/configured' route. This is simply a
...
public class CIM_NextHopRouting extends CIM_LogicalElement  {
	...
	public final static String CIM_CLASS_NAME = "CIM_NextHopRouting"; //$NON-NLS-1$
