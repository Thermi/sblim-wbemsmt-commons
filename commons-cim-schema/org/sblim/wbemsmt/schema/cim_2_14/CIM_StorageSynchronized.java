 ...
 *
 *
 * Description:  Indicates that two Storage objects were replicated at the specified point in
 * time. If the CopyType property is set to 'Sync' (=3), then synchronization of
 * the Storage objects is preserved.
 * 
 */
...
/**
 *  Indicates that two Storage objects were replicated at the specified point in
 * time. If the CopyType property is set to 'Sync' (=3), then synchronization of
 * the Storage objects is preserved.
 */
public class CIM_StorageSynchronized extends CIM_Synchronized  {
	
	public final static String CIM_CLASS_NAME = "CIM_StorageSynchronized"; //$NON-NLS-1$
	...
	*/
	...
	static {
		...
		for (int i = 0; i < CIM_Synchronized.Java_Package_List.size(); i++) {
			if (((String)CIM_Synchronized.Java_Package_List.elementAt(i)).equals("org.sblim.wbemsmt.schema.cim_2_14")){
				continue;
			}
