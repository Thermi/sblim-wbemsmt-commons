...
public final class CIM_SCSIControllerHelper {

	public final static String CIM_CLASS_NAME = "CIM_SCSIController";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_SCSIController(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_SCSIController.Java_Package_List.size(); i++) {
						if (!((String)(CIM_SCSIController.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_SCSIController.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_SCSIController.Java_Package_List.setElementAt((String)(CIM_SCSIController.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_SCSIController.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_SCSIController.");
						resultArrayList.add(new CIM_SCSIController(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_SCSIController.");
						resultArrayList.add(new CIM_SCSIController(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_SCSIController.");
						resultArrayList.add(new CIM_SCSIController(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_SCSIController getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_SCSIControllerHelper.getInstance(cimClient, new CIM_SCSIController(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SCSIController getInstance(CIMClient cimClient, CIM_SCSIController dataInstance) {
		...
		return CIM_SCSIControllerHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SCSIController getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_SCSIController dataInstance = null;
		try {
			dataInstance = new CIM_SCSIController(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_SCSIController createInstance(CIMClient cimClient, CIM_SCSIController dataInstance, boolean notifyDifferencies){ 
		...
		CIM_SCSIController newDataInstance = null;
		try {
			newDataInstance = CIM_SCSIControllerHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_SCSIController modifyInstance(CIMClient cimClient, CIM_SCSIController dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_SCSIControllerHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_SCSIController dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_SCSIController dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_SCSIController.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_SCSIController.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_SCSIController.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_SCSIController.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_SCSIController.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ControllerTimeouts(UnsignedInt32 ControllerTimeouts) {
		return true;
	}

	public static boolean isValid_MaxDataWidth(UnsignedInt32 MaxDataWidth) {
		return true;
	}

	public static boolean isValid_MaxTransferRate(UnsignedInt64 MaxTransferRate) {
		return true;
	}

	public static boolean isValid_ProtectionManagement(UnsignedInt16 ProtectionManagement) {
		return true;
	}

	public static boolean isValid_SignalCapabilities(UnsignedInt16[] SignalCapabilities) {
		return true;
	}
