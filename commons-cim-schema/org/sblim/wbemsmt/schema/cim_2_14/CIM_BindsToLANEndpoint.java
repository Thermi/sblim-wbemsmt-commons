 ...
 *
 *
 * Description:  This association makes explicit the dependency of a SAP or ProtocolEndpoint on
 * an underlying LANEndpoint, on the same system.
 * 
 */
...
/**
 *  This association makes explicit the dependency of a SAP or ProtocolEndpoint on
 * an underlying LANEndpoint, on the same system.
 */
public class CIM_BindsToLANEndpoint extends CIM_BindsTo  {
	
	public final static String CIM_CLASS_NAME = "CIM_BindsToLANEndpoint"; //$NON-NLS-1$
	...
	*/
	...
	static {
		...
		for (int i = 0; i < CIM_BindsTo.Java_Package_List.size(); i++) {
			if (((String)CIM_BindsTo.Java_Package_List.elementAt(i)).equals("org.sblim.wbemsmt.schema.cim_2_14")){
				continue;
			}
