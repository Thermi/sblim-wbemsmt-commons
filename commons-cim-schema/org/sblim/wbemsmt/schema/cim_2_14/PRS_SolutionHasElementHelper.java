/** 
 * PRS_SolutionHasElementHelper.java
 *
...
public final class PRS_SolutionHasElementHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new PRS_SolutionHasElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<PRS_SolutionHasElement.Java_Package_List.size(); i++) {
						if (!((String)(PRS_SolutionHasElement.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(PRS_SolutionHasElement.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							PRS_SolutionHasElement.Java_Package_List.setElementAt((String)(PRS_SolutionHasElement.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (PRS_SolutionHasElement.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_SolutionHasElement.");
						resultArrayList.add(new PRS_SolutionHasElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_SolutionHasElement.");
						resultArrayList.add(new PRS_SolutionHasElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_SolutionHasElement.");
						resultArrayList.add(new PRS_SolutionHasElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_SolutionHasElement getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_SolutionHasElementHelper.getInstance(cimClient, new PRS_SolutionHasElement(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_SolutionHasElement getInstance(CIMClient cimClient, PRS_SolutionHasElement dataInstance) {
		...
		return PRS_SolutionHasElementHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_SolutionHasElement getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_SolutionHasElement dataInstance = null;
		try {
			dataInstance = new PRS_SolutionHasElement(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_SolutionHasElement.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_SolutionHasElement.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_SolutionHasElement.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_SolutionHasElement.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
