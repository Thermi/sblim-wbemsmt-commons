 ...
 *
 *
 * Description:  A ZoneSet is a group of zones that are managed collectively by the same domain.
 * The managing domain is indicated by the HostedCollection association. For
 * example, for Fibre Channel, this property would indicate a set of zones that
 * are part of a fabric. The zones are under enforcement by the fabric, only if
...
/**
 *  A ZoneSet is a group of zones that are managed collectively by the same domain.
 * The managing domain is indicated by the HostedCollection association. For
 * example, for Fibre Channel, this property would indicate a set of zones that
 * are part of a fabric. The zones are under enforcement by the fabric, only if
 * the Zoneset is set to active.
...
public class CIM_ZoneSet extends CIM_ConnectivityCollection  {
	...
	public final static String CIM_CLASS_NAME = "CIM_ZoneSet"; //$NON-NLS-1$
