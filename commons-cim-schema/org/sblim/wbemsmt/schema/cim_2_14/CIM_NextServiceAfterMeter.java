 ...
 *
 *
 * Description:  This association describes a predecessor-successor relationship between a
 * MeterService and one or more ConditioningService objects that process traffic
 * from the meter. For example, for devices that implement preamble marking, the
 * FollowingService reference (after the meter) is a PreambleMarkerService - to
 * record the results of the metering in the preamble. It might be expected that
 * the NextServiceAfterMeter association would subclass from NextService.
 * However, meters are 1:n fan-out elements, and require a mechanism to
 * distinguish between the different results/outputs of the meter. Therefore,
 * this association defines a new key property, MeterResult, which is used to
 * record the result and identify the output through which this traffic left the
 * meter.
 * 
 */
...
public class CIM_NextServiceAfterMeter  {
	...
	public final static String CIM_CLASS_NAME = "CIM_NextServiceAfterMeter"; //$NON-NLS-1$
