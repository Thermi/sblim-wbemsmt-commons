 ...
 * will be promoted to UserEntity in a later major release of CIM, but cannot be
 * promoted at this time since UserEntity is abstract today, with no keys. Key
 * and required properties cannot be added to a class in a minor version release
...
public class CIM_UserContact extends CIM_UserEntity  {
	...
	public final static String CIM_CLASS_NAME = "CIM_UserContact"; //$NON-NLS-1$
	...
	*/
	...
	static {
		...
		for (int i = 0; i < CIM_UserEntity.Java_Package_List.size(); i++) {
			if (((String)CIM_UserEntity.Java_Package_List.elementAt(i)).equals("org.sblim.wbemsmt.schema.cim_2_14")){
				continue;
			}
