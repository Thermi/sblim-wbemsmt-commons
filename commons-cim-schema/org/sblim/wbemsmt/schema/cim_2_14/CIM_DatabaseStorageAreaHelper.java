/** 
 * CIM_DatabaseStorageAreaHelper.java
 *
...
public final class CIM_DatabaseStorageAreaHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_DatabaseStorageArea.Java_Package_List.size(); i++) {
						if (!((String)(CIM_DatabaseStorageArea.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_DatabaseStorageArea.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_DatabaseStorageArea.Java_Package_List.setElementAt((String)(CIM_DatabaseStorageArea.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_DatabaseStorageArea.");
						resultArrayList.add(new CIM_DatabaseStorageArea(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_DatabaseStorageArea.");
						resultArrayList.add(new CIM_DatabaseStorageArea(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_DatabaseStorageArea.");
						resultArrayList.add(new CIM_DatabaseStorageArea(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_DatabaseStorageArea getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_DatabaseStorageAreaHelper.getInstance(cimClient, new CIM_DatabaseStorageArea(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DatabaseStorageArea getInstance(CIMClient cimClient, CIM_DatabaseStorageArea dataInstance) {
		...
		return CIM_DatabaseStorageAreaHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DatabaseStorageArea getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_DatabaseStorageArea dataInstance = null;
		try {
			dataInstance = new CIM_DatabaseStorageArea(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_DatabaseStorageArea createInstance(CIMClient cimClient, CIM_DatabaseStorageArea dataInstance, boolean notifyDifferencies){ 
		...
		CIM_DatabaseStorageArea newDataInstance = null;
		try {
			newDataInstance = CIM_DatabaseStorageAreaHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_DatabaseStorageArea modifyInstance(CIMClient cimClient, CIM_DatabaseStorageArea dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_DatabaseStorageAreaHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_DatabaseStorageArea dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_DatabaseStorageArea dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_DatabaseStorageArea.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_DatabaseStorageArea.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_DatabaseStorageArea.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_DatabaseStorageArea.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_DatabaseStorageArea.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_IsSystemArea(Boolean IsSystemArea) {
		return true;
	}

	public static boolean isValid_LastBackup(Calendar LastBackup) {
		return true;
	}
