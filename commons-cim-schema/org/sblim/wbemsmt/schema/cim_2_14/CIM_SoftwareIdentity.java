 ...
 * for asset tracking and/or installation dependency management. When the
 * IsEntity property has the value TRUE, the instance of SoftwareIdentity
 * represents an individually identifiable entity similar to Physical Element.
 * SoftwareIdentity does NOT indicate whether the software is installed,
 * executing, etc. This extra information may be provided through specialized
 * associations to Software Identity. For instance, both
 * InstalledSoftwareIdentity and ElementSoftwareIdentity may be used to indicate
 * that the software identified by this class is installed. SoftwareIdentity is
 * used when managing the software components of a ManagedElement that is
 * management focus. Since software may be acquired, SoftwareIdentity can be
 * associated with a Product using the ProductSoftwareComponent relationship.
 * The Application Model manages the deployment and installation of software via
 * the classes, SoftwareFeatures and SoftwareElements. SoftwareFeature and
 * SoftwareElement are used when the software component is the management focus.
 * The deployment/installation concepts are related to the asset/identity one.
 * In fact, a SoftwareIdentity may correspond to a Product, or to one or more
 * SoftwareFeatures or SoftwareElements - depending on the granularity of these
 * classes and the deployment model. The correspondence of Software Identity to
 * Product, SoftwareFeature or SoftwareElement is indicated using the
 * ConcreteIdentity association. Note that there may not be sufficient detail or
 * instrumentation to instantiate ConcreteIdentity. And, if the association is
 * instantiated, some duplication of information may result. For example, the
 * Vendor described in the instances of Product and SoftwareIdentity MAY be the
 * same. However, this is not necessarily true, and it is why vendor and similar
 * information are duplicated in this class. Note that ConcreteIdentity can also
 * be used to describe the relationship of the software to any LogicalFiles that
 * result from installing it. As above, there may not be sufficient detail or
...
For DMTF defined instances, the 'preferred' algorithm MUST be used with the <OrgID> set to 'CIM'.
	...
	*/
	...
	public final static String[] CIM_VALUEMAP_CLASSIFICATIONS = {"Unknown","Other","Driver","Configuration Software","Application Software","Instrumentation","Firmware/BIOS","Diagnostic Software","Operating System","Middleware","Firmware","BIOS/FCode","Support/Service Pack","Software Bundle","DMTF Reserved","Vendor Reserved"};
	...
	public final static String CLASSIFICATIONS_SUPPORT_SERVICEPACK = "12";
	public final static String CLASSIFICATIONS_SOFTWAREBUNDLE = "13";
