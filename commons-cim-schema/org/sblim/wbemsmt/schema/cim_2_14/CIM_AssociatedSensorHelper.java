/** 
 * CIM_AssociatedSensorHelper.java
 *
...
public final class CIM_AssociatedSensorHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_AssociatedSensor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_AssociatedSensor.Java_Package_List.size(); i++) {
						if (!((String)(CIM_AssociatedSensor.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_AssociatedSensor.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_AssociatedSensor.Java_Package_List.setElementAt((String)(CIM_AssociatedSensor.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_AssociatedSensor.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_AssociatedSensor.");
						resultArrayList.add(new CIM_AssociatedSensor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_AssociatedSensor.");
						resultArrayList.add(new CIM_AssociatedSensor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_AssociatedSensor.");
						resultArrayList.add(new CIM_AssociatedSensor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_AssociatedSensor getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_AssociatedSensorHelper.getInstance(cimClient, new CIM_AssociatedSensor(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_AssociatedSensor getInstance(CIMClient cimClient, CIM_AssociatedSensor dataInstance) {
		...
		return CIM_AssociatedSensorHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_AssociatedSensor getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_AssociatedSensor dataInstance = null;
		try {
			dataInstance = new CIM_AssociatedSensor(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_AssociatedSensor createInstance(CIMClient cimClient, CIM_AssociatedSensor dataInstance, boolean notifyDifferencies){ 
		...
		CIM_AssociatedSensor newDataInstance = null;
		try {
			newDataInstance = CIM_AssociatedSensorHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_AssociatedSensor modifyInstance(CIMClient cimClient, CIM_AssociatedSensor dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_AssociatedSensorHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_AssociatedSensor dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_AssociatedSensor dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_AssociatedSensor.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_AssociatedSensor.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_AssociatedSensor.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_AssociatedSensor.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_AssociatedSensor.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
