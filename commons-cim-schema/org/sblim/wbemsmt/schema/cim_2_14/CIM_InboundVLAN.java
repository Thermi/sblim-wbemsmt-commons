 ...
 * dependencies of a SwitchPort when operating in a VLAN. If there is an
 * association between a particular SwitchPort and a particular VLAN, then there
 * is the possibility that a packet received by the port will be assigned to the
 * VLAN (or if the packet already has a VLAN tag, that the packet will not be
 * dropped). If there is no such association, then there is NO possibility that
 * a packet received by the port will progress through the switch having been
 * assigned to the referenced VLAN.
 * 
 */
...
public class CIM_InboundVLAN extends CIM_SAPSAPDependency  {
	...
	public final static String CIM_CLASS_NAME = "CIM_InboundVLAN"; //$NON-NLS-1$
