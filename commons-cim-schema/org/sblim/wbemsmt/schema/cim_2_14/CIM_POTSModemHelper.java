...
public final class CIM_POTSModemHelper {

	public final static String CIM_CLASS_NAME = "CIM_POTSModem";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_POTSModem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_POTSModem.Java_Package_List.size(); i++) {
						if (!((String)(CIM_POTSModem.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_POTSModem.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_POTSModem.Java_Package_List.setElementAt((String)(CIM_POTSModem.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_POTSModem.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_POTSModem.");
						resultArrayList.add(new CIM_POTSModem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_POTSModem.");
						resultArrayList.add(new CIM_POTSModem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_POTSModem.");
						resultArrayList.add(new CIM_POTSModem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_POTSModem getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_POTSModemHelper.getInstance(cimClient, new CIM_POTSModem(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_POTSModem getInstance(CIMClient cimClient, CIM_POTSModem dataInstance) {
		...
		return CIM_POTSModemHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_POTSModem getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_POTSModem dataInstance = null;
		try {
			dataInstance = new CIM_POTSModem(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_POTSModem createInstance(CIMClient cimClient, CIM_POTSModem dataInstance, boolean notifyDifferencies){ 
		...
		CIM_POTSModem newDataInstance = null;
		try {
			newDataInstance = CIM_POTSModemHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_POTSModem modifyInstance(CIMClient cimClient, CIM_POTSModem dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_POTSModemHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_POTSModem dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_POTSModem dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_POTSModem.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_POTSModem.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_POTSModem.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_POTSModem.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_POTSModem.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AnswerMode(UnsignedInt16 AnswerMode) {
		return true;
	}

	public static boolean isValid_BlackListedNumbers(String[] BlackListedNumbers) {
		return true;
	}
	...
	public static boolean isValid_CountriesSupported(String[] CountriesSupported) {
		return true;
	}

	public static boolean isValid_CountrySelected(String CountrySelected) {
		return true;
	}

	public static boolean isValid_CurrentPasswords(String[] CurrentPasswords) {
		return true;
	}

	public static boolean isValid_DialType(UnsignedInt16 DialType) {
		return true;
	}
	...
	public static boolean isValid_Equalization(UnsignedInt16 Equalization) {
		return true;
	}

	public static boolean isValid_InactivityTimeout(UnsignedInt32 InactivityTimeout) {
		return true;
	}

	public static boolean isValid_MaxBaudRateToPhone(UnsignedInt32 MaxBaudRateToPhone) {
		return true;
	}

	public static boolean isValid_MaxBaudRateToSerialPort(UnsignedInt32 MaxBaudRateToSerialPort) {
		return true;
	}

	public static boolean isValid_MaxNumberOfPasswords(UnsignedInt16 MaxNumberOfPasswords) {
		return true;
	}

	public static boolean isValid_RingsBeforeAnswer(UnsignedInt8 RingsBeforeAnswer) {
		return true;
	}

	public static boolean isValid_SpeakerVolumeInfo(UnsignedInt16 SpeakerVolumeInfo) {
		return true;
	}

	public static boolean isValid_SupportsCallback(Boolean SupportsCallback) {
		return true;
	}

	public static boolean isValid_SupportsSynchronousConnect(Boolean SupportsSynchronousConnect) {
		return true;
	}
