/** 
 * CIM_ProtocolControllerHelper.java
 *
...
public final class CIM_ProtocolControllerHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_ProtocolController.");
						resultArrayList.add(new CIM_ProtocolController(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_ProtocolController.");
						resultArrayList.add(new CIM_ProtocolController(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_ProtocolController.");
						resultArrayList.add(new CIM_ProtocolController(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_ProtocolController getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ProtocolControllerHelper.getInstance(cimClient, new CIM_ProtocolController(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ProtocolController getInstance(CIMClient cimClient, CIM_ProtocolController dataInstance) {
		...
		return CIM_ProtocolControllerHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ProtocolController getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_ProtocolController dataInstance = null;
		try {
			dataInstance = new CIM_ProtocolController(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_ProtocolController.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_ProtocolController.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_ProtocolController.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_ProtocolController.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_MaxUnitsControlled(UnsignedInt32 MaxUnitsControlled) {
		return true;
	}
