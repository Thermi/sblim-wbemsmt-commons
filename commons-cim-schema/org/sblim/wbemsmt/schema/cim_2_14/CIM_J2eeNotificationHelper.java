/** 
 * CIM_J2eeNotificationHelper.java
 *
...
public final class CIM_J2eeNotificationHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_J2eeNotification(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_J2eeNotification.Java_Package_List.size(); i++) {
						if (!((String)(CIM_J2eeNotification.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_J2eeNotification.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_J2eeNotification.Java_Package_List.setElementAt((String)(CIM_J2eeNotification.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_J2eeNotification.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_J2eeNotification.");
						resultArrayList.add(new CIM_J2eeNotification(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_J2eeNotification.");
						resultArrayList.add(new CIM_J2eeNotification(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_J2eeNotification.");
						resultArrayList.add(new CIM_J2eeNotification(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_J2eeNotification getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_J2eeNotificationHelper.getInstance(cimClient, new CIM_J2eeNotification(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_J2eeNotification getInstance(CIMClient cimClient, CIM_J2eeNotification dataInstance) {
		...
		return CIM_J2eeNotificationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_J2eeNotification getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_J2eeNotification dataInstance = null;
		try {
			dataInstance = new CIM_J2eeNotification(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_J2eeNotification createInstance(CIMClient cimClient, CIM_J2eeNotification dataInstance, boolean notifyDifferencies){ 
		...
		CIM_J2eeNotification newDataInstance = null;
		try {
			newDataInstance = CIM_J2eeNotificationHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_J2eeNotification modifyInstance(CIMClient cimClient, CIM_J2eeNotification dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_J2eeNotificationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_J2eeNotification dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_J2eeNotification dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_J2eeNotification.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_J2eeNotification.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_J2eeNotification.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_J2eeNotification.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_J2eeNotification.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_IndicationIdentifier(String IndicationIdentifier) {
		return true;
	}

	public static boolean isValid_IndicationType(UnsignedInt16 IndicationType) {
		return true;
	}

	public static boolean isValid_Message(String Message) {
		return true;
	}

	public static boolean isValid_OtherIndicationType(String OtherIndicationType) {
		return true;
	}

	public static boolean isValid_SequenceNumber(Long SequenceNumber) {
		return true;
	}

	public static boolean isValid_UserData(String UserData) {
		return true;
	}
