...
public final class CIM_DoorHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_Door.Java_Package_List.size(); i++) {
						if (!((String)(CIM_Door.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_Door.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_Door.Java_Package_List.setElementAt((String)(CIM_Door.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_Door.");
						resultArrayList.add(new CIM_Door(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_Door.");
						resultArrayList.add(new CIM_Door(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_Door.");
						resultArrayList.add(new CIM_Door(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_Door getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_DoorHelper.getInstance(cimClient, new CIM_Door(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Door getInstance(CIMClient cimClient, CIM_Door dataInstance) {
		...
		return CIM_DoorHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Door getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_Door dataInstance = null;
		try {
			dataInstance = new CIM_Door(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Door createInstance(CIMClient cimClient, CIM_Door dataInstance, boolean notifyDifferencies){ 
		...
		CIM_Door newDataInstance = null;
		try {
			newDataInstance = CIM_DoorHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Door modifyInstance(CIMClient cimClient, CIM_Door dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_DoorHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_Door dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_Door dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_Door.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Door.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Door.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Door.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Door.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_LastOpened(Calendar LastOpened) {
		return true;
	}
	...
	public static boolean isValid_Open(Boolean Open) {
		return true;
	}

	public static boolean isValid_Timeout(UnsignedInt32 Timeout) {
		return true;
	}
