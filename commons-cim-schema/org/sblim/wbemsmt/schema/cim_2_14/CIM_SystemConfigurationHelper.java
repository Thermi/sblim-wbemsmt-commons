...
public final class CIM_SystemConfigurationHelper {
	...
	*/	
	public final static CIM_SystemConfiguration getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_SystemConfigurationHelper.getInstance(cimClient, new CIM_SystemConfiguration(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SystemConfiguration getInstance(CIMClient cimClient, CIM_SystemConfiguration dataInstance) {
		...
		return CIM_SystemConfigurationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SystemConfiguration getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_SystemConfiguration(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_SystemConfiguration createInstance(CIMClient cimClient, CIM_SystemConfiguration dataInstance, boolean notifyDifferencies){ 
		...
		CIM_SystemConfiguration newDataInstance = null;
		try {
			newDataInstance = CIM_SystemConfigurationHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_SystemConfiguration modifyInstance(CIMClient cimClient, CIM_SystemConfiguration dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_SystemConfigurationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_SystemConfiguration dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_SystemConfiguration dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_SystemConfiguration.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_SystemConfiguration.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_SystemConfiguration.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_SystemConfiguration.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_SystemConfiguration.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
