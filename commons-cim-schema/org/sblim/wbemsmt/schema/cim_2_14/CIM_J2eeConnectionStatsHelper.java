/** 
 * CIM_J2eeConnectionStatsHelper.java
 *
...
public final class CIM_J2eeConnectionStatsHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_J2eeConnectionStats(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					...
					for (int i=0; clazz==null && i<CIM_J2eeConnectionStats.Java_Package_List.size(); i++) {
						if (!((String)(CIM_J2eeConnectionStats.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_J2eeConnectionStats.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_J2eeConnectionStats.Java_Package_List.setElementAt((String)(CIM_J2eeConnectionStats.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_J2eeConnectionStats.");
						resultArrayList.add(new CIM_J2eeConnectionStats(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_J2eeConnectionStats.");
						resultArrayList.add(new CIM_J2eeConnectionStats(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_J2eeConnectionStats.");
						resultArrayList.add(new CIM_J2eeConnectionStats(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_J2eeConnectionStats getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_J2eeConnectionStatsHelper.getInstance(cimClient, new CIM_J2eeConnectionStats(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_J2eeConnectionStats getInstance(CIMClient cimClient, CIM_J2eeConnectionStats dataInstance) {
		...
		return CIM_J2eeConnectionStatsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_J2eeConnectionStats getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_J2eeConnectionStats dataInstance = null;
		try {
			dataInstance = new CIM_J2eeConnectionStats(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_J2eeConnectionStats createInstance(CIMClient cimClient, CIM_J2eeConnectionStats dataInstance, boolean notifyDifferencies){ 
		...
		CIM_J2eeConnectionStats newDataInstance = null;
		try {
			newDataInstance = CIM_J2eeConnectionStatsHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_J2eeConnectionStats modifyInstance(CIMClient cimClient, CIM_J2eeConnectionStats dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_J2eeConnectionStatsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_J2eeConnectionStats dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_J2eeConnectionStats dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_J2eeConnectionStats.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_J2eeConnectionStats.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_J2eeConnectionStats.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_J2eeConnectionStats.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_J2eeConnectionStats.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_UseTime(Calendar UseTime) {
		return true;
	}

	public static boolean isValid_UseTimeMaxTime(Calendar UseTimeMaxTime) {
		return true;
	}

	public static boolean isValid_UseTimeMinTime(Calendar UseTimeMinTime) {
		return true;
	}

	public static boolean isValid_UseTimeTotalTime(Calendar UseTimeTotalTime) {
		return true;
	}

	public static boolean isValid_WaitTime(Calendar WaitTime) {
		return true;
	}

	public static boolean isValid_WaitTimeMaxTime(Calendar WaitTimeMaxTime) {
		return true;
	}

	public static boolean isValid_WaitTimeMinTime(Calendar WaitTimeMinTime) {
		return true;
	}

	public static boolean isValid_WaitTimeTotalTime(Calendar WaitTimeTotalTime) {
		return true;
	}
