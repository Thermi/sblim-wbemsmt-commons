...
public final class CIM_VLANServiceHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_VLANService.");
						resultArrayList.add(new CIM_VLANService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_VLANService.");
						resultArrayList.add(new CIM_VLANService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_VLANService.");
						resultArrayList.add(new CIM_VLANService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_VLANService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_VLANServiceHelper.getInstance(cimClient, new CIM_VLANService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_VLANService getInstance(CIMClient cimClient, CIM_VLANService dataInstance) {
		...
		return CIM_VLANServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_VLANService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_VLANService dataInstance = null;
		try {
			dataInstance = new CIM_VLANService(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_VLANService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_VLANService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_VLANService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_VLANService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
