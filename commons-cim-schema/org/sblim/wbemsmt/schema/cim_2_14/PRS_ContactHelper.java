...
public final class PRS_ContactHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Contact.");
						resultArrayList.add(new PRS_Contact(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Contact.");
						resultArrayList.add(new PRS_Contact(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Contact.");
						resultArrayList.add(new PRS_Contact(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Contact getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_ContactHelper.getInstance(cimClient, new PRS_Contact(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Contact getInstance(CIMClient cimClient, PRS_Contact dataInstance) {
		...
		return PRS_ContactHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Contact getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Contact dataInstance = null;
		try {
			dataInstance = new PRS_Contact(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Contact createInstance(CIMClient cimClient, PRS_Contact dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Contact newDataInstance = null;
		try {
			newDataInstance = PRS_ContactHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Contact modifyInstance(CIMClient cimClient, PRS_Contact dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_ContactHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Contact dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Contact dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Contact.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Contact.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Contact.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Contact.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Contact.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
