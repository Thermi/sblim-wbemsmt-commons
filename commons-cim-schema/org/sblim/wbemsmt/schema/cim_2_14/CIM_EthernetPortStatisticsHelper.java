/** 
 * CIM_EthernetPortStatisticsHelper.java
 *
...
public final class CIM_EthernetPortStatisticsHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_EthernetPortStatistics(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_EthernetPortStatistics.Java_Package_List.size(); i++) {
						if (!((String)(CIM_EthernetPortStatistics.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_EthernetPortStatistics.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_EthernetPortStatistics.Java_Package_List.setElementAt((String)(CIM_EthernetPortStatistics.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_EthernetPortStatistics.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_EthernetPortStatistics.");
						resultArrayList.add(new CIM_EthernetPortStatistics(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_EthernetPortStatistics.");
						resultArrayList.add(new CIM_EthernetPortStatistics(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_EthernetPortStatistics.");
						resultArrayList.add(new CIM_EthernetPortStatistics(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_EthernetPortStatistics getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_EthernetPortStatisticsHelper.getInstance(cimClient, new CIM_EthernetPortStatistics(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_EthernetPortStatistics getInstance(CIMClient cimClient, CIM_EthernetPortStatistics dataInstance) {
		...
		return CIM_EthernetPortStatisticsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_EthernetPortStatistics getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_EthernetPortStatistics dataInstance = null;
		try {
			dataInstance = new CIM_EthernetPortStatistics(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_EthernetPortStatistics createInstance(CIMClient cimClient, CIM_EthernetPortStatistics dataInstance, boolean notifyDifferencies){ 
		...
		CIM_EthernetPortStatistics newDataInstance = null;
		try {
			newDataInstance = CIM_EthernetPortStatisticsHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_EthernetPortStatistics modifyInstance(CIMClient cimClient, CIM_EthernetPortStatistics dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_EthernetPortStatisticsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_EthernetPortStatistics dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_EthernetPortStatistics dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_EthernetPortStatistics.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_EthernetPortStatistics.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_EthernetPortStatistics.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_EthernetPortStatistics.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_EthernetPortStatistics.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AlignmentErrors(UnsignedInt32 AlignmentErrors) {
		return true;
	}

	public static boolean isValid_CarrierSenseErrors(UnsignedInt32 CarrierSenseErrors) {
		return true;
	}

	public static boolean isValid_DeferredTransmissions(UnsignedInt32 DeferredTransmissions) {
		return true;
	}

	public static boolean isValid_ExcessiveCollisions(UnsignedInt32 ExcessiveCollisions) {
		return true;
	}

	public static boolean isValid_FCSErrors(UnsignedInt32 FCSErrors) {
		return true;
	}

	public static boolean isValid_FrameTooLongs(UnsignedInt32 FrameTooLongs) {
		return true;
	}

	public static boolean isValid_InternalMACReceiveErrors(UnsignedInt32 InternalMACReceiveErrors) {
		return true;
	}

	public static boolean isValid_InternalMACTransmitErrors(UnsignedInt32 InternalMACTransmitErrors) {
		return true;
	}

	public static boolean isValid_LateCollisions(UnsignedInt32 LateCollisions) {
		return true;
	}

	public static boolean isValid_MultipleCollisionFrames(UnsignedInt32 MultipleCollisionFrames) {
		return true;
	}

	public static boolean isValid_PacketsReceived(UnsignedInt64 PacketsReceived) {
		return true;
	}

	public static boolean isValid_PacketsTransmitted(UnsignedInt64 PacketsTransmitted) {
		return true;
	}

	public static boolean isValid_SQETestErrors(UnsignedInt32 SQETestErrors) {
		return true;
	}

	public static boolean isValid_SingleCollisionFrames(UnsignedInt32 SingleCollisionFrames) {
		return true;
	}

	public static boolean isValid_SymbolErrors(UnsignedInt32 SymbolErrors) {
		return true;
	}
