...
public final class CIM_OSProcessHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_OSProcess(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_OSProcess.Java_Package_List.size(); i++) {
						if (!((String)(CIM_OSProcess.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_OSProcess.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_OSProcess.Java_Package_List.setElementAt((String)(CIM_OSProcess.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_OSProcess.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static CIM_OSProcess getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_OSProcessHelper.getInstance(cimClient, new CIM_OSProcess(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_OSProcess getInstance(CIMClient cimClient, CIM_OSProcess dataInstance) {
		...
		return CIM_OSProcessHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_OSProcess getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_OSProcess(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_OSProcess createInstance(CIMClient cimClient, CIM_OSProcess dataInstance, boolean notifyDifferencies){ 
		...
		CIM_OSProcess newDataInstance = null;
		try {
			newDataInstance = CIM_OSProcessHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_OSProcess modifyInstance(CIMClient cimClient, CIM_OSProcess dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_OSProcessHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_OSProcess dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_OSProcess dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_OSProcess.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_OSProcess.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_OSProcess.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_OSProcess.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_OSProcess.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
