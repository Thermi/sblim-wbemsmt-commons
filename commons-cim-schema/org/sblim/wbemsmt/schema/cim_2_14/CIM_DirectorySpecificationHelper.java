/** 
 * CIM_DirectorySpecificationHelper.java
 *
...
public final class CIM_DirectorySpecificationHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_DirectorySpecification.Java_Package_List.size(); i++) {
						if (!((String)(CIM_DirectorySpecification.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_DirectorySpecification.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_DirectorySpecification.Java_Package_List.setElementAt((String)(CIM_DirectorySpecification.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_DirectorySpecification.");
						resultArrayList.add(new CIM_DirectorySpecification(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_DirectorySpecification.");
						resultArrayList.add(new CIM_DirectorySpecification(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_DirectorySpecification.");
						resultArrayList.add(new CIM_DirectorySpecification(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_DirectorySpecification getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_DirectorySpecificationHelper.getInstance(cimClient, new CIM_DirectorySpecification(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DirectorySpecification getInstance(CIMClient cimClient, CIM_DirectorySpecification dataInstance) {
		...
		return CIM_DirectorySpecificationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DirectorySpecification getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_DirectorySpecification dataInstance = null;
		try {
			dataInstance = new CIM_DirectorySpecification(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_DirectorySpecification createInstance(CIMClient cimClient, CIM_DirectorySpecification dataInstance, boolean notifyDifferencies){ 
		...
		CIM_DirectorySpecification newDataInstance = null;
		try {
			newDataInstance = CIM_DirectorySpecificationHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_DirectorySpecification modifyInstance(CIMClient cimClient, CIM_DirectorySpecification dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_DirectorySpecificationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_DirectorySpecification dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_DirectorySpecification dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_DirectorySpecification.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_DirectorySpecification.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_DirectorySpecification.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_DirectorySpecification.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_DirectorySpecification.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_DirectoryPath(String DirectoryPath) {
		return true;
	}

	public static boolean isValid_DirectoryType(UnsignedInt16 DirectoryType) {
		return true;
	}
