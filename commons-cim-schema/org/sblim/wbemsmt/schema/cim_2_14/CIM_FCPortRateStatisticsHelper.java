/** 
 * CIM_FCPortRateStatisticsHelper.java
 *
...
public final class CIM_FCPortRateStatisticsHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_FCPortRateStatistics(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_FCPortRateStatistics.Java_Package_List.size(); i++) {
						if (!((String)(CIM_FCPortRateStatistics.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_FCPortRateStatistics.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_FCPortRateStatistics.Java_Package_List.setElementAt((String)(CIM_FCPortRateStatistics.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_FCPortRateStatistics.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_FCPortRateStatistics.");
						resultArrayList.add(new CIM_FCPortRateStatistics(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_FCPortRateStatistics.");
						resultArrayList.add(new CIM_FCPortRateStatistics(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_FCPortRateStatistics.");
						resultArrayList.add(new CIM_FCPortRateStatistics(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_FCPortRateStatistics getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_FCPortRateStatisticsHelper.getInstance(cimClient, new CIM_FCPortRateStatistics(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_FCPortRateStatistics getInstance(CIMClient cimClient, CIM_FCPortRateStatistics dataInstance) {
		...
		return CIM_FCPortRateStatisticsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_FCPortRateStatistics getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_FCPortRateStatistics dataInstance = null;
		try {
			dataInstance = new CIM_FCPortRateStatistics(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_FCPortRateStatistics createInstance(CIMClient cimClient, CIM_FCPortRateStatistics dataInstance, boolean notifyDifferencies){ 
		...
		CIM_FCPortRateStatistics newDataInstance = null;
		try {
			newDataInstance = CIM_FCPortRateStatisticsHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_FCPortRateStatistics modifyInstance(CIMClient cimClient, CIM_FCPortRateStatistics dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_FCPortRateStatisticsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_FCPortRateStatistics dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_FCPortRateStatistics dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_FCPortRateStatistics.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_FCPortRateStatistics.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_FCPortRateStatistics.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_FCPortRateStatistics.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_FCPortRateStatistics.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_MaxRxFrameRate(UnsignedInt64 MaxRxFrameRate) {
		return true;
	}

	public static boolean isValid_MaxTxFrameRate(UnsignedInt64 MaxTxFrameRate) {
		return true;
	}

	public static boolean isValid_PeakRxRate(UnsignedInt64 PeakRxRate) {
		return true;
	}

	public static boolean isValid_PeakTxRate(UnsignedInt64 PeakTxRate) {
		return true;
	}

	public static boolean isValid_RxFrameRate(UnsignedInt64 RxFrameRate) {
		return true;
	}

	public static boolean isValid_RxRate(UnsignedInt64 RxRate) {
		return true;
	}

	public static boolean isValid_TxFrameRate(UnsignedInt64 TxFrameRate) {
		return true;
	}

	public static boolean isValid_TxRate(UnsignedInt64 TxRate) {
		return true;
	}
