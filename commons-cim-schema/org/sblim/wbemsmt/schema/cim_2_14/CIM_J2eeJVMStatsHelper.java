...
public final class CIM_J2eeJVMStatsHelper {

	public final static String CIM_CLASS_NAME = "CIM_J2eeJVMStats";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_J2eeJVMStats(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_J2eeJVMStats.Java_Package_List.size(); i++) {
						if (!((String)(CIM_J2eeJVMStats.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_J2eeJVMStats.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_J2eeJVMStats.Java_Package_List.setElementAt((String)(CIM_J2eeJVMStats.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_J2eeJVMStats.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_J2eeJVMStats.");
						resultArrayList.add(new CIM_J2eeJVMStats(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_J2eeJVMStats.");
						resultArrayList.add(new CIM_J2eeJVMStats(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_J2eeJVMStats.");
						resultArrayList.add(new CIM_J2eeJVMStats(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_J2eeJVMStats getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_J2eeJVMStatsHelper.getInstance(cimClient, new CIM_J2eeJVMStats(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_J2eeJVMStats getInstance(CIMClient cimClient, CIM_J2eeJVMStats dataInstance) {
		...
		return CIM_J2eeJVMStatsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_J2eeJVMStats getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_J2eeJVMStats dataInstance = null;
		try {
			dataInstance = new CIM_J2eeJVMStats(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_J2eeJVMStats createInstance(CIMClient cimClient, CIM_J2eeJVMStats dataInstance, boolean notifyDifferencies){ 
		...
		CIM_J2eeJVMStats newDataInstance = null;
		try {
			newDataInstance = CIM_J2eeJVMStatsHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_J2eeJVMStats modifyInstance(CIMClient cimClient, CIM_J2eeJVMStats dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_J2eeJVMStatsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_J2eeJVMStats dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_J2eeJVMStats dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_J2eeJVMStats.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_J2eeJVMStats.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_J2eeJVMStats.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_J2eeJVMStats.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_J2eeJVMStats.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_HeapSize(UnsignedInt64 HeapSize) {
		return true;
	}

	public static boolean isValid_HeapSizeHighWaterMark(UnsignedInt64 HeapSizeHighWaterMark) {
		return true;
	}

	public static boolean isValid_HeapSizeLowWaterMark(UnsignedInt64 HeapSizeLowWaterMark) {
		return true;
	}

	public static boolean isValid_HeapSizeLowerBound(UnsignedInt64 HeapSizeLowerBound) {
		return true;
	}

	public static boolean isValid_HeapSizeUpperBound(UnsignedInt64 HeapSizeUpperBound) {
		return true;
	}

	public static boolean isValid_UpTime(Calendar UpTime) {
		return true;
	}
