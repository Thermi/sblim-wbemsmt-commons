...
public final class PRS_ReferenceHelper {

	public final static String CIM_CLASS_NAME = "PRS_Reference";
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<PRS_Reference.Java_Package_List.size(); i++) {
						if (!((String)(PRS_Reference.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(PRS_Reference.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							PRS_Reference.Java_Package_List.setElementAt((String)(PRS_Reference.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (PRS_Reference.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Reference.");
						resultArrayList.add(new PRS_Reference(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Reference.");
						resultArrayList.add(new PRS_Reference(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Reference.");
						resultArrayList.add(new PRS_Reference(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Reference getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_ReferenceHelper.getInstance(cimClient, new PRS_Reference(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Reference getInstance(CIMClient cimClient, PRS_Reference dataInstance) {
		...
		return PRS_ReferenceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Reference getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Reference dataInstance = null;
		try {
			dataInstance = new PRS_Reference(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Reference createInstance(CIMClient cimClient, PRS_Reference dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Reference newDataInstance = null;
		try {
			newDataInstance = PRS_ReferenceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Reference modifyInstance(CIMClient cimClient, PRS_Reference dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_ReferenceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Reference dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Reference dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Reference.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Reference.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Reference.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Reference.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Reference.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
