...
public final class CIM_StorageLibraryHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_StorageLibrary.Java_Package_List.size(); i++) {
						if (!((String)(CIM_StorageLibrary.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_StorageLibrary.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_StorageLibrary.Java_Package_List.setElementAt((String)(CIM_StorageLibrary.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_StorageLibrary.");
						resultArrayList.add(new CIM_StorageLibrary(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_StorageLibrary.");
						resultArrayList.add(new CIM_StorageLibrary(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_StorageLibrary.");
						resultArrayList.add(new CIM_StorageLibrary(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_StorageLibrary getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_StorageLibraryHelper.getInstance(cimClient, new CIM_StorageLibrary(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_StorageLibrary getInstance(CIMClient cimClient, CIM_StorageLibrary dataInstance) {
		...
		return CIM_StorageLibraryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_StorageLibrary getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_StorageLibrary dataInstance = null;
		try {
			dataInstance = new CIM_StorageLibrary(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_StorageLibrary createInstance(CIMClient cimClient, CIM_StorageLibrary dataInstance, boolean notifyDifferencies){ 
		...
		CIM_StorageLibrary newDataInstance = null;
		try {
			newDataInstance = CIM_StorageLibraryHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_StorageLibrary modifyInstance(CIMClient cimClient, CIM_StorageLibrary dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_StorageLibraryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_StorageLibrary dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_StorageLibrary dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_StorageLibrary.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_StorageLibrary.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_StorageLibrary.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_StorageLibrary.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_StorageLibrary.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AuditInProgress(Boolean AuditInProgress) {
		return true;
	}

	public static boolean isValid_AuditNeeded(Boolean AuditNeeded) {
		return true;
	}

	public static boolean isValid_Automated(Boolean Automated) {
		return true;
	}
	...
	public static boolean isValid_MaxAuditTime(UnsignedInt64 MaxAuditTime) {
		return true;
	}

	public static boolean isValid_Overfilled(Boolean Overfilled) {
		return true;
	}

	public static boolean isValid_RoboticsEnabled(Boolean RoboticsEnabled) {
		return true;
	}
