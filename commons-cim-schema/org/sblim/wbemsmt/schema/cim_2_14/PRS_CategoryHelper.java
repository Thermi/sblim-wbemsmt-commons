...
public final class PRS_CategoryHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Category.");
						resultArrayList.add(new PRS_Category(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Category.");
						resultArrayList.add(new PRS_Category(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Category.");
						resultArrayList.add(new PRS_Category(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Category getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_CategoryHelper.getInstance(cimClient, new PRS_Category(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Category getInstance(CIMClient cimClient, PRS_Category dataInstance) {
		...
		return PRS_CategoryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Category getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Category dataInstance = null;
		try {
			dataInstance = new PRS_Category(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Category createInstance(CIMClient cimClient, PRS_Category dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Category newDataInstance = null;
		try {
			newDataInstance = PRS_CategoryHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Category modifyInstance(CIMClient cimClient, PRS_Category dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_CategoryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Category dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Category dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Category.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Category.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Category.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Category.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Category.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CategoryContents(String CategoryContents) {
		return true;
	}

	public static boolean isValid_CategoryType(String CategoryType) {
		return true;
	}
