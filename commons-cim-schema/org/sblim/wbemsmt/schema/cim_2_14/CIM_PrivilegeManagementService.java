 ...
 *
 *
 * Description:  The PrivilegeManagementService is responsible for creating, deleting, and
 * associating AuthorizedPrivilege instances. References to 'subject' and
 * 'target' define the entities that are associated with an AuthorizedPrivilege
 * instance via the relationships, AuthorizedSubject and AuthorizedTarget,
 * respectively. When created, an AuthorizedPrivilege instance is related to
 * this (PrivilegeManagement)Service via the association, ConcreteDependency.
 * 
 */
...
/**
 *  The PrivilegeManagementService is responsible for creating, deleting, and
 * associating AuthorizedPrivilege instances. References to 'subject' and
 * 'target' define the entities that are associated with an AuthorizedPrivilege
 * instance via the relationships, AuthorizedSubject and AuthorizedTarget,
 * respectively. When created, an AuthorizedPrivilege instance is related to
...
public class CIM_PrivilegeManagementService extends CIM_AuthorizationService  {
	...
	public final static String CIM_CLASS_NAME = "CIM_PrivilegeManagementService"; //$NON-NLS-1$
	...
	*/
	...
	static {
		...
		for (int i = 0; i < CIM_AuthorizationService.Java_Package_List.size(); i++) {
			if (((String)CIM_AuthorizationService.Java_Package_List.elementAt(i)).equals("org.sblim.wbemsmt.schema.cim_2_14")){
				continue;
			}
