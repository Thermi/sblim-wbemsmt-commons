...
public final class CIM_EnabledLogicalElementCapabilitiesHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_EnabledLogicalElementCapabilities(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_EnabledLogicalElementCapabilities.Java_Package_List.size(); i++) {
						if (!((String)(CIM_EnabledLogicalElementCapabilities.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_EnabledLogicalElementCapabilities.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_EnabledLogicalElementCapabilities.Java_Package_List.setElementAt((String)(CIM_EnabledLogicalElementCapabilities.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_EnabledLogicalElementCapabilities.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static CIM_EnabledLogicalElementCapabilities getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_EnabledLogicalElementCapabilitiesHelper.getInstance(cimClient, new CIM_EnabledLogicalElementCapabilities(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_EnabledLogicalElementCapabilities getInstance(CIMClient cimClient, CIM_EnabledLogicalElementCapabilities dataInstance) {
		...
		return CIM_EnabledLogicalElementCapabilitiesHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_EnabledLogicalElementCapabilities getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_EnabledLogicalElementCapabilities(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_EnabledLogicalElementCapabilities createInstance(CIMClient cimClient, CIM_EnabledLogicalElementCapabilities dataInstance, boolean notifyDifferencies){ 
		...
		CIM_EnabledLogicalElementCapabilities newDataInstance = null;
		try {
			newDataInstance = CIM_EnabledLogicalElementCapabilitiesHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_EnabledLogicalElementCapabilities modifyInstance(CIMClient cimClient, CIM_EnabledLogicalElementCapabilities dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_EnabledLogicalElementCapabilitiesHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_EnabledLogicalElementCapabilities dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_EnabledLogicalElementCapabilities dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_EnabledLogicalElementCapabilities.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_EnabledLogicalElementCapabilities.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_EnabledLogicalElementCapabilities.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_EnabledLogicalElementCapabilities.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_EnabledLogicalElementCapabilities.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ElementNameEditSupported(Boolean ElementNameEditSupported) {
		return true;
	}

	public static boolean isValid_MaxElementNameLen(UnsignedInt16 MaxElementNameLen) {
		return true;
	}

	public static boolean isValid_RequestedStatesSupported(UnsignedInt16[] RequestedStatesSupported) {
		return true;
	}
