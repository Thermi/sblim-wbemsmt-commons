...
public final class CIM_IRQHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_IRQ.Java_Package_List.size(); i++) {
						if (!((String)(CIM_IRQ.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_IRQ.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_IRQ.Java_Package_List.setElementAt((String)(CIM_IRQ.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_IRQ.");
						resultArrayList.add(new CIM_IRQ(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_IRQ.");
						resultArrayList.add(new CIM_IRQ(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_IRQ.");
						resultArrayList.add(new CIM_IRQ(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_IRQ getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_IRQHelper.getInstance(cimClient, new CIM_IRQ(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_IRQ getInstance(CIMClient cimClient, CIM_IRQ dataInstance) {
		...
		return CIM_IRQHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_IRQ getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_IRQ dataInstance = null;
		try {
			dataInstance = new CIM_IRQ(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_IRQ createInstance(CIMClient cimClient, CIM_IRQ dataInstance, boolean notifyDifferencies){ 
		...
		CIM_IRQ newDataInstance = null;
		try {
			newDataInstance = CIM_IRQHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_IRQ modifyInstance(CIMClient cimClient, CIM_IRQ dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_IRQHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_IRQ dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_IRQ dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_IRQ.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_IRQ.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_IRQ.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_IRQ.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_IRQ.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Hardware(Boolean Hardware) {
		return true;
	}

	public static boolean isValid_IRQNumber(UnsignedInt32 IRQNumber) {
		return true;
	}

	public static boolean isValid_Shareable(Boolean Shareable) {
		return true;
	}

	public static boolean isValid_TriggerLevel(UnsignedInt16 TriggerLevel) {
		return true;
	}

	public static boolean isValid_TriggerType(UnsignedInt16 TriggerType) {
		return true;
	}
