 ...
 *
 *
 * Description:  Slots are special types of Connectors into which adapter Cards are typically
 * inserted. The SlotInSlot relationship represents the ability of a special
 * adapter to extend the existing Slot structure to enable otherwise
 * incompatible Cards to be plugged into a Frame or Hosting Board. The adapter
 * effectively creates a new Slot and can be thought of (conceptually) as a Slot
 * in a Slot. This enables Cards that would otherwise be physically and/or
 * electrically incompatible with the existing Slots to be supported, by
 * interfacing to the Slot provided by the adapter. This has many practical
 * uses. For example, networking boards are very expensive. As new hardware
 * becomes available, Chassis and even Card configurations change. To protect
 * the investment of their customers, networking vendors will manufacture
 * special adapters that enable old Cards to fit into new Chassis or Hosting
 * Boards and/or new Cards to fit into old. This is done using a special adapter
 * that fits over one or more existing Slots and presents a new Slot into which
 * the Card can plug.
 * 
 */
...
/**
 *  Slots are special types of Connectors into which adapter Cards are typically
 * inserted. The SlotInSlot relationship represents the ability of a special
 * adapter to extend the existing Slot structure to enable otherwise
 * incompatible Cards to be plugged into a Frame or Hosting Board. The adapter
 * effectively creates a new Slot and can be thought of (conceptually) as a Slot
...
public class CIM_SlotInSlot extends CIM_ConnectedTo  {
	...
	public final static String CIM_CLASS_NAME = "CIM_SlotInSlot"; //$NON-NLS-1$
	...
	static {
		...
		for (int i = 0; i < CIM_ConnectedTo.Java_Package_List.size(); i++) {
			if (((String)CIM_ConnectedTo.Java_Package_List.elementAt(i)).equals("org.sblim.wbemsmt.schema.cim_2_14")){
				continue;
			}
