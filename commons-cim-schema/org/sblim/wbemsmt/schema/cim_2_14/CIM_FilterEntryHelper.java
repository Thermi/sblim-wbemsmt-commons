...
public final class CIM_FilterEntryHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_FilterEntry(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_FilterEntry.Java_Package_List.size(); i++) {
						if (!((String)(CIM_FilterEntry.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_FilterEntry.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_FilterEntry.Java_Package_List.setElementAt((String)(CIM_FilterEntry.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_FilterEntry.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_FilterEntry.");
						resultArrayList.add(new CIM_FilterEntry(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_FilterEntry.");
						resultArrayList.add(new CIM_FilterEntry(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_FilterEntry.");
						resultArrayList.add(new CIM_FilterEntry(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_FilterEntry getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_FilterEntryHelper.getInstance(cimClient, new CIM_FilterEntry(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_FilterEntry getInstance(CIMClient cimClient, CIM_FilterEntry dataInstance) {
		...
		return CIM_FilterEntryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_FilterEntry getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_FilterEntry dataInstance = null;
		try {
			dataInstance = new CIM_FilterEntry(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_FilterEntry createInstance(CIMClient cimClient, CIM_FilterEntry dataInstance, boolean notifyDifferencies){ 
		...
		CIM_FilterEntry newDataInstance = null;
		try {
			newDataInstance = CIM_FilterEntryHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_FilterEntry modifyInstance(CIMClient cimClient, CIM_FilterEntry dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_FilterEntryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_FilterEntry dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_FilterEntry dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_FilterEntry.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_FilterEntry.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_FilterEntry.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_FilterEntry.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_FilterEntry.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Action(UnsignedInt16 Action) {
		return true;
	}

	public static boolean isValid_DefaultFilter(Boolean DefaultFilter) {
		return true;
	}
	...
	public static boolean isValid_MatchConditionValue(String MatchConditionValue) {
		return true;
	}

	public static boolean isValid_OtherMatchConditionType(String OtherMatchConditionType) {
		return true;
	}

	public static boolean isValid_OtherTrafficType(String OtherTrafficType) {
		return true;
	}

	public static boolean isValid_TrafficClass(String TrafficClass) {
		return true;
	}
