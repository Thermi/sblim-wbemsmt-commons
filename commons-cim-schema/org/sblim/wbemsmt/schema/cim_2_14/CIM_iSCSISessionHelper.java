...
public final class CIM_iSCSISessionHelper {

	public final static String CIM_CLASS_NAME = "CIM_iSCSISession";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_iSCSISession(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_iSCSISession.Java_Package_List.size(); i++) {
						if (!((String)(CIM_iSCSISession.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_iSCSISession.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_iSCSISession.Java_Package_List.setElementAt((String)(CIM_iSCSISession.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_iSCSISession.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_iSCSISession.");
						resultArrayList.add(new CIM_iSCSISession(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_iSCSISession.");
						resultArrayList.add(new CIM_iSCSISession(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_iSCSISession.");
						resultArrayList.add(new CIM_iSCSISession(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_iSCSISession getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_iSCSISessionHelper.getInstance(cimClient, new CIM_iSCSISession(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_iSCSISession getInstance(CIMClient cimClient, CIM_iSCSISession dataInstance) {
		...
		return CIM_iSCSISessionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_iSCSISession getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_iSCSISession dataInstance = null;
		try {
			dataInstance = new CIM_iSCSISession(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_iSCSISession createInstance(CIMClient cimClient, CIM_iSCSISession dataInstance, boolean notifyDifferencies){ 
		...
		CIM_iSCSISession newDataInstance = null;
		try {
			newDataInstance = CIM_iSCSISessionHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_iSCSISession modifyInstance(CIMClient cimClient, CIM_iSCSISession dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_iSCSISessionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_iSCSISession dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_iSCSISession dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_iSCSISession.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_iSCSISession.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_iSCSISession.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_iSCSISession.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_iSCSISession.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CurrentConnections(UnsignedInt32 CurrentConnections) {
		return true;
	}

	public static boolean isValid_DataPDUInOrder(Boolean DataPDUInOrder) {
		return true;
	}

	public static boolean isValid_DataSequenceInOrder(Boolean DataSequenceInOrder) {
		return true;
	}

	public static boolean isValid_DefaultTimeToRetain(UnsignedInt32 DefaultTimeToRetain) {
		return true;
	}

	public static boolean isValid_DefaultTimeToWait(UnsignedInt32 DefaultTimeToWait) {
		return true;
	}

	public static boolean isValid_Directionality(UnsignedInt16 Directionality) {
		return true;
	}

	public static boolean isValid_EndPointName(String EndPointName) {
		return true;
	}

	public static boolean isValid_ErrorRecoveryLevel(UnsignedInt32 ErrorRecoveryLevel) {
		return true;
	}

	public static boolean isValid_ImmediateData(Boolean ImmediateData) {
		return true;
	}

	public static boolean isValid_InitialR2T(Boolean InitialR2T) {
		return true;
	}
	...
	public static boolean isValid_SessionType(UnsignedInt16 SessionType) {
		return true;
	}

	public static boolean isValid_TSIH(UnsignedInt32 TSIH) {
		return true;
	}
