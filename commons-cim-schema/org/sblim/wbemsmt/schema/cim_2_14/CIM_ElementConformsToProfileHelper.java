...
public final class CIM_ElementConformsToProfileHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_ElementConformsToProfile(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_ElementConformsToProfile.Java_Package_List.size(); i++) {
						if (!((String)(CIM_ElementConformsToProfile.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_ElementConformsToProfile.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_ElementConformsToProfile.Java_Package_List.setElementAt((String)(CIM_ElementConformsToProfile.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_ElementConformsToProfile.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static CIM_ElementConformsToProfile getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ElementConformsToProfileHelper.getInstance(cimClient, new CIM_ElementConformsToProfile(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ElementConformsToProfile getInstance(CIMClient cimClient, CIM_ElementConformsToProfile dataInstance) {
		...
		return CIM_ElementConformsToProfileHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ElementConformsToProfile getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_ElementConformsToProfile(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ElementConformsToProfile createInstance(CIMClient cimClient, CIM_ElementConformsToProfile dataInstance, boolean notifyDifferencies){ 
		...
		CIM_ElementConformsToProfile newDataInstance = null;
		try {
			newDataInstance = CIM_ElementConformsToProfileHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ElementConformsToProfile modifyInstance(CIMClient cimClient, CIM_ElementConformsToProfile dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_ElementConformsToProfileHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_ElementConformsToProfile dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_ElementConformsToProfile dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_ElementConformsToProfile.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_ElementConformsToProfile.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_ElementConformsToProfile.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_ElementConformsToProfile.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_ElementConformsToProfile.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
