/** 
 * PRS_SolutionElementHelper.java
 *
...
public final class PRS_SolutionElementHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<PRS_SolutionElement.Java_Package_List.size(); i++) {
						if (!((String)(PRS_SolutionElement.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(PRS_SolutionElement.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							PRS_SolutionElement.Java_Package_List.setElementAt((String)(PRS_SolutionElement.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_SolutionElement.");
						resultArrayList.add(new PRS_SolutionElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_SolutionElement.");
						resultArrayList.add(new PRS_SolutionElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_SolutionElement.");
						resultArrayList.add(new PRS_SolutionElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_SolutionElement getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_SolutionElementHelper.getInstance(cimClient, new PRS_SolutionElement(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_SolutionElement getInstance(CIMClient cimClient, PRS_SolutionElement dataInstance) {
		...
		return PRS_SolutionElementHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_SolutionElement getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_SolutionElement dataInstance = null;
		try {
			dataInstance = new PRS_SolutionElement(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_SolutionElement.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_SolutionElement.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_SolutionElement.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_SolutionElement.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
