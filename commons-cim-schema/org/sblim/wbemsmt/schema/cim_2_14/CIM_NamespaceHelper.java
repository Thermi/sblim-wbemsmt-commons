...
public final class CIM_NamespaceHelper {
	...
	*/	
	public final static CIM_Namespace getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_NamespaceHelper.getInstance(cimClient, new CIM_Namespace(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Namespace getInstance(CIMClient cimClient, CIM_Namespace dataInstance) {
		...
		return CIM_NamespaceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Namespace getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_Namespace(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_Namespace.getPackages();
	...
	*/	
	public final static CIM_Namespace createInstance(CIMClient cimClient, CIM_Namespace dataInstance, boolean notifyDifferencies){ 
		...
		CIM_Namespace newDataInstance = null;
		try {
			newDataInstance = CIM_NamespaceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Namespace modifyInstance(CIMClient cimClient, CIM_Namespace dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_NamespaceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_Namespace dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_Namespace dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_Namespace.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Namespace.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Namespace.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Namespace.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Namespace.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ClassInfo(UnsignedInt16 ClassInfo) {
		return true;
	}

	public static boolean isValid_ClassType(UnsignedInt16 ClassType) {
		return true;
	}

	public static boolean isValid_ClassTypeVersion(String ClassTypeVersion) {
		return true;
	}
	...
	public static boolean isValid_DescriptionOfClassInfo(String DescriptionOfClassInfo) {
		return true;
	}

	public static boolean isValid_DescriptionOfClassType(String DescriptionOfClassType) {
		return true;
	}
