...
public final class CIM_DropperServiceHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_DropperService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_DropperService.Java_Package_List.size(); i++) {
						if (!((String)(CIM_DropperService.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_DropperService.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_DropperService.Java_Package_List.setElementAt((String)(CIM_DropperService.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_DropperService.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_DropperService.");
						resultArrayList.add(new CIM_DropperService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_DropperService.");
						resultArrayList.add(new CIM_DropperService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_DropperService.");
						resultArrayList.add(new CIM_DropperService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_DropperService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_DropperServiceHelper.getInstance(cimClient, new CIM_DropperService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DropperService getInstance(CIMClient cimClient, CIM_DropperService dataInstance) {
		...
		return CIM_DropperServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DropperService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_DropperService dataInstance = null;
		try {
			dataInstance = new CIM_DropperService(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_DropperService createInstance(CIMClient cimClient, CIM_DropperService dataInstance, boolean notifyDifferencies){ 
		...
		CIM_DropperService newDataInstance = null;
		try {
			newDataInstance = CIM_DropperServiceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_DropperService modifyInstance(CIMClient cimClient, CIM_DropperService dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_DropperServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_DropperService dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_DropperService dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_DropperService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_DropperService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_DropperService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_DropperService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_DropperService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AlwaysDrop(UnsignedInt16 AlwaysDrop) {
		return true;
	}

	public static boolean isValid_DropFrom(UnsignedInt16 DropFrom) {
		return true;
	}

	public static boolean isValid_DropMaintainMetric(UnsignedInt16 DropMaintainMetric) {
		return true;
	}

	public static boolean isValid_DropStartMetric(UnsignedInt16 DropStartMetric) {
		return true;
	}

	public static boolean isValid_DropperType(UnsignedInt16 DropperType) {
		return true;
	}

	public static boolean isValid_OtherDropperType(String OtherDropperType) {
		return true;
	}
