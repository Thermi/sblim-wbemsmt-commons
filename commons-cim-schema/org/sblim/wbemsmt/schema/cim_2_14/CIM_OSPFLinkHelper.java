...
public final class CIM_OSPFLinkHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_OSPFLink.Java_Package_List.size(); i++) {
						if (!((String)(CIM_OSPFLink.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_OSPFLink.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_OSPFLink.Java_Package_List.setElementAt((String)(CIM_OSPFLink.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_OSPFLink.");
						resultArrayList.add(new CIM_OSPFLink(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_OSPFLink.");
						resultArrayList.add(new CIM_OSPFLink(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_OSPFLink.");
						resultArrayList.add(new CIM_OSPFLink(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_OSPFLink getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_OSPFLinkHelper.getInstance(cimClient, new CIM_OSPFLink(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_OSPFLink getInstance(CIMClient cimClient, CIM_OSPFLink dataInstance) {
		...
		return CIM_OSPFLinkHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_OSPFLink getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_OSPFLink dataInstance = null;
		try {
			dataInstance = new CIM_OSPFLink(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_OSPFLink createInstance(CIMClient cimClient, CIM_OSPFLink dataInstance, boolean notifyDifferencies){ 
		...
		CIM_OSPFLink newDataInstance = null;
		try {
			newDataInstance = CIM_OSPFLinkHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_OSPFLink modifyInstance(CIMClient cimClient, CIM_OSPFLink dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_OSPFLinkHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_OSPFLink dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_OSPFLink dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_OSPFLink.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_OSPFLink.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_OSPFLink.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_OSPFLink.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_OSPFLink.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_HelloInterval(UnsignedInt32 HelloInterval) {
		return true;
	}

	public static boolean isValid_LinkType(UnsignedInt16 LinkType) {
		return true;
	}

	public static boolean isValid_MulticastForwarding(UnsignedInt16 MulticastForwarding) {
		return true;
	}

	public static boolean isValid_RouterDeadInterval(UnsignedInt32 RouterDeadInterval) {
		return true;
	}
