 ...
 * If the pipe is not composed of lower-level entities (i.e., its
 * AggregationBehavior property is set to 2), then no instances of this
 * association should be defined where the pipe has the role of GroupComponent.
 * In the context of M.3100, this semantic is modeled as a Trail that is made up
 * of one or more Connections. Both Trails and Connections are subclasses of
 * M.3100's Pipe. Because of the flexibility of the NetworkPipeComposition
 * association, there is no need to subclass NetworkPipe, as was done in M.3100,
 * but merely to instantiate this association to describe the bundling of the
 ...
 * AggregationBehavior, of NetworkPipe. If the pipes are combined in a sequence,
 * the ordering is conveyed via the property, AggregationSequence, on this
 * association.
 * 
 */
...
/**
 *  NetworkPipeComposition describes the makeup a pipe, based on lower-level ones.
 * If the pipe is not composed of lower-level entities (i.e., its
 * AggregationBehavior property is set to 2), then no instances of this
 * association should be defined where the pipe has the role of GroupComponent.
 * In the context of M.3100, this semantic is modeled as a Trail that is made up
...
public class CIM_NetworkPipeComposition extends CIM_Component  {
	...
	public final static String CIM_CLASS_NAME = "CIM_NetworkPipeComposition"; //$NON-NLS-1$
