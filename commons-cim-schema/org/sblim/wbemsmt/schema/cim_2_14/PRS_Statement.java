 ...
 *
 *
 * Description:  PRS_Statement contains the text describing a problem, any relevant background
 * information, and its resolution. Statements are units of text ranging from
 * brief sentences or phrases to paragraphs or pages. There is no limit to the
 * number of PRS_Statements within a Solution, as long as they support the
 * intent and focus of their root class. Root classes include: PRS_Reference,
 * PRS_Problem or PRS_Resolution. PRS_Statements may be grouped together to
 * build compound 'sentences'. For example, StatementA = Printer output is
 * blotchy. StatementB = Print jobs per day are less than 50. ExpressionA =
 * StatementA AND StatementB.
 * 
 */
...
/**
 *  PRS_Statement contains the text describing a problem, any relevant background
 * information, and its resolution. Statements are units of text ranging from
 * brief sentences or phrases to paragraphs or pages. There is no limit to the
 * number of PRS_Statements within a Solution, as long as they support the
 * intent and focus of their root class. Root classes include: PRS_Reference,
...
public class PRS_Statement extends PRS_ExpressionElement  {
	...
	public final static String CIM_CLASS_NAME = "PRS_Statement"; //$NON-NLS-1$
	...
	*/
	...
	static {
		...
		for (int i = 0; i < PRS_ExpressionElement.Java_Package_List.size(); i++) {
			if (((String)PRS_ExpressionElement.Java_Package_List.elementAt(i)).equals("org.sblim.wbemsmt.schema.cim_2_14")){
				continue;
			}
