/** 
 * CIM_IPSOFilterEntryHelper.java
 *
...
public final class CIM_IPSOFilterEntryHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_IPSOFilterEntry(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_IPSOFilterEntry.Java_Package_List.size(); i++) {
						if (!((String)(CIM_IPSOFilterEntry.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_IPSOFilterEntry.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_IPSOFilterEntry.Java_Package_List.setElementAt((String)(CIM_IPSOFilterEntry.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_IPSOFilterEntry.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_IPSOFilterEntry.");
						resultArrayList.add(new CIM_IPSOFilterEntry(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_IPSOFilterEntry.");
						resultArrayList.add(new CIM_IPSOFilterEntry(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_IPSOFilterEntry.");
						resultArrayList.add(new CIM_IPSOFilterEntry(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_IPSOFilterEntry getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_IPSOFilterEntryHelper.getInstance(cimClient, new CIM_IPSOFilterEntry(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_IPSOFilterEntry getInstance(CIMClient cimClient, CIM_IPSOFilterEntry dataInstance) {
		...
		return CIM_IPSOFilterEntryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_IPSOFilterEntry getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_IPSOFilterEntry dataInstance = null;
		try {
			dataInstance = new CIM_IPSOFilterEntry(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_IPSOFilterEntry createInstance(CIMClient cimClient, CIM_IPSOFilterEntry dataInstance, boolean notifyDifferencies){ 
		...
		CIM_IPSOFilterEntry newDataInstance = null;
		try {
			newDataInstance = CIM_IPSOFilterEntryHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_IPSOFilterEntry modifyInstance(CIMClient cimClient, CIM_IPSOFilterEntry dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_IPSOFilterEntryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_IPSOFilterEntry dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_IPSOFilterEntry dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_IPSOFilterEntry.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_IPSOFilterEntry.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_IPSOFilterEntry.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_IPSOFilterEntry.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_IPSOFilterEntry.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ClassificationLevel(UnsignedInt16 ClassificationLevel) {
		return true;
	}

	public static boolean isValid_MatchConditionType(UnsignedInt16 MatchConditionType) {
		return true;
	}

	public static boolean isValid_OtherClassificationLevel(String OtherClassificationLevel) {
		return true;
	}

	public static boolean isValid_OtherProtectionAuthorities(String[] OtherProtectionAuthorities) {
		return true;
	}

	public static boolean isValid_ProtectionAuthorities(UnsignedInt16[] ProtectionAuthorities) {
		return true;
	}
