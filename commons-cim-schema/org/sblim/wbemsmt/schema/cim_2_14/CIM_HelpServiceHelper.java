...
public final class CIM_HelpServiceHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_HelpService.Java_Package_List.size(); i++) {
						if (!((String)(CIM_HelpService.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_HelpService.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_HelpService.Java_Package_List.setElementAt((String)(CIM_HelpService.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_HelpService.");
						resultArrayList.add(new CIM_HelpService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_HelpService.");
						resultArrayList.add(new CIM_HelpService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_HelpService.");
						resultArrayList.add(new CIM_HelpService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_HelpService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_HelpServiceHelper.getInstance(cimClient, new CIM_HelpService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_HelpService getInstance(CIMClient cimClient, CIM_HelpService dataInstance) {
		...
		return CIM_HelpServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_HelpService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_HelpService dataInstance = null;
		try {
			dataInstance = new CIM_HelpService(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_HelpService createInstance(CIMClient cimClient, CIM_HelpService dataInstance, boolean notifyDifferencies){ 
		...
		CIM_HelpService newDataInstance = null;
		try {
			newDataInstance = CIM_HelpServiceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_HelpService modifyInstance(CIMClient cimClient, CIM_HelpService dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_HelpServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_HelpService dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_HelpService dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_HelpService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_HelpService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_HelpService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_HelpService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_HelpService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_DeliveryOptions(UnsignedInt16[] DeliveryOptions) {
		return true;
	}

	public static boolean isValid_DocumentDescriptions(String[] DocumentDescriptions) {
		return true;
	}
	...
	public static boolean isValid_DocumentsAvailable(String[] DocumentsAvailable) {
		return true;
	}

	public static boolean isValid_OtherDeliveryOptionDescription(String OtherDeliveryOptionDescription) {
		return true;
	}
