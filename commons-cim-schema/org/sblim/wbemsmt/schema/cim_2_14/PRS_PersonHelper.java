...
public final class PRS_PersonHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Person.");
						resultArrayList.add(new PRS_Person(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Person.");
						resultArrayList.add(new PRS_Person(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Person.");
						resultArrayList.add(new PRS_Person(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Person getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_PersonHelper.getInstance(cimClient, new PRS_Person(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Person getInstance(CIMClient cimClient, PRS_Person dataInstance) {
		...
		return PRS_PersonHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Person getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Person dataInstance = null;
		try {
			dataInstance = new PRS_Person(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Person createInstance(CIMClient cimClient, PRS_Person dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Person newDataInstance = null;
		try {
			newDataInstance = PRS_PersonHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Person modifyInstance(CIMClient cimClient, PRS_Person dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_PersonHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Person dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Person dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Person.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Person.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Person.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Person.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Person.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CommunicationMode(UnsignedInt8 CommunicationMode) {
		return true;
	}

	public static boolean isValid_ContactType(String ContactType) {
		return true;
	}

	public static boolean isValid_FirstName(String FirstName) {
		return true;
	}

	public static boolean isValid_LastName(String LastName) {
		return true;
	}

	public static boolean isValid_Salutation(String Salutation) {
		return true;
	}
