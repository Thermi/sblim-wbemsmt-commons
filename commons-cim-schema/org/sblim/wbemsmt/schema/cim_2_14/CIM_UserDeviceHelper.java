...
public final class CIM_UserDeviceHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_UserDevice(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_UserDevice.Java_Package_List.size(); i++) {
						if (!((String)(CIM_UserDevice.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_UserDevice.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_UserDevice.Java_Package_List.setElementAt((String)(CIM_UserDevice.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_UserDevice.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_UserDevice.");
						resultArrayList.add(new CIM_UserDevice(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_UserDevice.");
						resultArrayList.add(new CIM_UserDevice(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_UserDevice.");
						resultArrayList.add(new CIM_UserDevice(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_UserDevice getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_UserDeviceHelper.getInstance(cimClient, new CIM_UserDevice(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_UserDevice getInstance(CIMClient cimClient, CIM_UserDevice dataInstance) {
		...
		return CIM_UserDeviceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_UserDevice getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_UserDevice dataInstance = null;
		try {
			dataInstance = new CIM_UserDevice(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_UserDevice.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_UserDevice.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_UserDevice.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_UserDevice.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
