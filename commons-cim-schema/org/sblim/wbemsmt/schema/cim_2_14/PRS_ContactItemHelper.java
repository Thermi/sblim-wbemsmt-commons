...
public final class PRS_ContactItemHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_ContactItem.");
						resultArrayList.add(new PRS_ContactItem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_ContactItem.");
						resultArrayList.add(new PRS_ContactItem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_ContactItem.");
						resultArrayList.add(new PRS_ContactItem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_ContactItem getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_ContactItemHelper.getInstance(cimClient, new PRS_ContactItem(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_ContactItem getInstance(CIMClient cimClient, PRS_ContactItem dataInstance) {
		...
		return PRS_ContactItemHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_ContactItem getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_ContactItem dataInstance = null;
		try {
			dataInstance = new PRS_ContactItem(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_ContactItem.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_ContactItem.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_ContactItem.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_ContactItem.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
