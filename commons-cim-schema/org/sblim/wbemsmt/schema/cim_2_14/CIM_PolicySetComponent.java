 ...
 *
 *
 * Description:  PolicySetComponent is a concrete aggregation that collects instances of the
 * subclasses of PolicySet (i.e., PolicyGroups and PolicyRules). Instances are
 * collected in sets that use the same decision strategy. They are prioritized
 * relative to each other, within the set, using the Priority property of this
 * aggregation. Together, the PolicySet.PolicyDecisionStrategy and PolicySet
 * Component.Priority properties determine the processing for the groups and
 * rules contained in a PolicySet. A larger priority value represents a higher
 * priority. Note that the Priority property MUST have a unique value when
 * compared with others defined for the same aggregating PolicySet. Thus, the
...
/**
 *  PolicySetComponent is a concrete aggregation that collects instances of the
 * subclasses of PolicySet (i.e., PolicyGroups and PolicyRules). Instances are
 * collected in sets that use the same decision strategy. They are prioritized
 * relative to each other, within the set, using the Priority property of this
 * aggregation. Together, the PolicySet.PolicyDecisionStrategy and PolicySet
...
public class CIM_PolicySetComponent extends CIM_PolicyComponent  {
	...
	public final static String CIM_CLASS_NAME = "CIM_PolicySetComponent"; //$NON-NLS-1$
