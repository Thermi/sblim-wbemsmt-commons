/** 
 * CIM_iSCSIConnectionHelper.java
 *
...
public final class CIM_iSCSIConnectionHelper {

	public final static String CIM_CLASS_NAME = "CIM_iSCSIConnection";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_iSCSIConnection(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_iSCSIConnection.Java_Package_List.size(); i++) {
						if (!((String)(CIM_iSCSIConnection.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_iSCSIConnection.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_iSCSIConnection.Java_Package_List.setElementAt((String)(CIM_iSCSIConnection.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_iSCSIConnection.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_iSCSIConnection.");
						resultArrayList.add(new CIM_iSCSIConnection(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_iSCSIConnection.");
						resultArrayList.add(new CIM_iSCSIConnection(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_iSCSIConnection.");
						resultArrayList.add(new CIM_iSCSIConnection(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_iSCSIConnection getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_iSCSIConnectionHelper.getInstance(cimClient, new CIM_iSCSIConnection(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_iSCSIConnection getInstance(CIMClient cimClient, CIM_iSCSIConnection dataInstance) {
		...
		return CIM_iSCSIConnectionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_iSCSIConnection getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_iSCSIConnection dataInstance = null;
		try {
			dataInstance = new CIM_iSCSIConnection(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_iSCSIConnection createInstance(CIMClient cimClient, CIM_iSCSIConnection dataInstance, boolean notifyDifferencies){ 
		...
		CIM_iSCSIConnection newDataInstance = null;
		try {
			newDataInstance = CIM_iSCSIConnectionHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_iSCSIConnection modifyInstance(CIMClient cimClient, CIM_iSCSIConnection dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_iSCSIConnectionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_iSCSIConnection dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_iSCSIConnection dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_iSCSIConnection.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_iSCSIConnection.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_iSCSIConnection.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_iSCSIConnection.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_iSCSIConnection.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ActiveiSCSIVersion(Boolean ActiveiSCSIVersion) {
		return true;
	}

	public static boolean isValid_AuthenticationMethodUsed(UnsignedInt16 AuthenticationMethodUsed) {
		return true;
	}

	public static boolean isValid_ConnectionID(UnsignedInt32 ConnectionID) {
		return true;
	}

	public static boolean isValid_DataDigestMethod(UnsignedInt16 DataDigestMethod) {
		return true;
	}

	public static boolean isValid_HeaderDigestMethod(UnsignedInt16 HeaderDigestMethod) {
		return true;
	}
	...
	public static boolean isValid_MaxTransmitDataSegmentLength(UnsignedInt32 MaxTransmitDataSegmentLength) {
		return true;
	}

	public static boolean isValid_MutualAuthentication(Boolean MutualAuthentication) {
		return true;
	}

	public static boolean isValid_OtherDataDigestMethod(String OtherDataDigestMethod) {
		return true;
	}

	public static boolean isValid_OtherHeaderDigestMethod(String OtherHeaderDigestMethod) {
		return true;
	}

	public static boolean isValid_ReceivingMarkers(Boolean ReceivingMarkers) {
		return true;
	}

	public static boolean isValid_SendingMarkers(Boolean SendingMarkers) {
		return true;
	}
