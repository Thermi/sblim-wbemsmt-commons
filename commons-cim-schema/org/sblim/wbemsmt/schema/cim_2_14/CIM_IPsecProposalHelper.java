...
public final class CIM_IPsecProposalHelper {

	public final static String CIM_CLASS_NAME = "CIM_IPsecProposal";
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_IPsecProposal.Java_Package_List.size(); i++) {
						if (!((String)(CIM_IPsecProposal.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_IPsecProposal.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_IPsecProposal.Java_Package_List.setElementAt((String)(CIM_IPsecProposal.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_IPsecProposal.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_IPsecProposal.");
						resultArrayList.add(new CIM_IPsecProposal(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_IPsecProposal.");
						resultArrayList.add(new CIM_IPsecProposal(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_IPsecProposal.");
						resultArrayList.add(new CIM_IPsecProposal(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_IPsecProposal getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_IPsecProposalHelper.getInstance(cimClient, new CIM_IPsecProposal(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_IPsecProposal getInstance(CIMClient cimClient, CIM_IPsecProposal dataInstance) {
		...
		return CIM_IPsecProposalHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_IPsecProposal getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_IPsecProposal dataInstance = null;
		try {
			dataInstance = new CIM_IPsecProposal(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_IPsecProposal createInstance(CIMClient cimClient, CIM_IPsecProposal dataInstance, boolean notifyDifferencies){ 
		...
		CIM_IPsecProposal newDataInstance = null;
		try {
			newDataInstance = CIM_IPsecProposalHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_IPsecProposal modifyInstance(CIMClient cimClient, CIM_IPsecProposal dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_IPsecProposalHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_IPsecProposal dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_IPsecProposal dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_IPsecProposal.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_IPsecProposal.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_IPsecProposal.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_IPsecProposal.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_IPsecProposal.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
