/** 
 * CIM_OSPFAreaConfigurationHelper.java
 *
...
public final class CIM_OSPFAreaConfigurationHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_OSPFAreaConfiguration.Java_Package_List.size(); i++) {
						if (!((String)(CIM_OSPFAreaConfiguration.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_OSPFAreaConfiguration.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_OSPFAreaConfiguration.Java_Package_List.setElementAt((String)(CIM_OSPFAreaConfiguration.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_OSPFAreaConfiguration.");
						resultArrayList.add(new CIM_OSPFAreaConfiguration(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_OSPFAreaConfiguration.");
						resultArrayList.add(new CIM_OSPFAreaConfiguration(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_OSPFAreaConfiguration.");
						resultArrayList.add(new CIM_OSPFAreaConfiguration(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_OSPFAreaConfiguration getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_OSPFAreaConfigurationHelper.getInstance(cimClient, new CIM_OSPFAreaConfiguration(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_OSPFAreaConfiguration getInstance(CIMClient cimClient, CIM_OSPFAreaConfiguration dataInstance) {
		...
		return CIM_OSPFAreaConfigurationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_OSPFAreaConfiguration getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_OSPFAreaConfiguration dataInstance = null;
		try {
			dataInstance = new CIM_OSPFAreaConfiguration(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_OSPFAreaConfiguration createInstance(CIMClient cimClient, CIM_OSPFAreaConfiguration dataInstance, boolean notifyDifferencies){ 
		...
		CIM_OSPFAreaConfiguration newDataInstance = null;
		try {
			newDataInstance = CIM_OSPFAreaConfigurationHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_OSPFAreaConfiguration modifyInstance(CIMClient cimClient, CIM_OSPFAreaConfiguration dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_OSPFAreaConfigurationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_OSPFAreaConfiguration dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_OSPFAreaConfiguration dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_OSPFAreaConfiguration.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_OSPFAreaConfiguration.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_OSPFAreaConfiguration.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_OSPFAreaConfiguration.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_OSPFAreaConfiguration.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_StubDefaultCost(UnsignedInt32 StubDefaultCost) {
		return true;
	}

	public static boolean isValid_StubMetricType(UnsignedInt16 StubMetricType) {
		return true;
	}
