...
public final class CIM_VirtualComputerSystemHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_VirtualComputerSystem.Java_Package_List.size(); i++) {
						if (!((String)(CIM_VirtualComputerSystem.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_VirtualComputerSystem.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_VirtualComputerSystem.Java_Package_List.setElementAt((String)(CIM_VirtualComputerSystem.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_VirtualComputerSystem.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static CIM_VirtualComputerSystem getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_VirtualComputerSystemHelper.getInstance(cimClient, new CIM_VirtualComputerSystem(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_VirtualComputerSystem getInstance(CIMClient cimClient, CIM_VirtualComputerSystem dataInstance) {
		...
		return CIM_VirtualComputerSystemHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_VirtualComputerSystem getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_VirtualComputerSystem(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_VirtualComputerSystem createInstance(CIMClient cimClient, CIM_VirtualComputerSystem dataInstance, boolean notifyDifferencies){ 
		...
		CIM_VirtualComputerSystem newDataInstance = null;
		try {
			newDataInstance = CIM_VirtualComputerSystemHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_VirtualComputerSystem modifyInstance(CIMClient cimClient, CIM_VirtualComputerSystem dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_VirtualComputerSystemHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_VirtualComputerSystem dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_VirtualComputerSystem dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_VirtualComputerSystem.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_VirtualComputerSystem.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_VirtualComputerSystem.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_VirtualComputerSystem.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_VirtualComputerSystem.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_VirtualSystem(String VirtualSystem) {
		return true;
	}
