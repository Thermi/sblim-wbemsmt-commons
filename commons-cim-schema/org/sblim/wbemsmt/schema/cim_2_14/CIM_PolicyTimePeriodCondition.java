 ...
 *
 *
 * Description:  This class provides a means of representing the time periods during which a
 * PolicySet is valid, i.e., active. At all times that fall outside these time
 * periods, the PolicySet has no effect. A PolicySet is treated as valid at ALL
 * times, if it does not specify a PolicyTimePeriodCondition. In some cases a
 * Policy Consumer may need to perform certain setup / cleanup actions when a
 * PolicySet becomes active / inactive. For example, sessions that were
 * established while a PolicySet was active might need to be taken down when the
 * PolicySet becomes inactive. In other cases, however, such sessions might be
 * left up. In this case, the effect of deactivating the PolicySet would just be
 * to prevent the establishment of new sessions. Setup / cleanup behaviors on
 * validity period transitions are not currently addressed by the Policy Model,
 * and must be specified in 'guideline' documents or via subclasses of
 * CIM_PolicySet, CIM_PolicyTimePeriod Condition or other concrete subclasses of
 * CIM_Policy. If such behaviors need to be under the control of the policy
 * administrator, then a mechanism to allow this control must also be specified
 * in the subclasses. PolicyTimePeriodCondition is defined as a subclass of
 * PolicyCondition. This is to allow the inclusion of time-based criteria in the
 * AND/OR condition definitions for a PolicyRule. Instances of this class may
 * have up to five properties identifying time periods at different levels. The
 * values of all the properties present in an instance are ANDed together to
 * determine the validity period(s) for the instance. For example, an instance
 * with an overall validity range of January 1, 2000 through December 31, 2000;
 * a month mask that selects March and April; a day-of-the-week mask that
 * selects Fridays; and a time of day range of 0800 through 1600 would be
 * represented using the following time periods: Friday, March 5, 2000, from
 * 0800 through 1600; Friday, March 12, 2000, from 0800 through 1600; Friday,
 * March 19, 2000, from 0800 through 1600; Friday, March 26, 2000, from 0800
 * through 1600; Friday, April 2, 2000, from 0800 through 1600; Friday, April 9,
 * 2000, from 0800 through 1600; Friday, April 16, 2000, from 0800 through 1600;
 * Friday, April 23, 2000, from 0800 through 1600; Friday, April 30, 2000, from
 * 0800 through 1600. Properties not present in an instance of
 * PolicyTimePeriodCondition are implicitly treated as having their value
 * 'always enabled'. Thus, in the example above, the day-of-the-month mask is
 * not present, and so the validity period for the instance implicitly includes
 * a day-of-the-month mask that selects all days of the month. If this 'missing
 * property' rule is applied to its fullest, we see that there is a second way
 * to indicate that a PolicySet is always enabled: associate with it an instance
 * of PolicyTimePeriodCondition whose only properties with specific values are
 * its key properties.
 * 
 */
...
public class CIM_PolicyTimePeriodCondition extends CIM_PolicyCondition  {
	...
	public final static String CIM_CLASS_NAME = "CIM_PolicyTimePeriodCondition"; //$NON-NLS-1$
	...
	public final static String CIM_ASSOCIATOR_CLASS_NAME_CIM_POLICYSETVALIDITYPERIOD = "CIM_PolicySetValidityPeriod"; //$NON-NLS-1$
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i = 0; clazz == null && i < CIM_PolicyTimePeriodCondition.Java_Package_List.size(); i++) {
						if (!((String)(CIM_PolicyTimePeriodCondition.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_PolicyTimePeriodCondition.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_PolicyTimePeriodCondition.Java_Package_List.setElementAt((String)(CIM_PolicyTimePeriodCondition.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
