...
public final class PRS_ProblemHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Problem.");
						resultArrayList.add(new PRS_Problem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Problem.");
						resultArrayList.add(new PRS_Problem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Problem.");
						resultArrayList.add(new PRS_Problem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Problem getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_ProblemHelper.getInstance(cimClient, new PRS_Problem(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Problem getInstance(CIMClient cimClient, PRS_Problem dataInstance) {
		...
		return PRS_ProblemHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Problem getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Problem dataInstance = null;
		try {
			dataInstance = new PRS_Problem(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Problem createInstance(CIMClient cimClient, PRS_Problem dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Problem newDataInstance = null;
		try {
			newDataInstance = PRS_ProblemHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Problem modifyInstance(CIMClient cimClient, PRS_Problem dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_ProblemHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Problem dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Problem dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Problem.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Problem.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Problem.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Problem.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Problem.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Impact(UnsignedInt16 Impact) {
		return true;
	}

	public static boolean isValid_Likelihood(UnsignedInt16 Likelihood) {
		return true;
	}
