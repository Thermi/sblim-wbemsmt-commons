 ...
 *
 *
 * Description:  Capabilities and management of an UninterruptiblePowerSupply. The properties of
 * the UPS Device indicate when incoming power is trimmed or boosted, and the
 * aggregated information of the batteries, generators, and so on that comprise
 * the Device. These individual components (for example, multiple batteries) can
...
/**
 *  Capabilities and management of an UninterruptiblePowerSupply. The properties of
 * the UPS Device indicate when incoming power is trimmed or boosted, and the
 * aggregated information of the batteries, generators, and so on that comprise
 * the Device. These individual components (for example, multiple batteries) can
 * also be independently modeled and associated with the UPS.
...
public class CIM_UninterruptiblePowerSupply extends CIM_PowerSupply  {
	...
	public final static String CIM_CLASS_NAME = "CIM_UninterruptiblePowerSupply"; //$NON-NLS-1$
	...
	*/
	...
	/**
	*	EstimatedRunTime is an estimate in minutes of the time that depletion of the battery, generator, or other device, will occur under the present load conditions if the utility power is off or is lost and remains off.
	*/
	...
	*/
	...
	/**
	*	If the voltage (in millivolts) drops below the value specified by Range1InputVoltageLow, the UPS will compensate by boosting the voltage using its power sources. A value of 0 indicates that the voltage at which boosting occurs is 'unknown'.
	*/
	...
	*/
	...
	/**
	*	If the voltage (in millivolts) drops below the value specified by Range2InputVoltageLow, the UPS will compensate by boosting the voltage using its power sources. A value of 0 indicates that the voltage at which boosting occurs is 'unknown'.
	*/
	public final static String CIM_PROPERTY_RANGE2INPUTVOLTAGELOW = "Range2InputVoltageLow"; //$NON-NLS-1$
	/**
	*	The indication of the capacity that remains in the UPS batteries, generator, and so on. A value of "Normal" (2) indicates that the remaining estimated minutes of run-time are greater than the 'low power' state defined for the UPS (typically two minutes). A value of "Low" (3) indicates that the remaining estimated minutes of run-time are less than or equal to the defined 'low power' state of the device. A value of "Depleted" (4) indicates that the UPS will be unable to sustain the present load when and if the utility power is lost (including the possibility that the utility power is currently absent).
	*/
	public final static String CIM_PROPERTY_REMAININGCAPACITYSTATUS = "RemainingCapacityStatus"; //$NON-NLS-1$
	/**
	*	TimeOnBackup indicates the elapsed time in seconds since the UPS last switched to battery power, generator, or so on or the time since the UPS was last restarted, whichever is less. 0 is returned if the UPS is 'on line'.
	*/
	...
	static {
		...
		for (int i = 0; i < CIM_PowerSupply.Java_Package_List.size(); i++) {
			if (((String)CIM_PowerSupply.Java_Package_List.elementAt(i)).equals("org.sblim.wbemsmt.schema.cim_2_14")){
				continue;
			}
