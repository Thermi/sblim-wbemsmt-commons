/** 
 * CIM_SNMPTrapIndicationHelper.java
 *
...
public final class CIM_SNMPTrapIndicationHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_SNMPTrapIndication(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_SNMPTrapIndication.Java_Package_List.size(); i++) {
						if (!((String)(CIM_SNMPTrapIndication.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_SNMPTrapIndication.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_SNMPTrapIndication.Java_Package_List.setElementAt((String)(CIM_SNMPTrapIndication.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_SNMPTrapIndication.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_SNMPTrapIndication.");
						resultArrayList.add(new CIM_SNMPTrapIndication(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_SNMPTrapIndication.");
						resultArrayList.add(new CIM_SNMPTrapIndication(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_SNMPTrapIndication.");
						resultArrayList.add(new CIM_SNMPTrapIndication(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_SNMPTrapIndication getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_SNMPTrapIndicationHelper.getInstance(cimClient, new CIM_SNMPTrapIndication(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SNMPTrapIndication getInstance(CIMClient cimClient, CIM_SNMPTrapIndication dataInstance) {
		...
		return CIM_SNMPTrapIndicationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SNMPTrapIndication getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_SNMPTrapIndication dataInstance = null;
		try {
			dataInstance = new CIM_SNMPTrapIndication(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_SNMPTrapIndication createInstance(CIMClient cimClient, CIM_SNMPTrapIndication dataInstance, boolean notifyDifferencies){ 
		...
		CIM_SNMPTrapIndication newDataInstance = null;
		try {
			newDataInstance = CIM_SNMPTrapIndicationHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_SNMPTrapIndication modifyInstance(CIMClient cimClient, CIM_SNMPTrapIndication dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_SNMPTrapIndicationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_SNMPTrapIndication dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_SNMPTrapIndication dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_SNMPTrapIndication.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_SNMPTrapIndication.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_SNMPTrapIndication.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_SNMPTrapIndication.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_SNMPTrapIndication.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AgentAddress(String AgentAddress) {
		return true;
	}

	public static boolean isValid_Enterprise(String Enterprise) {
		return true;
	}

	public static boolean isValid_GenericTrap(UnsignedInt16 GenericTrap) {
		return true;
	}

	public static boolean isValid_SpecificTrap(UnsignedInt32 SpecificTrap) {
		return true;
	}
	...
	public static boolean isValid_VarBindNames(String[] VarBindNames) {
		return true;
	}

	public static boolean isValid_VarBindSyntaxes(UnsignedInt16[] VarBindSyntaxes) {
		return true;
	}

	public static boolean isValid_VarBindValues(String[] VarBindValues) {
		return true;
	}
