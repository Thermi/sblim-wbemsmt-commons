/** 
 * CIM_TransformOfPreconfiguredActionHelper.java
 *
...
public final class CIM_TransformOfPreconfiguredActionHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_TransformOfPreconfiguredAction(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_TransformOfPreconfiguredAction.Java_Package_List.size(); i++) {
						if (!((String)(CIM_TransformOfPreconfiguredAction.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_TransformOfPreconfiguredAction.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_TransformOfPreconfiguredAction.Java_Package_List.setElementAt((String)(CIM_TransformOfPreconfiguredAction.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_TransformOfPreconfiguredAction.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_TransformOfPreconfiguredAction.");
						resultArrayList.add(new CIM_TransformOfPreconfiguredAction(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_TransformOfPreconfiguredAction.");
						resultArrayList.add(new CIM_TransformOfPreconfiguredAction(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_TransformOfPreconfiguredAction.");
						resultArrayList.add(new CIM_TransformOfPreconfiguredAction(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_TransformOfPreconfiguredAction getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_TransformOfPreconfiguredActionHelper.getInstance(cimClient, new CIM_TransformOfPreconfiguredAction(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_TransformOfPreconfiguredAction getInstance(CIMClient cimClient, CIM_TransformOfPreconfiguredAction dataInstance) {
		...
		return CIM_TransformOfPreconfiguredActionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_TransformOfPreconfiguredAction getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_TransformOfPreconfiguredAction dataInstance = null;
		try {
			dataInstance = new CIM_TransformOfPreconfiguredAction(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_TransformOfPreconfiguredAction createInstance(CIMClient cimClient, CIM_TransformOfPreconfiguredAction dataInstance, boolean notifyDifferencies){ 
		...
		CIM_TransformOfPreconfiguredAction newDataInstance = null;
		try {
			newDataInstance = CIM_TransformOfPreconfiguredActionHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_TransformOfPreconfiguredAction modifyInstance(CIMClient cimClient, CIM_TransformOfPreconfiguredAction dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_TransformOfPreconfiguredActionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_TransformOfPreconfiguredAction dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_TransformOfPreconfiguredAction dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_TransformOfPreconfiguredAction.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_TransformOfPreconfiguredAction.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_TransformOfPreconfiguredAction.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_TransformOfPreconfiguredAction.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_TransformOfPreconfiguredAction.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_InboundDirection(Boolean InboundDirection) {
		return true;
	}

	public static boolean isValid_SPI(UnsignedInt32 SPI) {
		return true;
	}

	public static boolean isValid_CIM_SATransform(CIM_SATransform CIM_SATransform) {
		return true;
	}

	public static boolean isValid_CIM_PreconfiguredSAAction(CIM_PreconfiguredSAAction CIM_PreconfiguredSAAction) {
		return true;
	}
