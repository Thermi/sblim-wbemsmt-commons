...
public final class CIM_ModemHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_Modem.");
						resultArrayList.add(new CIM_Modem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_Modem.");
						resultArrayList.add(new CIM_Modem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_Modem.");
						resultArrayList.add(new CIM_Modem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_Modem getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ModemHelper.getInstance(cimClient, new CIM_Modem(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Modem getInstance(CIMClient cimClient, CIM_Modem dataInstance) {
		...
		return CIM_ModemHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Modem getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_Modem dataInstance = null;
		try {
			dataInstance = new CIM_Modem(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Modem.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Modem.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Modem.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Modem.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
