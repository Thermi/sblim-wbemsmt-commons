 ...
 *
 *
 * Description:  Each OSPF router has an association to one or more OSPF areas. Into these
 * areas, a router advertises summaries filtered by specific address ranges.
 * This would natively be modeled as a ternary association (router/OSPFService
 * to OSPFArea to RangeOfIPAddresses), which is problematic in some
 * implementations. To get around this issue, the OSPFAreaConfiguration class is
 * defined to act as a focal point to bring together the router (indicated as a
 * ComputerSystem with an instance of OSPFService, associated to the
 * AreaConfiguration using OSPFServiceConfiguration), the area (indicated as an
 * OSPFArea, associated to the AreaConfiguration using AreaOfConfiguration) and
 * the address ranges for advertising (indicated as instances of
 * RangeOfIPAddresses, associated to the AreaConfiguration using
 * RangesOfConfiguration). There would be an instance of OSPFAreaConfiguration
 * for each connected area of a router/OSPFService. When network administrators
 * want to control the advertisements of OSPF routers by filters, they first
 * define the relevant ranges. In order for a router to handle a range, an
 * instance of RangeOfIPAddresses MUST be associated to the relevant
 * OSPFAreaConfiguration (using the relationship, RangesOf Configuration). The
 * association between the range and area configuration contains a property
 * defining the handling (EnableAdvertise) to allow or disallow advertisements
 * in the range. Notes: - Because RangeOfIPAddresses is scoped by a System (via
 * the HostedCollection association), an instance of RangeOfIPAddresses would be
 * associated to an OSPFArea, satisfying this semantic. - This class is
 * inherited from LogicalElement, because a suitable subclass 'lower' in the
 * inheritance hiearchy does not exist.
 * 
 */
...
public class CIM_OSPFAreaConfiguration extends CIM_LogicalElement  {
	...
	public final static String CIM_CLASS_NAME = "CIM_OSPFAreaConfiguration"; //$NON-NLS-1$
	...
	public final static String CIM_ASSOCIATOR_CLASS_NAME_CIM_OSPFSERVICECONFIGURATION = "CIM_OSPFServiceConfiguration"; //$NON-NLS-1$
	public final static String CIM_ASSOCIATOR_CLASS_NAME_CIM_RANGESOFCONFIGURATION = "CIM_RangesOfConfiguration"; //$NON-NLS-1$
