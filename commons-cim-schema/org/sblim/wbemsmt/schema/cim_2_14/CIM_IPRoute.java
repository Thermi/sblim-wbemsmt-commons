 ...
 *
 *
 * Description:  An IPRoute relates a destination address to the address or interface through
 * which the remote address may be reached. The destination address may be a
 * specific IP endpoint or a subnet, dependent on the mask. An instance of this
 * class represents either static or dynamic routing. Static routes are
 * distinguished by setting the IsStatic boolean property to TRUE. Since many
 * routes between endpoints can be defined (using different route calculation
 * algorithms), the CIM_IPRoute class is defined as Abstract. This forces
 * subclassing (for example, see CIM_BGPIPRoute) and allows the instances of its
 * subclasses to be distinguished based on their CreationClassName key property.
 * IPRoute is deprecated in lieu of the more general, concrete NextHopIPRoute
 * class. NextHopIPRoute allows the definition of BOTH a next hop address and an
 * interface for transmission of the traffic. Also, it does not mandate the
 * instantiation of a ForwardingService class.
 * 
 */
...
/**
 *  An IPRoute relates a destination address to the address or interface through
 * which the remote address may be reached. The destination address may be a
 * specific IP endpoint or a subnet, dependent on the mask. An instance of this
 * class represents either static or dynamic routing. Static routes are
 * distinguished by setting the IsStatic boolean property to TRUE. Since many
...
public class CIM_IPRoute extends CIM_NextHopRouting  {
	...
	public final static String CIM_CLASS_NAME = "CIM_IPRoute"; //$NON-NLS-1$
	public final static String CIM_CLASS_DISPLAYNAME = CIM_CLASS_NAME;

	private boolean validCimInstance = false;
	
	public final static String CIM_CLASS_VERSION = "2.7.0";
	public final static String CIM_ASSOCIATOR_CLASS_NAME_CIM_CALCULATEDROUTES = "CIM_CalculatedRoutes"; //$NON-NLS-1$
	...
	*/
	...
	static {
		...
		for (int i = 0; i < CIM_NextHopRouting.Java_Package_List.size(); i++) {
			if (((String)CIM_NextHopRouting.Java_Package_List.elementAt(i)).equals("org.sblim.wbemsmt.schema.cim_2_14")){
				continue;
			}
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i = 0; clazz == null && i < CIM_IPRoute.Java_Package_List.size(); i++) {
						if (!((String)(CIM_IPRoute.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_IPRoute.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_IPRoute.Java_Package_List.setElementAt((String)(CIM_IPRoute.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
