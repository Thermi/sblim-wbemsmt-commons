/** 
 * CIM_TemperatureSensorHelper.java
 *
...
public final class CIM_TemperatureSensorHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_TemperatureSensor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_TemperatureSensor.");
						resultArrayList.add(new CIM_TemperatureSensor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_TemperatureSensor.");
						resultArrayList.add(new CIM_TemperatureSensor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_TemperatureSensor.");
						resultArrayList.add(new CIM_TemperatureSensor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_TemperatureSensor getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_TemperatureSensorHelper.getInstance(cimClient, new CIM_TemperatureSensor(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_TemperatureSensor getInstance(CIMClient cimClient, CIM_TemperatureSensor dataInstance) {
		...
		return CIM_TemperatureSensorHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_TemperatureSensor getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_TemperatureSensor dataInstance = null;
		try {
			dataInstance = new CIM_TemperatureSensor(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_TemperatureSensor createInstance(CIMClient cimClient, CIM_TemperatureSensor dataInstance, boolean notifyDifferencies){ 
		...
		CIM_TemperatureSensor newDataInstance = null;
		try {
			newDataInstance = CIM_TemperatureSensorHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_TemperatureSensor modifyInstance(CIMClient cimClient, CIM_TemperatureSensor dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_TemperatureSensorHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_TemperatureSensor dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_TemperatureSensor dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_TemperatureSensor.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_TemperatureSensor.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_TemperatureSensor.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_TemperatureSensor.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_TemperatureSensor.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_SensorType(UnsignedInt16 SensorType) {
		return true;
	}
