 ...
 *
 *
 * Description:  SATransform is the base class for the various types of transforms that are in
 * effect for an existing IPsecSAEndpoint, or aggregated into phase 2 proposals
 * used during the IPsec negotiation process. This class is defined as a kind of
 * ScopedSettingData, since it describes the set of transform properties that
 * MUST be applied together, if negotiated. This subclassing is different than
 * that defined in IETF's IPSP Policy draft - where it is subclassed from
 * Policy. The definition as SettingData is more consistent with the application
 * of the properties as a set, to the negotiated Security Association. Note that
 * an instance of this class is 'scoped' or contained by a System (either a
 * ComputerSystem or AdminDomain), rather than by a SecurityAssociationEndpoint.
 * This is indicated by instantiating the ScopedSetting association, and is
 * necessary since the transform data is used in IPsec negotiation policies
 * (also scoped by Systems) before ever being associated with a
 * SecurityAssociationEndpoint. The latter semantics (i.e., that 'this'
 * transform is negotiated for 'this' Security Association) is conveyed via a
...
/**
 *  SATransform is the base class for the various types of transforms that are in
 * effect for an existing IPsecSAEndpoint, or aggregated into phase 2 proposals
 * used during the IPsec negotiation process. This class is defined as a kind of
 * ScopedSettingData, since it describes the set of transform properties that
 * MUST be applied together, if negotiated. This subclassing is different than
...
public class CIM_SATransform extends CIM_ScopedSettingData  {
	...
	public final static String CIM_CLASS_NAME = "CIM_SATransform"; //$NON-NLS-1$
	...
	public final static String CIM_ASSOCIATOR_CLASS_NAME_CIM_TRANSFORMOFSECURITYASSOCIATION = "CIM_TransformOfSecurityAssociation"; //$NON-NLS-1$
