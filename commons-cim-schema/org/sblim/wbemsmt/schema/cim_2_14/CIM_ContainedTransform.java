 ...
 *
 *
 * Description:  ContainedTransform associates a proposal with a list of transforms. If multiple
 * transforms of a given type are included in a proposal, these transforms are
 * interpreted as alternatives -- i.e., logically ORed with each other. The
 * order of preference is dictated by the SequenceNumber property. Sets of
 * transforms of different types are logically ANDed. For example, a proposal
 * based on two AH transforms and three ESP transforms means one of the AH AND
 * one of the ESP transforms MUST be chosen. Note that at least 1 transform MUST
...
/**
 *  ContainedTransform associates a proposal with a list of transforms. If multiple
 * transforms of a given type are included in a proposal, these transforms are
 * interpreted as alternatives -- i.e., logically ORed with each other. The
 * order of preference is dictated by the SequenceNumber property. Sets of
 * transforms of different types are logically ANDed. For example, a proposal
...
public class CIM_ContainedTransform extends CIM_Component  {
	...
	public final static String CIM_CLASS_NAME = "CIM_ContainedTransform"; //$NON-NLS-1$
