 ...
 *
 *
 * Description:  This relationship associates a PublicKeyCertificate with the Principal who has
 * the PrivateKey used with the PublicKey. The PrivateKey is not modeled, since
 * it is not a data element that ever SHOULD be accessible via management
 * applications, other than key recovery services, which are outside our scope.
 * Since the UsersAccess class and this association's superclass are deprecated,
 * this association is also deprecated. There is no need to have a special
 * subclass for public-private credentials. This is especially true since the
 * properties of the association describe aspects of the certificate and its
...
/**
 *  This relationship associates a PublicKeyCertificate with the Principal who has
 * the PrivateKey used with the PublicKey. The PrivateKey is not modeled, since
 * it is not a data element that ever SHOULD be accessible via management
 * applications, other than key recovery services, which are outside our scope.
 * Since the UsersAccess class and this association's superclass are deprecated,
...
public class CIM_PublicPrivateKeyPair extends CIM_UsersCredential  {
	...
	public final static String CIM_CLASS_NAME = "CIM_PublicPrivateKeyPair"; //$NON-NLS-1$
	...
	*/
	...
	static {
		...
		for (int i = 0; i < CIM_UsersCredential.Java_Package_List.size(); i++) {
			if (((String)CIM_UsersCredential.Java_Package_List.elementAt(i)).equals("org.sblim.wbemsmt.schema.cim_2_14")){
				continue;
			}
