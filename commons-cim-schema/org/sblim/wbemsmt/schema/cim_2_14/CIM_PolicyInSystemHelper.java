...
public final class CIM_PolicyInSystemHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_PolicyInSystem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_PolicyInSystem.Java_Package_List.size(); i++) {
						if (!((String)(CIM_PolicyInSystem.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_PolicyInSystem.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_PolicyInSystem.Java_Package_List.setElementAt((String)(CIM_PolicyInSystem.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_PolicyInSystem.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_PolicyInSystem.");
						resultArrayList.add(new CIM_PolicyInSystem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_PolicyInSystem.");
						resultArrayList.add(new CIM_PolicyInSystem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_PolicyInSystem.");
						resultArrayList.add(new CIM_PolicyInSystem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_PolicyInSystem getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_PolicyInSystemHelper.getInstance(cimClient, new CIM_PolicyInSystem(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PolicyInSystem getInstance(CIMClient cimClient, CIM_PolicyInSystem dataInstance) {
		...
		return CIM_PolicyInSystemHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PolicyInSystem getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_PolicyInSystem dataInstance = null;
		try {
			dataInstance = new CIM_PolicyInSystem(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_PolicyInSystem.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_PolicyInSystem.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_PolicyInSystem.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_PolicyInSystem.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
