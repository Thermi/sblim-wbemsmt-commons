...
public final class CIM_DeviceSoftwareHelper {

	public final static String CIM_CLASS_NAME = "CIM_DeviceSoftware";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_DeviceSoftware(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_DeviceSoftware.Java_Package_List.size(); i++) {
						if (!((String)(CIM_DeviceSoftware.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_DeviceSoftware.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_DeviceSoftware.Java_Package_List.setElementAt((String)(CIM_DeviceSoftware.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_DeviceSoftware.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_DeviceSoftware.");
						resultArrayList.add(new CIM_DeviceSoftware(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_DeviceSoftware.");
						resultArrayList.add(new CIM_DeviceSoftware(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_DeviceSoftware.");
						resultArrayList.add(new CIM_DeviceSoftware(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_DeviceSoftware getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_DeviceSoftwareHelper.getInstance(cimClient, new CIM_DeviceSoftware(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DeviceSoftware getInstance(CIMClient cimClient, CIM_DeviceSoftware dataInstance) {
		...
		return CIM_DeviceSoftwareHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DeviceSoftware getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_DeviceSoftware dataInstance = null;
		try {
			dataInstance = new CIM_DeviceSoftware(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_DeviceSoftware createInstance(CIMClient cimClient, CIM_DeviceSoftware dataInstance, boolean notifyDifferencies){ 
		...
		CIM_DeviceSoftware newDataInstance = null;
		try {
			newDataInstance = CIM_DeviceSoftwareHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_DeviceSoftware modifyInstance(CIMClient cimClient, CIM_DeviceSoftware dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_DeviceSoftwareHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_DeviceSoftware dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_DeviceSoftware dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_DeviceSoftware.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_DeviceSoftware.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_DeviceSoftware.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_DeviceSoftware.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_DeviceSoftware.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_LoadedOnDevice(Boolean LoadedOnDevice) {
		return true;
	}

	public static boolean isValid_Purpose(UnsignedInt16 Purpose) {
		return true;
	}

	public static boolean isValid_PurposeDescription(String PurposeDescription) {
		return true;
	}

	public static boolean isValid_UpgradeableOnDevice(Boolean UpgradeableOnDevice) {
		return true;
	}
