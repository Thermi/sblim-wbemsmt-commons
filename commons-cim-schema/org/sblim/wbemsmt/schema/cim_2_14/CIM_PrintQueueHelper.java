...
public final class CIM_PrintQueueHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_PrintQueue.Java_Package_List.size(); i++) {
						if (!((String)(CIM_PrintQueue.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_PrintQueue.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_PrintQueue.Java_Package_List.setElementAt((String)(CIM_PrintQueue.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_PrintQueue.");
						resultArrayList.add(new CIM_PrintQueue(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_PrintQueue.");
						resultArrayList.add(new CIM_PrintQueue(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_PrintQueue.");
						resultArrayList.add(new CIM_PrintQueue(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_PrintQueue getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_PrintQueueHelper.getInstance(cimClient, new CIM_PrintQueue(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PrintQueue getInstance(CIMClient cimClient, CIM_PrintQueue dataInstance) {
		...
		return CIM_PrintQueueHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PrintQueue getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_PrintQueue dataInstance = null;
		try {
			dataInstance = new CIM_PrintQueue(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_PrintQueue createInstance(CIMClient cimClient, CIM_PrintQueue dataInstance, boolean notifyDifferencies){ 
		...
		CIM_PrintQueue newDataInstance = null;
		try {
			newDataInstance = CIM_PrintQueueHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_PrintQueue modifyInstance(CIMClient cimClient, CIM_PrintQueue dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_PrintQueueHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_PrintQueue dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_PrintQueue dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_PrintQueue.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_PrintQueue.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_PrintQueue.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_PrintQueue.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_PrintQueue.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_DefaultJobPriority(UnsignedInt32 DefaultJobPriority) {
		return true;
	}

	public static boolean isValid_JobPriorityHigh(UnsignedInt32 JobPriorityHigh) {
		return true;
	}

	public static boolean isValid_JobPriorityLow(UnsignedInt32 JobPriorityLow) {
		return true;
	}

	public static boolean isValid_MaxJobSize(UnsignedInt32 MaxJobSize) {
		return true;
	}

	public static boolean isValid_NumberOnQueue(UnsignedInt32 NumberOnQueue) {
		return true;
	}

	public static boolean isValid_QueueAccepting(Boolean QueueAccepting) {
		return true;
	}

	public static boolean isValid_QueueEnabled(Boolean QueueEnabled) {
		return true;
	}

	public static boolean isValid_QueueStatus(UnsignedInt16 QueueStatus) {
		return true;
	}

	public static boolean isValid_QueueStatusInfo(String QueueStatusInfo) {
		return true;
	}
