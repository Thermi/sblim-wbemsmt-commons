/** 
 * CIM_VersionCompatibilityCheckHelper.java
 *
...
public final class CIM_VersionCompatibilityCheckHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_VersionCompatibilityCheck(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_VersionCompatibilityCheck.Java_Package_List.size(); i++) {
						if (!((String)(CIM_VersionCompatibilityCheck.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_VersionCompatibilityCheck.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_VersionCompatibilityCheck.Java_Package_List.setElementAt((String)(CIM_VersionCompatibilityCheck.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_VersionCompatibilityCheck.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_VersionCompatibilityCheck.");
						resultArrayList.add(new CIM_VersionCompatibilityCheck(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_VersionCompatibilityCheck.");
						resultArrayList.add(new CIM_VersionCompatibilityCheck(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_VersionCompatibilityCheck.");
						resultArrayList.add(new CIM_VersionCompatibilityCheck(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_VersionCompatibilityCheck getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_VersionCompatibilityCheckHelper.getInstance(cimClient, new CIM_VersionCompatibilityCheck(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_VersionCompatibilityCheck getInstance(CIMClient cimClient, CIM_VersionCompatibilityCheck dataInstance) {
		...
		return CIM_VersionCompatibilityCheckHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_VersionCompatibilityCheck getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_VersionCompatibilityCheck dataInstance = null;
		try {
			dataInstance = new CIM_VersionCompatibilityCheck(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_VersionCompatibilityCheck createInstance(CIMClient cimClient, CIM_VersionCompatibilityCheck dataInstance, boolean notifyDifferencies){ 
		...
		CIM_VersionCompatibilityCheck newDataInstance = null;
		try {
			newDataInstance = CIM_VersionCompatibilityCheckHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_VersionCompatibilityCheck modifyInstance(CIMClient cimClient, CIM_VersionCompatibilityCheck dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_VersionCompatibilityCheckHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_VersionCompatibilityCheck dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_VersionCompatibilityCheck dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_VersionCompatibilityCheck.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_VersionCompatibilityCheck.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_VersionCompatibilityCheck.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_VersionCompatibilityCheck.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_VersionCompatibilityCheck.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AllowDownVersion(Boolean AllowDownVersion) {
		return true;
	}

	public static boolean isValid_AllowMultipleVersions(Boolean AllowMultipleVersions) {
		return true;
	}

	public static boolean isValid_Reinstall(Boolean Reinstall) {
		return true;
	}
