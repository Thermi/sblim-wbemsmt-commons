/** 
 * CIM_StorageSettingWithHintsHelper.java
 *
...
public final class CIM_StorageSettingWithHintsHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_StorageSettingWithHints(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_StorageSettingWithHints.Java_Package_List.size(); i++) {
						if (!((String)(CIM_StorageSettingWithHints.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_StorageSettingWithHints.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_StorageSettingWithHints.Java_Package_List.setElementAt((String)(CIM_StorageSettingWithHints.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_StorageSettingWithHints.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_StorageSettingWithHints.");
						resultArrayList.add(new CIM_StorageSettingWithHints(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_StorageSettingWithHints.");
						resultArrayList.add(new CIM_StorageSettingWithHints(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_StorageSettingWithHints.");
						resultArrayList.add(new CIM_StorageSettingWithHints(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_StorageSettingWithHints getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_StorageSettingWithHintsHelper.getInstance(cimClient, new CIM_StorageSettingWithHints(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_StorageSettingWithHints getInstance(CIMClient cimClient, CIM_StorageSettingWithHints dataInstance) {
		...
		return CIM_StorageSettingWithHintsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_StorageSettingWithHints getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_StorageSettingWithHints dataInstance = null;
		try {
			dataInstance = new CIM_StorageSettingWithHints(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_StorageSettingWithHints createInstance(CIMClient cimClient, CIM_StorageSettingWithHints dataInstance, boolean notifyDifferencies){ 
		...
		CIM_StorageSettingWithHints newDataInstance = null;
		try {
			newDataInstance = CIM_StorageSettingWithHintsHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_StorageSettingWithHints modifyInstance(CIMClient cimClient, CIM_StorageSettingWithHints dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_StorageSettingWithHintsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_StorageSettingWithHints dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_StorageSettingWithHints dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_StorageSettingWithHints.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_StorageSettingWithHints.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_StorageSettingWithHints.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_StorageSettingWithHints.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_StorageSettingWithHints.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AccessBandwidthWeight(UnsignedInt16 AccessBandwidthWeight) {
		return true;
	}

	public static boolean isValid_AccessDirectionHint(UnsignedInt16 AccessDirectionHint) {
		return true;
	}

	public static boolean isValid_AccessLatencyHint(UnsignedInt16 AccessLatencyHint) {
		return true;
	}

	public static boolean isValid_AccessRandomnessHint(UnsignedInt16 AccessRandomnessHint) {
		return true;
	}

	public static boolean isValid_AccessSizeHint(UnsignedInt16[] AccessSizeHint) {
		return true;
	}

	public static boolean isValid_DataAvailabilityHint(UnsignedInt16 DataAvailabilityHint) {
		return true;
	}

	public static boolean isValid_StorageCostHint(UnsignedInt16 StorageCostHint) {
		return true;
	}

	public static boolean isValid_StorageEfficiencyHint(UnsignedInt16 StorageEfficiencyHint) {
		return true;
	}
