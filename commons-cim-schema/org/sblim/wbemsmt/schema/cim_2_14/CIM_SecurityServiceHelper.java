/** 
 * CIM_SecurityServiceHelper.java
 *
...
public final class CIM_SecurityServiceHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_SecurityService.");
						resultArrayList.add(new CIM_SecurityService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_SecurityService.");
						resultArrayList.add(new CIM_SecurityService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_SecurityService.");
						resultArrayList.add(new CIM_SecurityService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_SecurityService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_SecurityServiceHelper.getInstance(cimClient, new CIM_SecurityService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SecurityService getInstance(CIMClient cimClient, CIM_SecurityService dataInstance) {
		...
		return CIM_SecurityServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SecurityService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_SecurityService dataInstance = null;
		try {
			dataInstance = new CIM_SecurityService(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_SecurityService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_SecurityService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_SecurityService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_SecurityService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
