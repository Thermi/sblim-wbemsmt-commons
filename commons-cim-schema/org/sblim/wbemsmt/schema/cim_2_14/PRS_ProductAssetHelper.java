...
public final class PRS_ProductAssetHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new PRS_ProductAsset(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<PRS_ProductAsset.Java_Package_List.size(); i++) {
						if (!((String)(PRS_ProductAsset.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(PRS_ProductAsset.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							PRS_ProductAsset.Java_Package_List.setElementAt((String)(PRS_ProductAsset.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (PRS_ProductAsset.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_ProductAsset.");
						resultArrayList.add(new PRS_ProductAsset(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_ProductAsset.");
						resultArrayList.add(new PRS_ProductAsset(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_ProductAsset.");
						resultArrayList.add(new PRS_ProductAsset(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_ProductAsset getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_ProductAssetHelper.getInstance(cimClient, new PRS_ProductAsset(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_ProductAsset getInstance(CIMClient cimClient, PRS_ProductAsset dataInstance) {
		...
		return PRS_ProductAssetHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_ProductAsset getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_ProductAsset dataInstance = null;
		try {
			dataInstance = new PRS_ProductAsset(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_ProductAsset createInstance(CIMClient cimClient, PRS_ProductAsset dataInstance, boolean notifyDifferencies){ 
		...
		PRS_ProductAsset newDataInstance = null;
		try {
			newDataInstance = PRS_ProductAssetHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_ProductAsset modifyInstance(CIMClient cimClient, PRS_ProductAsset dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_ProductAssetHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_ProductAsset dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_ProductAsset dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_ProductAsset.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_ProductAsset.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_ProductAsset.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_ProductAsset.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_ProductAsset.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_PRS_Product(PRS_Product PRS_Product) {
		return true;
	}
