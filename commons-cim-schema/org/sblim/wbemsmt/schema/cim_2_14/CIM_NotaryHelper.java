...
public final class CIM_NotaryHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_Notary.");
						resultArrayList.add(new CIM_Notary(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_Notary.");
						resultArrayList.add(new CIM_Notary(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_Notary.");
						resultArrayList.add(new CIM_Notary(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_Notary getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_NotaryHelper.getInstance(cimClient, new CIM_Notary(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Notary getInstance(CIMClient cimClient, CIM_Notary dataInstance) {
		...
		return CIM_NotaryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Notary getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_Notary dataInstance = null;
		try {
			dataInstance = new CIM_Notary(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Notary createInstance(CIMClient cimClient, CIM_Notary dataInstance, boolean notifyDifferencies){ 
		...
		CIM_Notary newDataInstance = null;
		try {
			newDataInstance = CIM_NotaryHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Notary modifyInstance(CIMClient cimClient, CIM_Notary dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_NotaryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_Notary dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_Notary dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_Notary.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Notary.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Notary.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Notary.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Notary.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CharterExpired(Calendar CharterExpired) {
		return true;
	}

	public static boolean isValid_CharterIssued(Calendar CharterIssued) {
		return true;
	}

	public static boolean isValid_Comparitors(UnsignedInt16 Comparitors) {
		return true;
	}

	public static boolean isValid_SealProtocol(String SealProtocol) {
		return true;
	}
