...
public final class CIM_LogicalPortHelper {
	...
	*/	
	public final static CIM_LogicalPort getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_LogicalPortHelper.getInstance(cimClient, new CIM_LogicalPort(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LogicalPort getInstance(CIMClient cimClient, CIM_LogicalPort dataInstance) {
		...
		return CIM_LogicalPortHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LogicalPort getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_LogicalPort(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_LogicalPort createInstance(CIMClient cimClient, CIM_LogicalPort dataInstance, boolean notifyDifferencies){ 
		...
		CIM_LogicalPort newDataInstance = null;
		try {
			newDataInstance = CIM_LogicalPortHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_LogicalPort modifyInstance(CIMClient cimClient, CIM_LogicalPort dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_LogicalPortHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_LogicalPort dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_LogicalPort dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_LogicalPort.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_LogicalPort.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_LogicalPort.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_LogicalPort.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_LogicalPort.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_MaxSpeed(UnsignedInt64 MaxSpeed) {
		return true;
	}

	public static boolean isValid_OtherPortType(String OtherPortType) {
		return true;
	}
	...
	public static boolean isValid_UsageRestriction(UnsignedInt16 UsageRestriction) {
		return true;
	}
