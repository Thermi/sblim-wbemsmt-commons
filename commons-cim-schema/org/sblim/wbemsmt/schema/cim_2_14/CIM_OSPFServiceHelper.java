...
public final class CIM_OSPFServiceHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_OSPFService.Java_Package_List.size(); i++) {
						if (!((String)(CIM_OSPFService.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_OSPFService.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_OSPFService.Java_Package_List.setElementAt((String)(CIM_OSPFService.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_OSPFService.");
						resultArrayList.add(new CIM_OSPFService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_OSPFService.");
						resultArrayList.add(new CIM_OSPFService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_OSPFService.");
						resultArrayList.add(new CIM_OSPFService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_OSPFService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_OSPFServiceHelper.getInstance(cimClient, new CIM_OSPFService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_OSPFService getInstance(CIMClient cimClient, CIM_OSPFService dataInstance) {
		...
		return CIM_OSPFServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_OSPFService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_OSPFService dataInstance = null;
		try {
			dataInstance = new CIM_OSPFService(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_OSPFService createInstance(CIMClient cimClient, CIM_OSPFService dataInstance, boolean notifyDifferencies){ 
		...
		CIM_OSPFService newDataInstance = null;
		try {
			newDataInstance = CIM_OSPFServiceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_OSPFService modifyInstance(CIMClient cimClient, CIM_OSPFService dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_OSPFServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_OSPFService dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_OSPFService dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_OSPFService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_OSPFService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_OSPFService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_OSPFService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_OSPFService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ExitOverflowInterval(UnsignedInt32 ExitOverflowInterval) {
		return true;
	}

	public static boolean isValid_IsInterAreaMulticastForwarder(Boolean IsInterAreaMulticastForwarder) {
		return true;
	}

	public static boolean isValid_IsInterAsMulticastForwarder(Boolean IsInterAsMulticastForwarder) {
		return true;
	}

	public static boolean isValid_RFC1583Compatibility(Boolean RFC1583Compatibility) {
		return true;
	}

	public static boolean isValid_RunningMOSPF(Boolean RunningMOSPF) {
		return true;
	}
