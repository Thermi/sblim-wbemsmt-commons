/** 
 * CIM_DatabaseSegmentSettingDataHelper.java
 *
...
public final class CIM_DatabaseSegmentSettingDataHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_DatabaseSegmentSettingData(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_DatabaseSegmentSettingData.Java_Package_List.size(); i++) {
						if (!((String)(CIM_DatabaseSegmentSettingData.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_DatabaseSegmentSettingData.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_DatabaseSegmentSettingData.Java_Package_List.setElementAt((String)(CIM_DatabaseSegmentSettingData.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_DatabaseSegmentSettingData.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_DatabaseSegmentSettingData.");
						resultArrayList.add(new CIM_DatabaseSegmentSettingData(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_DatabaseSegmentSettingData.");
						resultArrayList.add(new CIM_DatabaseSegmentSettingData(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_DatabaseSegmentSettingData.");
						resultArrayList.add(new CIM_DatabaseSegmentSettingData(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_DatabaseSegmentSettingData getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_DatabaseSegmentSettingDataHelper.getInstance(cimClient, new CIM_DatabaseSegmentSettingData(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DatabaseSegmentSettingData getInstance(CIMClient cimClient, CIM_DatabaseSegmentSettingData dataInstance) {
		...
		return CIM_DatabaseSegmentSettingDataHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DatabaseSegmentSettingData getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_DatabaseSegmentSettingData dataInstance = null;
		try {
			dataInstance = new CIM_DatabaseSegmentSettingData(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_DatabaseSegmentSettingData createInstance(CIMClient cimClient, CIM_DatabaseSegmentSettingData dataInstance, boolean notifyDifferencies){ 
		...
		CIM_DatabaseSegmentSettingData newDataInstance = null;
		try {
			newDataInstance = CIM_DatabaseSegmentSettingDataHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_DatabaseSegmentSettingData modifyInstance(CIMClient cimClient, CIM_DatabaseSegmentSettingData dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_DatabaseSegmentSettingDataHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_DatabaseSegmentSettingData dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_DatabaseSegmentSettingData dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_DatabaseSegmentSettingData.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_DatabaseSegmentSettingData.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_DatabaseSegmentSettingData.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_DatabaseSegmentSettingData.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_DatabaseSegmentSettingData.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_InitialExtentSize(UnsignedInt64 InitialExtentSize) {
		return true;
	}

	public static boolean isValid_MaximumExtents(UnsignedInt64 MaximumExtents) {
		return true;
	}

	public static boolean isValid_MinimumExtents(UnsignedInt64 MinimumExtents) {
		return true;
	}

	public static boolean isValid_NextExtentSize(UnsignedInt64 NextExtentSize) {
		return true;
	}

	public static boolean isValid_NumberOfFreeLists(UnsignedInt32 NumberOfFreeLists) {
		return true;
	}

	public static boolean isValid_PercentIncrease(UnsignedInt16 PercentIncrease) {
		return true;
	}
