 ...
 *
 *
 * Description:  Capabilities and management of a Printer Queue. A PrintQueue holds PrintJobs
 * that are placed on the Queue by a Print Service. Jobs can move from Queue to
 * Queue. The ABNF for CIM references to PWG Standards and MIBs is in the
 * directory 'ftp://ftp.pwg.org/pub/pwg/general/process' in the file
...
/**
 *  Capabilities and management of a Printer Queue. A PrintQueue holds PrintJobs
 * that are placed on the Queue by a Print Service. Jobs can move from Queue to
 * Queue. The ABNF for CIM references to PWG Standards and MIBs is in the
 * directory 'ftp://ftp.pwg.org/pub/pwg/general/process' in the file
 * 'pwg-cim-references-format-20060309.txt'. An example for a MappingStrings
...
public class CIM_PrintQueue extends CIM_JobDestination  {
	...
	public final static String CIM_CLASS_NAME = "CIM_PrintQueue"; //$NON-NLS-1$
	...
	*/
	...
	static {
		...
		for (int i = 0; i < CIM_JobDestination.Java_Package_List.size(); i++) {
			if (((String)CIM_JobDestination.Java_Package_List.elementAt(i)).equals("org.sblim.wbemsmt.schema.cim_2_14")){
				continue;
			}
