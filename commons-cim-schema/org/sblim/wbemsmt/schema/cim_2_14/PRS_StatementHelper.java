...
public final class PRS_StatementHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<PRS_Statement.Java_Package_List.size(); i++) {
						if (!((String)(PRS_Statement.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(PRS_Statement.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							PRS_Statement.Java_Package_List.setElementAt((String)(PRS_Statement.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Statement.");
						resultArrayList.add(new PRS_Statement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Statement.");
						resultArrayList.add(new PRS_Statement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Statement.");
						resultArrayList.add(new PRS_Statement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Statement getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_StatementHelper.getInstance(cimClient, new PRS_Statement(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Statement getInstance(CIMClient cimClient, PRS_Statement dataInstance) {
		...
		return PRS_StatementHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Statement getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Statement dataInstance = null;
		try {
			dataInstance = new PRS_Statement(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Statement createInstance(CIMClient cimClient, PRS_Statement dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Statement newDataInstance = null;
		try {
			newDataInstance = PRS_StatementHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Statement modifyInstance(CIMClient cimClient, PRS_Statement dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_StatementHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Statement dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Statement dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Statement.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Statement.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Statement.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Statement.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Statement.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_FeatureValue(String FeatureValue) {
		return true;
	}

	public static boolean isValid_Order(String Order) {
		return true;
	}

	public static boolean isValid_Relevance(UnsignedInt16 Relevance) {
		return true;
	}

	public static boolean isValid_StatementOperator(String StatementOperator) {
		return true;
	}

	public static boolean isValid_StatementRole(UnsignedInt8 StatementRole) {
		return true;
	}

	public static boolean isValid_Text(String Text) {
		return true;
	}
