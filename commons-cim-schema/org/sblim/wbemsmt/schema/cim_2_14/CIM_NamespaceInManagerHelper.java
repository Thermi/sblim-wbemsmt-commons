...
public final class CIM_NamespaceInManagerHelper {

	public final static String CIM_CLASS_NAME = "CIM_NamespaceInManager";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_NamespaceInManager(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_NamespaceInManager.Java_Package_List.size(); i++) {
						if (!((String)(CIM_NamespaceInManager.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_NamespaceInManager.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_NamespaceInManager.Java_Package_List.setElementAt((String)(CIM_NamespaceInManager.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_NamespaceInManager.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static CIM_NamespaceInManager getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_NamespaceInManagerHelper.getInstance(cimClient, new CIM_NamespaceInManager(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_NamespaceInManager getInstance(CIMClient cimClient, CIM_NamespaceInManager dataInstance) {
		...
		return CIM_NamespaceInManagerHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_NamespaceInManager getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_NamespaceInManager(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_NamespaceInManager createInstance(CIMClient cimClient, CIM_NamespaceInManager dataInstance, boolean notifyDifferencies){ 
		...
		CIM_NamespaceInManager newDataInstance = null;
		try {
			newDataInstance = CIM_NamespaceInManagerHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_NamespaceInManager modifyInstance(CIMClient cimClient, CIM_NamespaceInManager dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_NamespaceInManagerHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_NamespaceInManager dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_NamespaceInManager dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_NamespaceInManager.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_NamespaceInManager.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_NamespaceInManager.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_NamespaceInManager.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_NamespaceInManager.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
