...
public final class CIM_ActionHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_Action.Java_Package_List.size(); i++) {
						if (!((String)(CIM_Action.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_Action.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_Action.Java_Package_List.setElementAt((String)(CIM_Action.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_Action.");
						resultArrayList.add(new CIM_Action(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_Action.");
						resultArrayList.add(new CIM_Action(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_Action.");
						resultArrayList.add(new CIM_Action(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_Action getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ActionHelper.getInstance(cimClient, new CIM_Action(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Action getInstance(CIMClient cimClient, CIM_Action dataInstance) {
		...
		return CIM_ActionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Action getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_Action dataInstance = null;
		try {
			dataInstance = new CIM_Action(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Action.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Action.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Action.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Action.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ActionID(String ActionID) {
		return true;
	}
