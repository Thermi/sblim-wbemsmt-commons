...
public final class CIM_PCIControllerHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_PCIController(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_PCIController.Java_Package_List.size(); i++) {
						if (!((String)(CIM_PCIController.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_PCIController.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_PCIController.Java_Package_List.setElementAt((String)(CIM_PCIController.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_PCIController.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_PCIController.");
						resultArrayList.add(new CIM_PCIController(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_PCIController.");
						resultArrayList.add(new CIM_PCIController(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_PCIController.");
						resultArrayList.add(new CIM_PCIController(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_PCIController getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_PCIControllerHelper.getInstance(cimClient, new CIM_PCIController(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PCIController getInstance(CIMClient cimClient, CIM_PCIController dataInstance) {
		...
		return CIM_PCIControllerHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PCIController getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_PCIController dataInstance = null;
		try {
			dataInstance = new CIM_PCIController(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_PCIController createInstance(CIMClient cimClient, CIM_PCIController dataInstance, boolean notifyDifferencies){ 
		...
		CIM_PCIController newDataInstance = null;
		try {
			newDataInstance = CIM_PCIControllerHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_PCIController modifyInstance(CIMClient cimClient, CIM_PCIController dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_PCIControllerHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_PCIController dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_PCIController dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_PCIController.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_PCIController.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_PCIController.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_PCIController.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_PCIController.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CacheLineSize(UnsignedInt8 CacheLineSize) {
		return true;
	}
	...
	public static boolean isValid_ClassCode(UnsignedInt8 ClassCode) {
		return true;
	}

	public static boolean isValid_CommandRegister(UnsignedInt16 CommandRegister) {
		return true;
	}

	public static boolean isValid_DeviceSelectTiming(UnsignedInt16 DeviceSelectTiming) {
		return true;
	}

	public static boolean isValid_ExpansionROMBaseAddress(UnsignedInt32 ExpansionROMBaseAddress) {
		return true;
	}

	public static boolean isValid_InterruptPin(UnsignedInt16 InterruptPin) {
		return true;
	}

	public static boolean isValid_LatencyTimer(UnsignedInt8 LatencyTimer) {
		return true;
	}

	public static boolean isValid_SelfTestEnabled(Boolean SelfTestEnabled) {
		return true;
	}
