...
public final class CIM_SCSIInterfaceHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_SCSIInterface(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_SCSIInterface.Java_Package_List.size(); i++) {
						if (!((String)(CIM_SCSIInterface.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_SCSIInterface.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_SCSIInterface.Java_Package_List.setElementAt((String)(CIM_SCSIInterface.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_SCSIInterface.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_SCSIInterface.");
						resultArrayList.add(new CIM_SCSIInterface(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_SCSIInterface.");
						resultArrayList.add(new CIM_SCSIInterface(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_SCSIInterface.");
						resultArrayList.add(new CIM_SCSIInterface(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_SCSIInterface getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_SCSIInterfaceHelper.getInstance(cimClient, new CIM_SCSIInterface(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SCSIInterface getInstance(CIMClient cimClient, CIM_SCSIInterface dataInstance) {
		...
		return CIM_SCSIInterfaceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SCSIInterface getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_SCSIInterface dataInstance = null;
		try {
			dataInstance = new CIM_SCSIInterface(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_SCSIInterface createInstance(CIMClient cimClient, CIM_SCSIInterface dataInstance, boolean notifyDifferencies){ 
		...
		CIM_SCSIInterface newDataInstance = null;
		try {
			newDataInstance = CIM_SCSIInterfaceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_SCSIInterface modifyInstance(CIMClient cimClient, CIM_SCSIInterface dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_SCSIInterfaceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_SCSIInterface dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_SCSIInterface dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_SCSIInterface.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_SCSIInterface.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_SCSIInterface.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_SCSIInterface.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_SCSIInterface.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_InitiatorId(UnsignedInt32 InitiatorId) {
		return true;
	}

	public static boolean isValid_MaxQueueDepth(UnsignedInt32 MaxQueueDepth) {
		return true;
	}

	public static boolean isValid_QueueDepthLimit(UnsignedInt32 QueueDepthLimit) {
		return true;
	}

	public static boolean isValid_SCSIReservation(UnsignedInt16 SCSIReservation) {
		return true;
	}

	public static boolean isValid_SCSIRetries(UnsignedInt32 SCSIRetries) {
		return true;
	}

	public static boolean isValid_SCSISignal(UnsignedInt16 SCSISignal) {
		return true;
	}

	public static boolean isValid_SCSITimeouts(UnsignedInt32 SCSITimeouts) {
		return true;
	}

	public static boolean isValid_TargetId(UnsignedInt32 TargetId) {
		return true;
	}

	public static boolean isValid_TargetLUN(UnsignedInt64 TargetLUN) {
		return true;
	}

	public static boolean isValid_CIM_SCSIController(CIM_SCSIController CIM_SCSIController) {
		return true;
	}
