...
public final class CIM_ESPTransformHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_ESPTransform(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_ESPTransform.Java_Package_List.size(); i++) {
						if (!((String)(CIM_ESPTransform.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_ESPTransform.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_ESPTransform.Java_Package_List.setElementAt((String)(CIM_ESPTransform.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_ESPTransform.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_ESPTransform.");
						resultArrayList.add(new CIM_ESPTransform(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_ESPTransform.");
						resultArrayList.add(new CIM_ESPTransform(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_ESPTransform.");
						resultArrayList.add(new CIM_ESPTransform(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_ESPTransform getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ESPTransformHelper.getInstance(cimClient, new CIM_ESPTransform(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ESPTransform getInstance(CIMClient cimClient, CIM_ESPTransform dataInstance) {
		...
		return CIM_ESPTransformHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ESPTransform getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_ESPTransform dataInstance = null;
		try {
			dataInstance = new CIM_ESPTransform(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ESPTransform createInstance(CIMClient cimClient, CIM_ESPTransform dataInstance, boolean notifyDifferencies){ 
		...
		CIM_ESPTransform newDataInstance = null;
		try {
			newDataInstance = CIM_ESPTransformHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ESPTransform modifyInstance(CIMClient cimClient, CIM_ESPTransform dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_ESPTransformHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_ESPTransform dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_ESPTransform dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_ESPTransform.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_ESPTransform.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_ESPTransform.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_ESPTransform.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_ESPTransform.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CipherKeyLength(UnsignedInt16 CipherKeyLength) {
		return true;
	}

	public static boolean isValid_CipherKeyRounds(UnsignedInt16 CipherKeyRounds) {
		return true;
	}

	public static boolean isValid_CipherTransformId(UnsignedInt16 CipherTransformId) {
		return true;
	}

	public static boolean isValid_IntegrityTransformId(UnsignedInt16 IntegrityTransformId) {
		return true;
	}

	public static boolean isValid_OtherCipherTransformId(String OtherCipherTransformId) {
		return true;
	}

	public static boolean isValid_OtherIntegrityTransformId(String OtherIntegrityTransformId) {
		return true;
	}

	public static boolean isValid_ReplayPreventionWindowSize(UnsignedInt32 ReplayPreventionWindowSize) {
		return true;
	}

	public static boolean isValid_UseReplayPrevention(Boolean UseReplayPrevention) {
		return true;
	}
