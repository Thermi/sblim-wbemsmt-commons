/** 
 * CIM_DiagnosticServiceHelper.java
 *
...
public final class CIM_DiagnosticServiceHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_DiagnosticService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_DiagnosticService.Java_Package_List.size(); i++) {
						if (!((String)(CIM_DiagnosticService.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_DiagnosticService.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_DiagnosticService.Java_Package_List.setElementAt((String)(CIM_DiagnosticService.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_DiagnosticService.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_DiagnosticService.");
						resultArrayList.add(new CIM_DiagnosticService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_DiagnosticService.");
						resultArrayList.add(new CIM_DiagnosticService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_DiagnosticService.");
						resultArrayList.add(new CIM_DiagnosticService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_DiagnosticService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_DiagnosticServiceHelper.getInstance(cimClient, new CIM_DiagnosticService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DiagnosticService getInstance(CIMClient cimClient, CIM_DiagnosticService dataInstance) {
		...
		return CIM_DiagnosticServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DiagnosticService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_DiagnosticService dataInstance = null;
		try {
			dataInstance = new CIM_DiagnosticService(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_DiagnosticService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_DiagnosticService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_DiagnosticService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_DiagnosticService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
