 ...
 *
 *
 * Description:  Each instance of UnitOfWork identifies a transaction that is either in-process
 * or already completed. Because instances of 'in-process' UnitsOfWork can be
 * very short-lived and there can be a great number of active ones, use of this
 * class as an instrumentation source for determining response time may be
 * incorrect or inefficient, unless the rate and duration of the UnitsOfWork are
 * known. The intended use is to respond to queries about currently active or
 * recently completed UnitsOfWork. The length of time that a UnitOfWork instance
 * exists after the UnitOfWork completes is not defined and should be assumed to
 * be implementation-dependent. This class is weak to its definition
...
/**
 *  Each instance of UnitOfWork identifies a transaction that is either in-process
 * or already completed. Because instances of 'in-process' UnitsOfWork can be
 * very short-lived and there can be a great number of active ones, use of this
 * class as an instrumentation source for determining response time may be
 * incorrect or inefficient, unless the rate and duration of the UnitsOfWork are
...
public class CIM_UnitOfWork extends CIM_ManagedElement  {
	...
	public final static String CIM_CLASS_NAME = "CIM_UnitOfWork"; //$NON-NLS-1$
	...
	public final static String CIM_ASSOCIATOR_CLASS_NAME_CIM_STARTEDUOW = "CIM_StartedUoW"; //$NON-NLS-1$
	public final static String CIM_ASSOCIATOR_CLASS_NAME_CIM_UOWMETRIC = "CIM_UoWMetric"; //$NON-NLS-1$
