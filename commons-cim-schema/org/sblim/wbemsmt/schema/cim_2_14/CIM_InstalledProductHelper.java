/** 
 * CIM_InstalledProductHelper.java
 *
...
public final class CIM_InstalledProductHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_InstalledProduct.Java_Package_List.size(); i++) {
						if (!((String)(CIM_InstalledProduct.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_InstalledProduct.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_InstalledProduct.Java_Package_List.setElementAt((String)(CIM_InstalledProduct.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_InstalledProduct.");
						resultArrayList.add(new CIM_InstalledProduct(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_InstalledProduct.");
						resultArrayList.add(new CIM_InstalledProduct(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_InstalledProduct.");
						resultArrayList.add(new CIM_InstalledProduct(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_InstalledProduct getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_InstalledProductHelper.getInstance(cimClient, new CIM_InstalledProduct(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_InstalledProduct getInstance(CIMClient cimClient, CIM_InstalledProduct dataInstance) {
		...
		return CIM_InstalledProductHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_InstalledProduct getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_InstalledProduct dataInstance = null;
		try {
			dataInstance = new CIM_InstalledProduct(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_InstalledProduct createInstance(CIMClient cimClient, CIM_InstalledProduct dataInstance, boolean notifyDifferencies){ 
		...
		CIM_InstalledProduct newDataInstance = null;
		try {
			newDataInstance = CIM_InstalledProductHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_InstalledProduct modifyInstance(CIMClient cimClient, CIM_InstalledProduct dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_InstalledProductHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_InstalledProduct dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_InstalledProduct dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_InstalledProduct.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_InstalledProduct.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_InstalledProduct.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_InstalledProduct.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_InstalledProduct.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CollectionID(String CollectionID) {
		return true;
	}
	...
	public static boolean isValid_ProductIdentifyingNumber(String ProductIdentifyingNumber) {
		return true;
	}

	public static boolean isValid_ProductName(String ProductName) {
		return true;
	}

	public static boolean isValid_ProductVendor(String ProductVendor) {
		return true;
	}

	public static boolean isValid_ProductVersion(String ProductVersion) {
		return true;
	}

	public static boolean isValid_SystemID(String SystemID) {
		return true;
	}
