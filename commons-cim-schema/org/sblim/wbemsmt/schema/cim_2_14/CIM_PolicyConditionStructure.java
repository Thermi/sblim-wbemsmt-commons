 ...
 *
 *
 * Description:  PolicyConditions may be aggregated into rules and into compound conditions.
 * PolicyConditionStructure is the abstract aggregation class for the
 * structuring of policy conditions. The Conditions aggregated by a PolicyRule
 * or CompoundPolicyCondition are grouped into two levels of lists: either an
 * ORed set of ANDed sets of conditions (DNF, the default) or an ANDed set of
 * ORed sets of conditions (CNF). Individual PolicyConditions in these lists may
 * be negated. The property ConditionListType specifies which of these two
 * grouping schemes applies to a particular PolicyRule or
 * CompoundPolicyCondition instance. One or more PolicyTimePeriodConditions may
 * be among the conditions associated with a PolicyRule or
 * CompoundPolicyCondition via the PolicyConditionStructure subclass
 * association. In this case, the time periods are simply additional Conditions
 * to be evaluated along with any others that are specified.
 * 
 */
...
/**
 *  PolicyConditions may be aggregated into rules and into compound conditions.
 * PolicyConditionStructure is the abstract aggregation class for the
 * structuring of policy conditions. The Conditions aggregated by a PolicyRule
 * or CompoundPolicyCondition are grouped into two levels of lists: either an
 * ORed set of ANDed sets of conditions (DNF, the default) or an ANDed set of
...
public class CIM_PolicyConditionStructure extends CIM_PolicyComponent  {
	...
	public final static String CIM_CLASS_NAME = "CIM_PolicyConditionStructure"; //$NON-NLS-1$
