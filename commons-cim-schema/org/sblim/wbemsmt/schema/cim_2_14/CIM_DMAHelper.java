...
public final class CIM_DMAHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_DMA.Java_Package_List.size(); i++) {
						if (!((String)(CIM_DMA.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_DMA.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_DMA.Java_Package_List.setElementAt((String)(CIM_DMA.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_DMA.");
						resultArrayList.add(new CIM_DMA(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_DMA.");
						resultArrayList.add(new CIM_DMA(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_DMA.");
						resultArrayList.add(new CIM_DMA(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_DMA getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_DMAHelper.getInstance(cimClient, new CIM_DMA(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DMA getInstance(CIMClient cimClient, CIM_DMA dataInstance) {
		...
		return CIM_DMAHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DMA getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_DMA dataInstance = null;
		try {
			dataInstance = new CIM_DMA(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_DMA createInstance(CIMClient cimClient, CIM_DMA dataInstance, boolean notifyDifferencies){ 
		...
		CIM_DMA newDataInstance = null;
		try {
			newDataInstance = CIM_DMAHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_DMA modifyInstance(CIMClient cimClient, CIM_DMA dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_DMAHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_DMA dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_DMA dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_DMA.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_DMA.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_DMA.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_DMA.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_DMA.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AddressSize(UnsignedInt16 AddressSize) {
		return true;
	}

	public static boolean isValid_Availability(UnsignedInt16 Availability) {
		return true;
	}

	public static boolean isValid_BurstMode(Boolean BurstMode) {
		return true;
	}

	public static boolean isValid_ByteMode(UnsignedInt16 ByteMode) {
		return true;
	}
	...
	public static boolean isValid_ChannelTiming(UnsignedInt16 ChannelTiming) {
		return true;
	}
	...
	public static boolean isValid_DMAChannel(UnsignedInt32 DMAChannel) {
		return true;
	}

	public static boolean isValid_MaxTransferSize(UnsignedInt32 MaxTransferSize) {
		return true;
	}

	public static boolean isValid_TransferWidths(UnsignedInt16[] TransferWidths) {
		return true;
	}

	public static boolean isValid_TypeCTiming(UnsignedInt16 TypeCTiming) {
		return true;
	}

	public static boolean isValid_WordMode(UnsignedInt16 WordMode) {
		return true;
	}
