/** 
 * CIM_MultiStateSensorHelper.java
 *
...
public final class CIM_MultiStateSensorHelper {

	public final static String CIM_CLASS_NAME = "CIM_MultiStateSensor";
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_MultiStateSensor.Java_Package_List.size(); i++) {
						if (!((String)(CIM_MultiStateSensor.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_MultiStateSensor.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_MultiStateSensor.Java_Package_List.setElementAt((String)(CIM_MultiStateSensor.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_MultiStateSensor.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_MultiStateSensor.");
						resultArrayList.add(new CIM_MultiStateSensor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_MultiStateSensor.");
						resultArrayList.add(new CIM_MultiStateSensor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_MultiStateSensor.");
						resultArrayList.add(new CIM_MultiStateSensor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_MultiStateSensor getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_MultiStateSensorHelper.getInstance(cimClient, new CIM_MultiStateSensor(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_MultiStateSensor getInstance(CIMClient cimClient, CIM_MultiStateSensor dataInstance) {
		...
		return CIM_MultiStateSensorHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_MultiStateSensor getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_MultiStateSensor dataInstance = null;
		try {
			dataInstance = new CIM_MultiStateSensor(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_MultiStateSensor createInstance(CIMClient cimClient, CIM_MultiStateSensor dataInstance, boolean notifyDifferencies){ 
		...
		CIM_MultiStateSensor newDataInstance = null;
		try {
			newDataInstance = CIM_MultiStateSensorHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_MultiStateSensor modifyInstance(CIMClient cimClient, CIM_MultiStateSensor dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_MultiStateSensorHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_MultiStateSensor dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_MultiStateSensor dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_MultiStateSensor.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_MultiStateSensor.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_MultiStateSensor.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_MultiStateSensor.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_MultiStateSensor.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
