 ...
 *
 *
 * Description:  DropperService represents the ability to selectively drop network traffic,
 * and/or to invoke another ConditioningService (indicated using the NextService
 * association) for further processing of traffic that is not dropped. It is the
 * base class for different types of droppers. Droppers are distinguished by the
 * algorithm that they use to drop traffic. The NextService association has
 * special semantics for droppers, in addition to conveying the general
 * information of 'what happens next' (that apply to all Conditioning Services).
 * The queue(s) from which a particular dropper drops packets are identified by
 * following chain(s) of NextService associations 'rightwards' from the dropper,
...
public class CIM_DropperService extends CIM_ConditioningService  {
	...
	public final static String CIM_CLASS_NAME = "CIM_DropperService"; //$NON-NLS-1$
