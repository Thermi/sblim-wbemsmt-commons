...
public final class CIM_DirectoryHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_Directory.");
						resultArrayList.add(new CIM_Directory(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_Directory.");
						resultArrayList.add(new CIM_Directory(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_Directory.");
						resultArrayList.add(new CIM_Directory(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_Directory getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_DirectoryHelper.getInstance(cimClient, new CIM_Directory(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Directory getInstance(CIMClient cimClient, CIM_Directory dataInstance) {
		...
		return CIM_DirectoryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Directory getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_Directory(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Directory createInstance(CIMClient cimClient, CIM_Directory dataInstance, boolean notifyDifferencies){ 
		...
		CIM_Directory newDataInstance = null;
		try {
			newDataInstance = CIM_DirectoryHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Directory modifyInstance(CIMClient cimClient, CIM_Directory dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_DirectoryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_Directory dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_Directory dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_Directory.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Directory.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Directory.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Directory.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Directory.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
