...
public final class PRS_ResourceHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<PRS_Resource.Java_Package_List.size(); i++) {
						if (!((String)(PRS_Resource.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(PRS_Resource.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							PRS_Resource.Java_Package_List.setElementAt((String)(PRS_Resource.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Resource.");
						resultArrayList.add(new PRS_Resource(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Resource.");
						resultArrayList.add(new PRS_Resource(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Resource.");
						resultArrayList.add(new PRS_Resource(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Resource getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_ResourceHelper.getInstance(cimClient, new PRS_Resource(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Resource getInstance(CIMClient cimClient, PRS_Resource dataInstance) {
		...
		return PRS_ResourceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Resource getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Resource dataInstance = null;
		try {
			dataInstance = new PRS_Resource(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Resource createInstance(CIMClient cimClient, PRS_Resource dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Resource newDataInstance = null;
		try {
			newDataInstance = PRS_ResourceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Resource modifyInstance(CIMClient cimClient, PRS_Resource dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_ResourceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Resource dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Resource dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Resource.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Resource.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Resource.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Resource.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Resource.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Quantity(UnsignedInt16 Quantity) {
		return true;
	}

	public static boolean isValid_ResourceType(String ResourceType) {
		return true;
	}
