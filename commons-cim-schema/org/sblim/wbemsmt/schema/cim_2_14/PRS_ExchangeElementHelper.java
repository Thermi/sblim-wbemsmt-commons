/** 
 * PRS_ExchangeElementHelper.java
 *
...
public final class PRS_ExchangeElementHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_ExchangeElement.");
						resultArrayList.add(new PRS_ExchangeElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_ExchangeElement.");
						resultArrayList.add(new PRS_ExchangeElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_ExchangeElement.");
						resultArrayList.add(new PRS_ExchangeElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_ExchangeElement getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_ExchangeElementHelper.getInstance(cimClient, new PRS_ExchangeElement(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_ExchangeElement getInstance(CIMClient cimClient, PRS_ExchangeElement dataInstance) {
		...
		return PRS_ExchangeElementHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_ExchangeElement getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_ExchangeElement dataInstance = null;
		try {
			dataInstance = new PRS_ExchangeElement(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_ExchangeElement.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_ExchangeElement.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_ExchangeElement.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_ExchangeElement.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_DirtyFlag(UnsignedInt8 DirtyFlag) {
		return true;
	}

	public static boolean isValid_PRSUniqueName(String PRSUniqueName) {
		return true;
	}
