...
public final class CIM_UnitOfWorkHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_UnitOfWork.Java_Package_List.size(); i++) {
						if (!((String)(CIM_UnitOfWork.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_UnitOfWork.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_UnitOfWork.Java_Package_List.setElementAt((String)(CIM_UnitOfWork.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_UnitOfWork.");
						resultArrayList.add(new CIM_UnitOfWork(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_UnitOfWork.");
						resultArrayList.add(new CIM_UnitOfWork(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_UnitOfWork.");
						resultArrayList.add(new CIM_UnitOfWork(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_UnitOfWork getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_UnitOfWorkHelper.getInstance(cimClient, new CIM_UnitOfWork(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_UnitOfWork getInstance(CIMClient cimClient, CIM_UnitOfWork dataInstance) {
		...
		return CIM_UnitOfWorkHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_UnitOfWork getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_UnitOfWork dataInstance = null;
		try {
			dataInstance = new CIM_UnitOfWork(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_UnitOfWork createInstance(CIMClient cimClient, CIM_UnitOfWork dataInstance, boolean notifyDifferencies){ 
		...
		CIM_UnitOfWork newDataInstance = null;
		try {
			newDataInstance = CIM_UnitOfWorkHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_UnitOfWork modifyInstance(CIMClient cimClient, CIM_UnitOfWork dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_UnitOfWorkHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_UnitOfWork dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_UnitOfWork dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_UnitOfWork.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_UnitOfWork.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_UnitOfWork.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_UnitOfWork.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_UnitOfWork.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ElapsedTime(Calendar ElapsedTime) {
		return true;
	}

	public static boolean isValid_Id(Long Id) {
		return true;
	}

	public static boolean isValid_MutualContextId(String MutualContextId) {
		return true;
	}

	public static boolean isValid_StartTime(Calendar StartTime) {
		return true;
	}

	public static boolean isValid_Status(UnsignedInt16 Status) {
		return true;
	}

	public static boolean isValid_TraceLevel(UnsignedInt32 TraceLevel) {
		return true;
	}

	public static boolean isValid_UoWDefId(String UoWDefId) {
		return true;
	}
