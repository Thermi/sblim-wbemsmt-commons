 ...
 * the connection is not treated as a ManagedElement itself (that is, with
 * Status, statistics, and so on), but its existence is known. The SAPs that are
 * connected are typically at the same networking or application layer.
 * 
 */
