...
public final class CIM_VoltageSensorHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_VoltageSensor.");
						resultArrayList.add(new CIM_VoltageSensor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_VoltageSensor.");
						resultArrayList.add(new CIM_VoltageSensor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_VoltageSensor.");
						resultArrayList.add(new CIM_VoltageSensor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_VoltageSensor getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_VoltageSensorHelper.getInstance(cimClient, new CIM_VoltageSensor(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_VoltageSensor getInstance(CIMClient cimClient, CIM_VoltageSensor dataInstance) {
		...
		return CIM_VoltageSensorHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_VoltageSensor getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_VoltageSensor dataInstance = null;
		try {
			dataInstance = new CIM_VoltageSensor(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_VoltageSensor createInstance(CIMClient cimClient, CIM_VoltageSensor dataInstance, boolean notifyDifferencies){ 
		...
		CIM_VoltageSensor newDataInstance = null;
		try {
			newDataInstance = CIM_VoltageSensorHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_VoltageSensor modifyInstance(CIMClient cimClient, CIM_VoltageSensor dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_VoltageSensorHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_VoltageSensor dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_VoltageSensor dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_VoltageSensor.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_VoltageSensor.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_VoltageSensor.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_VoltageSensor.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_VoltageSensor.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
