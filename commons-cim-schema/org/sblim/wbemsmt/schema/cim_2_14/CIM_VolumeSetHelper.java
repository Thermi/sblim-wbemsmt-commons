...
public final class CIM_VolumeSetHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_VolumeSet.Java_Package_List.size(); i++) {
						if (!((String)(CIM_VolumeSet.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_VolumeSet.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_VolumeSet.Java_Package_List.setElementAt((String)(CIM_VolumeSet.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_VolumeSet.");
						resultArrayList.add(new CIM_VolumeSet(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_VolumeSet.");
						resultArrayList.add(new CIM_VolumeSet(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_VolumeSet.");
						resultArrayList.add(new CIM_VolumeSet(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_VolumeSet getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_VolumeSetHelper.getInstance(cimClient, new CIM_VolumeSet(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_VolumeSet getInstance(CIMClient cimClient, CIM_VolumeSet dataInstance) {
		...
		return CIM_VolumeSetHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_VolumeSet getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_VolumeSet dataInstance = null;
		try {
			dataInstance = new CIM_VolumeSet(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_VolumeSet createInstance(CIMClient cimClient, CIM_VolumeSet dataInstance, boolean notifyDifferencies){ 
		...
		CIM_VolumeSet newDataInstance = null;
		try {
			newDataInstance = CIM_VolumeSetHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_VolumeSet modifyInstance(CIMClient cimClient, CIM_VolumeSet dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_VolumeSetHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_VolumeSet dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_VolumeSet dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_VolumeSet.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_VolumeSet.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_VolumeSet.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_VolumeSet.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_VolumeSet.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_PSExtentInterleaveDepth(UnsignedInt64 PSExtentInterleaveDepth) {
		return true;
	}

	public static boolean isValid_PSExtentStripeLength(UnsignedInt64 PSExtentStripeLength) {
		return true;
	}

	public static boolean isValid_VolumeStatus(UnsignedInt16 VolumeStatus) {
		return true;
	}
