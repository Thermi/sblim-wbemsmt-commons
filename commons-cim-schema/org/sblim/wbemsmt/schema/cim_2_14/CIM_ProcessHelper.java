...
public final class CIM_ProcessHelper {
	...
	*/	
	public final static CIM_Process getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ProcessHelper.getInstance(cimClient, new CIM_Process(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Process getInstance(CIMClient cimClient, CIM_Process dataInstance) {
		...
		return CIM_ProcessHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Process getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_Process(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_Process.getPackages();
	...
	*/	
	public final static CIM_Process createInstance(CIMClient cimClient, CIM_Process dataInstance, boolean notifyDifferencies){ 
		...
		CIM_Process newDataInstance = null;
		try {
			newDataInstance = CIM_ProcessHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Process modifyInstance(CIMClient cimClient, CIM_Process dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_ProcessHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_Process dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_Process dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_Process.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Process.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Process.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Process.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Process.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ExecutionState(UnsignedInt16 ExecutionState) {
		return true;
	}
	...
	public static boolean isValid_KernelModeTime(UnsignedInt64 KernelModeTime) {
		return true;
	}
	...
	public static boolean isValid_OtherExecutionDescription(String OtherExecutionDescription) {
		return true;
	}
	...
	public static boolean isValid_TerminationDate(Calendar TerminationDate) {
		return true;
	}

	public static boolean isValid_UserModeTime(UnsignedInt64 UserModeTime) {
		return true;
	}

	public static boolean isValid_WorkingSetSize(UnsignedInt64 WorkingSetSize) {
		return true;
	}
