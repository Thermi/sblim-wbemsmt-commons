 ...
 *
 *
 * Description:  BGPIPRoute describes a BGP routing entry that connects two peer routers that
 * are running BGP. The source and destination addresses may be either specific
 * IP endpoints or IP subnets. Examining the BGPIPRoute class definition, note
 * that its superclass IPRoute is deprecated. Unfortunately, IPRoute cannot be
 * removed from the object hierarchy without a major Schema release. When/if
 * this occurs, the IPRoute superclass will be removed, and BGPIPRoute will
 * subclass from CIM_NextHopRouting directly.
 * 
 */
...
/**
 *  BGPIPRoute describes a BGP routing entry that connects two peer routers that
 * are running BGP. The source and destination addresses may be either specific
 * IP endpoints or IP subnets. Examining the BGPIPRoute class definition, note
 * that its superclass IPRoute is deprecated. Unfortunately, IPRoute cannot be
 * removed from the object hierarchy without a major Schema release. When/if
...
public class CIM_BGPIPRoute extends CIM_IPRoute  {
	...
	public final static String CIM_CLASS_NAME = "CIM_BGPIPRoute"; //$NON-NLS-1$
	public final static String CIM_CLASS_DISPLAYNAME = CIM_CLASS_NAME;

	private boolean validCimInstance = false;
	
	public final static String CIM_CLASS_VERSION = "2.6.0";
	public final static String CIM_ASSOCIATOR_CLASS_NAME_CIM_BGPATTRIBUTESFORROUTE = "CIM_BGPAttributesForRoute"; //$NON-NLS-1$
	...
	*/
	...
	static {
		...
		for (int i = 0; i < CIM_IPRoute.Java_Package_List.size(); i++) {
			if (((String)CIM_IPRoute.Java_Package_List.elementAt(i)).equals("org.sblim.wbemsmt.schema.cim_2_14")){
				continue;
			}
