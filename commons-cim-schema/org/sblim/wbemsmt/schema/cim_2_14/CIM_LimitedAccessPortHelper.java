/** 
 * CIM_LimitedAccessPortHelper.java
 *
...
public final class CIM_LimitedAccessPortHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_LimitedAccessPort(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_LimitedAccessPort.Java_Package_List.size(); i++) {
						if (!((String)(CIM_LimitedAccessPort.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_LimitedAccessPort.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_LimitedAccessPort.Java_Package_List.setElementAt((String)(CIM_LimitedAccessPort.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_LimitedAccessPort.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_LimitedAccessPort.");
						resultArrayList.add(new CIM_LimitedAccessPort(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_LimitedAccessPort.");
						resultArrayList.add(new CIM_LimitedAccessPort(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_LimitedAccessPort.");
						resultArrayList.add(new CIM_LimitedAccessPort(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_LimitedAccessPort getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_LimitedAccessPortHelper.getInstance(cimClient, new CIM_LimitedAccessPort(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LimitedAccessPort getInstance(CIMClient cimClient, CIM_LimitedAccessPort dataInstance) {
		...
		return CIM_LimitedAccessPortHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LimitedAccessPort getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_LimitedAccessPort dataInstance = null;
		try {
			dataInstance = new CIM_LimitedAccessPort(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_LimitedAccessPort createInstance(CIMClient cimClient, CIM_LimitedAccessPort dataInstance, boolean notifyDifferencies){ 
		...
		CIM_LimitedAccessPort newDataInstance = null;
		try {
			newDataInstance = CIM_LimitedAccessPortHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_LimitedAccessPort modifyInstance(CIMClient cimClient, CIM_LimitedAccessPort dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_LimitedAccessPortHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_LimitedAccessPort dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_LimitedAccessPort dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_LimitedAccessPort.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_LimitedAccessPort.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_LimitedAccessPort.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_LimitedAccessPort.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_LimitedAccessPort.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Direction(UnsignedInt16 Direction) {
		return true;
	}

	public static boolean isValid_ExportCount(UnsignedInt64 ExportCount) {
		return true;
	}

	public static boolean isValid_ExtendTimeout(UnsignedInt32 ExtendTimeout) {
		return true;
	}

	public static boolean isValid_Extended(Boolean Extended) {
		return true;
	}

	public static boolean isValid_ImportCount(UnsignedInt64 ImportCount) {
		return true;
	}

	public static boolean isValid_LastExtended(Calendar LastExtended) {
		return true;
	}

	public static boolean isValid_Locked(Boolean Locked) {
		return true;
	}
