...
public final class CIM_QueuingServiceHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_QueuingService.Java_Package_List.size(); i++) {
						if (!((String)(CIM_QueuingService.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_QueuingService.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_QueuingService.Java_Package_List.setElementAt((String)(CIM_QueuingService.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_QueuingService.");
						resultArrayList.add(new CIM_QueuingService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_QueuingService.");
						resultArrayList.add(new CIM_QueuingService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_QueuingService.");
						resultArrayList.add(new CIM_QueuingService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_QueuingService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_QueuingServiceHelper.getInstance(cimClient, new CIM_QueuingService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_QueuingService getInstance(CIMClient cimClient, CIM_QueuingService dataInstance) {
		...
		return CIM_QueuingServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_QueuingService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_QueuingService dataInstance = null;
		try {
			dataInstance = new CIM_QueuingService(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_QueuingService createInstance(CIMClient cimClient, CIM_QueuingService dataInstance, boolean notifyDifferencies){ 
		...
		CIM_QueuingService newDataInstance = null;
		try {
			newDataInstance = CIM_QueuingServiceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_QueuingService modifyInstance(CIMClient cimClient, CIM_QueuingService dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_QueuingServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_QueuingService dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_QueuingService dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_QueuingService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_QueuingService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_QueuingService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_QueuingService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_QueuingService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CurrentQueueDepth(UnsignedInt32 CurrentQueueDepth) {
		return true;
	}

	public static boolean isValid_DepthUnits(UnsignedInt16 DepthUnits) {
		return true;
	}
