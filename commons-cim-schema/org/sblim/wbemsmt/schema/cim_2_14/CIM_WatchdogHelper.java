...
public final class CIM_WatchdogHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_Watchdog(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_Watchdog.Java_Package_List.size(); i++) {
						if (!((String)(CIM_Watchdog.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_Watchdog.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_Watchdog.Java_Package_List.setElementAt((String)(CIM_Watchdog.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_Watchdog.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_Watchdog.");
						resultArrayList.add(new CIM_Watchdog(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_Watchdog.");
						resultArrayList.add(new CIM_Watchdog(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_Watchdog.");
						resultArrayList.add(new CIM_Watchdog(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_Watchdog getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_WatchdogHelper.getInstance(cimClient, new CIM_Watchdog(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Watchdog getInstance(CIMClient cimClient, CIM_Watchdog dataInstance) {
		...
		return CIM_WatchdogHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Watchdog getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_Watchdog dataInstance = null;
		try {
			dataInstance = new CIM_Watchdog(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Watchdog createInstance(CIMClient cimClient, CIM_Watchdog dataInstance, boolean notifyDifferencies){ 
		...
		CIM_Watchdog newDataInstance = null;
		try {
			newDataInstance = CIM_WatchdogHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Watchdog modifyInstance(CIMClient cimClient, CIM_Watchdog dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_WatchdogHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_Watchdog dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_Watchdog dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_Watchdog.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Watchdog.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Watchdog.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Watchdog.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Watchdog.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ActionOnExpiration(UnsignedInt16 ActionOnExpiration) {
		return true;
	}

	public static boolean isValid_MonitoredEntity(UnsignedInt16 MonitoredEntity) {
		return true;
	}

	public static boolean isValid_MonitoredEntityDescription(String MonitoredEntityDescription) {
		return true;
	}

	public static boolean isValid_MonitoredEntityOnLastExpiration(UnsignedInt16 MonitoredEntityOnLastExpiration) {
		return true;
	}

	public static boolean isValid_TimeOfLastExpiration(Calendar TimeOfLastExpiration) {
		return true;
	}

	public static boolean isValid_TimeoutInterval(UnsignedInt32 TimeoutInterval) {
		return true;
	}

	public static boolean isValid_TimerResolution(UnsignedInt32 TimerResolution) {
		return true;
	}
