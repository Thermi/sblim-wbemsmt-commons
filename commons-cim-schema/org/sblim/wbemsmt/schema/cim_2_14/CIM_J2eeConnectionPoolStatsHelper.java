/** 
 * CIM_J2eeConnectionPoolStatsHelper.java
 *
...
public final class CIM_J2eeConnectionPoolStatsHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_J2eeConnectionPoolStats(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					...
					for (int i=0; clazz==null && i<CIM_J2eeConnectionPoolStats.Java_Package_List.size(); i++) {
						if (!((String)(CIM_J2eeConnectionPoolStats.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_J2eeConnectionPoolStats.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_J2eeConnectionPoolStats.Java_Package_List.setElementAt((String)(CIM_J2eeConnectionPoolStats.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_J2eeConnectionPoolStats.");
						resultArrayList.add(new CIM_J2eeConnectionPoolStats(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_J2eeConnectionPoolStats.");
						resultArrayList.add(new CIM_J2eeConnectionPoolStats(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_J2eeConnectionPoolStats.");
						resultArrayList.add(new CIM_J2eeConnectionPoolStats(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_J2eeConnectionPoolStats getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_J2eeConnectionPoolStatsHelper.getInstance(cimClient, new CIM_J2eeConnectionPoolStats(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_J2eeConnectionPoolStats getInstance(CIMClient cimClient, CIM_J2eeConnectionPoolStats dataInstance) {
		...
		return CIM_J2eeConnectionPoolStatsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_J2eeConnectionPoolStats getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_J2eeConnectionPoolStats dataInstance = null;
		try {
			dataInstance = new CIM_J2eeConnectionPoolStats(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_J2eeConnectionPoolStats createInstance(CIMClient cimClient, CIM_J2eeConnectionPoolStats dataInstance, boolean notifyDifferencies){ 
		...
		CIM_J2eeConnectionPoolStats newDataInstance = null;
		try {
			newDataInstance = CIM_J2eeConnectionPoolStatsHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_J2eeConnectionPoolStats modifyInstance(CIMClient cimClient, CIM_J2eeConnectionPoolStats dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_J2eeConnectionPoolStatsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_J2eeConnectionPoolStats dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_J2eeConnectionPoolStats dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_J2eeConnectionPoolStats.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_J2eeConnectionPoolStats.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_J2eeConnectionPoolStats.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_J2eeConnectionPoolStats.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_J2eeConnectionPoolStats.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CloseCount(UnsignedInt64 CloseCount) {
		return true;
	}

	public static boolean isValid_CreateCount(UnsignedInt64 CreateCount) {
		return true;
	}

	public static boolean isValid_FreePoolSize(UnsignedInt64 FreePoolSize) {
		return true;
	}

	public static boolean isValid_FreePoolSizeHighWaterMark(UnsignedInt64 FreePoolSizeHighWaterMark) {
		return true;
	}

	public static boolean isValid_FreePoolSizeLowWaterMark(UnsignedInt64 FreePoolSizeLowWaterMark) {
		return true;
	}

	public static boolean isValid_FreePoolSizeLowerBound(UnsignedInt64 FreePoolSizeLowerBound) {
		return true;
	}

	public static boolean isValid_FreePoolSizeUpperBound(UnsignedInt64 FreePoolSizeUpperBound) {
		return true;
	}

	public static boolean isValid_PoolSize(UnsignedInt64 PoolSize) {
		return true;
	}

	public static boolean isValid_PoolSizeHighWaterMark(UnsignedInt64 PoolSizeHighWaterMark) {
		return true;
	}

	public static boolean isValid_PoolSizeLowWaterMark(UnsignedInt64 PoolSizeLowWaterMark) {
		return true;
	}

	public static boolean isValid_PoolSizeLowerBound(UnsignedInt64 PoolSizeLowerBound) {
		return true;
	}

	public static boolean isValid_PoolSizeUpperBound(UnsignedInt64 PoolSizeUpperBound) {
		return true;
	}

	public static boolean isValid_WaitingThreadCount(UnsignedInt64 WaitingThreadCount) {
		return true;
	}

	public static boolean isValid_WaitingThreadCountHighWaterMark(UnsignedInt64 WaitingThreadCountHighWaterMark) {
		return true;
	}

	public static boolean isValid_WaitingThreadCountLowWaterMark(UnsignedInt64 WaitingThreadCountLowWaterMark) {
		return true;
	}

	public static boolean isValid_WaitingThreadCountLowerBound(UnsignedInt64 WaitingThreadCountLowerBound) {
		return true;
	}

	public static boolean isValid_WaitingThreadCountUpperBound(UnsignedInt64 WaitingThreadCountUpperBound) {
		return true;
	}
