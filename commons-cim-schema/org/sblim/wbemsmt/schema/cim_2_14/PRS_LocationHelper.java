...
public final class PRS_LocationHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Location.");
						resultArrayList.add(new PRS_Location(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Location.");
						resultArrayList.add(new PRS_Location(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Location.");
						resultArrayList.add(new PRS_Location(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Location getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_LocationHelper.getInstance(cimClient, new PRS_Location(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Location getInstance(CIMClient cimClient, PRS_Location dataInstance) {
		...
		return PRS_LocationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Location getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Location dataInstance = null;
		try {
			dataInstance = new PRS_Location(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Location createInstance(CIMClient cimClient, PRS_Location dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Location newDataInstance = null;
		try {
			newDataInstance = PRS_LocationHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Location modifyInstance(CIMClient cimClient, PRS_Location dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_LocationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Location dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Location dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Location.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Location.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Location.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Location.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Location.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_LocationID(String LocationID) {
		return true;
	}

	public static boolean isValid_LocationType(UnsignedInt8 LocationType) {
		return true;
	}
