 ...
 *
 *
 * Description:  This class models host environment factors that influence the behavior of
 * Storage Systems. For example, a disk array has different SCSI responses for
 * clients (initiators) configured as AIX verses HPUX. Instances of this setting
 * class can be associated via ElementSettingData to a storage system's
 * LogicalPort, ProtocolController, or Volume instances when these elements have
 * host awareness. These associations are created by the management
 * instrumentation provider to reflect the current configuration. A client
 * deletes/creates these associations to request changes in element
 * host-awareness. This settings class can also be associated with
 * StorageHardwareID instances when that HW ID is configured with host
 * information. An instance of this setting may include several ClientType
 * values if the storage system treats them identically. The storage system
 * exposes all supported setting instances to a WBEM Enumerate request; the
 * client uses the returned settings to determine which types are available.
 * 
 */
...
/**
 *  This class models host environment factors that influence the behavior of
 * Storage Systems. For example, a disk array has different SCSI responses for
 * clients (initiators) configured as AIX verses HPUX. Instances of this setting
 * class can be associated via ElementSettingData to a storage system's
 * LogicalPort, ProtocolController, or Volume instances when these elements have
...
public class CIM_StorageClientSettingData extends CIM_SettingData  {
	...
	public final static String CIM_CLASS_NAME = "CIM_StorageClientSettingData"; //$NON-NLS-1$
