...
public final class CIM_BufferPoolHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_BufferPool.Java_Package_List.size(); i++) {
						if (!((String)(CIM_BufferPool.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_BufferPool.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_BufferPool.Java_Package_List.setElementAt((String)(CIM_BufferPool.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_BufferPool.");
						resultArrayList.add(new CIM_BufferPool(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_BufferPool.");
						resultArrayList.add(new CIM_BufferPool(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_BufferPool.");
						resultArrayList.add(new CIM_BufferPool(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_BufferPool getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_BufferPoolHelper.getInstance(cimClient, new CIM_BufferPool(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_BufferPool getInstance(CIMClient cimClient, CIM_BufferPool dataInstance) {
		...
		return CIM_BufferPoolHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_BufferPool getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_BufferPool dataInstance = null;
		try {
			dataInstance = new CIM_BufferPool(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_BufferPool createInstance(CIMClient cimClient, CIM_BufferPool dataInstance, boolean notifyDifferencies){ 
		...
		CIM_BufferPool newDataInstance = null;
		try {
			newDataInstance = CIM_BufferPoolHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_BufferPool modifyInstance(CIMClient cimClient, CIM_BufferPool dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_BufferPoolHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_BufferPool dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_BufferPool dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_BufferPool.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_BufferPool.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_BufferPool.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_BufferPool.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_BufferPool.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AvailableBuffers(UnsignedInt32 AvailableBuffers) {
		return true;
	}

	public static boolean isValid_BufferSize(UnsignedInt16 BufferSize) {
		return true;
	}
	...
	public static boolean isValid_SharedBuffers(UnsignedInt32 SharedBuffers) {
		return true;
	}

	public static boolean isValid_SizeOfBuffers(UnsignedInt32 SizeOfBuffers) {
		return true;
	}

	public static boolean isValid_TotalBuffers(UnsignedInt32 TotalBuffers) {
		return true;
	}
