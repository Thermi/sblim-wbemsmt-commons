...
public final class CIM_USBDeviceHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_USBDevice.Java_Package_List.size(); i++) {
						if (!((String)(CIM_USBDevice.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_USBDevice.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_USBDevice.Java_Package_List.setElementAt((String)(CIM_USBDevice.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_USBDevice.");
						resultArrayList.add(new CIM_USBDevice(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_USBDevice.");
						resultArrayList.add(new CIM_USBDevice(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_USBDevice.");
						resultArrayList.add(new CIM_USBDevice(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_USBDevice getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_USBDeviceHelper.getInstance(cimClient, new CIM_USBDevice(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_USBDevice getInstance(CIMClient cimClient, CIM_USBDevice dataInstance) {
		...
		return CIM_USBDeviceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_USBDevice getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_USBDevice dataInstance = null;
		try {
			dataInstance = new CIM_USBDevice(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_USBDevice createInstance(CIMClient cimClient, CIM_USBDevice dataInstance, boolean notifyDifferencies){ 
		...
		CIM_USBDevice newDataInstance = null;
		try {
			newDataInstance = CIM_USBDeviceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_USBDevice modifyInstance(CIMClient cimClient, CIM_USBDevice dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_USBDeviceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_USBDevice dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_USBDevice dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_USBDevice.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_USBDevice.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_USBDevice.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_USBDevice.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_USBDevice.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CurrentAlternateSettings(UnsignedInt8[] CurrentAlternateSettings) {
		return true;
	}

	public static boolean isValid_CurrentConfigValue(UnsignedInt8 CurrentConfigValue) {
		return true;
	}

	public static boolean isValid_NumberOfConfigs(UnsignedInt8 NumberOfConfigs) {
		return true;
	}

	public static boolean isValid_ProtocolCode(UnsignedInt8 ProtocolCode) {
		return true;
	}

	public static boolean isValid_SubclassCode(UnsignedInt8 SubclassCode) {
		return true;
	}

	public static boolean isValid_USBVersion(UnsignedInt16 USBVersion) {
		return true;
	}
