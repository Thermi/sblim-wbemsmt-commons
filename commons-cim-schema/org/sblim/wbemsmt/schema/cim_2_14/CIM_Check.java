 ...
 *
 *
 * Description:  A CIM_Check is a condition or characteristic that is expected to be true in an
 * environment defined or scoped by an instance of a CIM_ComputerSystem. The
 * Checks associated with a particular SoftwareElement are organized into one of
 * two groups using the Phase property of the CIM_SoftwareElementChecks
 * association. Conditions that are expected to be true when a SoftwareElement
 * is in a particular state and environment are known as 'in-state' conditions.
 * Conditions that need to be satisfied in order to transition the
 * SoftwareElement to its next state are known as 'next-state' conditions. A
 * CIM_ComputerSystem object represents the environment in which
 * CIM_SoftwareElements are already deployed/installed or into which the
 * elements will be deployed/installed. For the case in which an element is
 * already installed, the CIM_InstalledSoftwareElement association identifies
 * the CIM_ComputerSystem object that represents the "environment". When a
 * SoftwareElement is being deployed for installation on a ComputerSystem, that
 * system is the target of the Check and is identified using the TargetSystem
 * reference of the InvokeOnSystem method.
 * 
 */
...
/**
 *  A CIM_Check is a condition or characteristic that is expected to be true in an
 * environment defined or scoped by an instance of a CIM_ComputerSystem. The
 * Checks associated with a particular SoftwareElement are organized into one of
 * two groups using the Phase property of the CIM_SoftwareElementChecks
 * association. Conditions that are expected to be true when a SoftwareElement
...
public class CIM_Check extends CIM_ManagedElement  {
	...
	public final static String CIM_CLASS_NAME = "CIM_Check"; //$NON-NLS-1$
