 ...
 *
 *
 * Description:  CIM_Watchdog is a timer that is implemented in the system hardware. It allows
 * the hardware to monitor the state of the Operating System, BIOS, or a
 * software component that is installed on the System. If the monitored
 * component fails to re-arm the timer before its expiration, the hardware
 * assumes that the System is in a critical state, and could reset the
 * ComputerSystem. This feature can also be used as an application watchdog
 * timer for a mission-critical application. In this case, the application would
 * assume responsibility for re-arming the timer before expiration.
 * 
 */
...
/**
 *  CIM_Watchdog is a timer that is implemented in the system hardware. It allows
 * the hardware to monitor the state of the Operating System, BIOS, or a
 * software component that is installed on the System. If the monitored
 * component fails to re-arm the timer before its expiration, the hardware
 * assumes that the System is in a critical state, and could reset the
...
public class CIM_Watchdog extends CIM_LogicalDevice  {
	...
	*/
	...
	/**
	*	The entity that is currently being monitored by the WatchDog. This property is used to identify the module that is responsible for re-arming, or whose information is used to re-arm, the watchdog at periodic intervals.
	*/
	public final static String CIM_PROPERTY_MONITOREDENTITY = "MonitoredEntity"; //$NON-NLS-1$
	/**
	*	A string that describes more textual information about the monitored entity.
	*/
	...
	*/
	...
	/**
	*	The timeout interval that is used by the watchdog, in MicroSeconds.
	*/
