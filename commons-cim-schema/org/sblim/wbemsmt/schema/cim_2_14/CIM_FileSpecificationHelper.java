/** 
 * CIM_FileSpecificationHelper.java
 *
...
public final class CIM_FileSpecificationHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_FileSpecification.Java_Package_List.size(); i++) {
						if (!((String)(CIM_FileSpecification.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_FileSpecification.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_FileSpecification.Java_Package_List.setElementAt((String)(CIM_FileSpecification.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_FileSpecification.");
						resultArrayList.add(new CIM_FileSpecification(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_FileSpecification.");
						resultArrayList.add(new CIM_FileSpecification(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_FileSpecification.");
						resultArrayList.add(new CIM_FileSpecification(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_FileSpecification getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_FileSpecificationHelper.getInstance(cimClient, new CIM_FileSpecification(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_FileSpecification getInstance(CIMClient cimClient, CIM_FileSpecification dataInstance) {
		...
		return CIM_FileSpecificationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_FileSpecification getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_FileSpecification dataInstance = null;
		try {
			dataInstance = new CIM_FileSpecification(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_FileSpecification createInstance(CIMClient cimClient, CIM_FileSpecification dataInstance, boolean notifyDifferencies){ 
		...
		CIM_FileSpecification newDataInstance = null;
		try {
			newDataInstance = CIM_FileSpecificationHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_FileSpecification modifyInstance(CIMClient cimClient, CIM_FileSpecification dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_FileSpecificationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_FileSpecification dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_FileSpecification dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_FileSpecification.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_FileSpecification.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_FileSpecification.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_FileSpecification.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_FileSpecification.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CRC1(UnsignedInt32 CRC1) {
		return true;
	}

	public static boolean isValid_CRC2(UnsignedInt32 CRC2) {
		return true;
	}

	public static boolean isValid_CheckSum(UnsignedInt32 CheckSum) {
		return true;
	}

	public static boolean isValid_CreateTimeStamp(Calendar CreateTimeStamp) {
		return true;
	}
	...
	public static boolean isValid_MD5Checksum(String MD5Checksum) {
		return true;
	}
