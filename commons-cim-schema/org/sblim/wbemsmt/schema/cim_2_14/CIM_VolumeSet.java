 ...
 *
 *
 * Description:  VolumeSet is deprecated in lieu of CompositeExtent, which removes the
 * restriction that the Set must be exported. A VolumeSet can be used to model
 * an SCC RAID implementation. VolumeSets represent a contiguous range of
 * logical blocks presented to the operating environment. VolumeSets may
 * overlap. However, the underlying PhysicalExtents within the overlap shall not
 * contain any check data. They are BasedOn one or more ProtectedSpaceExtents.
 * These BasedOn associations should be instantiated or subclassed as needed.
 * 
 */
...
/**
 *  VolumeSet is deprecated in lieu of CompositeExtent, which removes the
 * restriction that the Set must be exported. A VolumeSet can be used to model
 * an SCC RAID implementation. VolumeSets represent a contiguous range of
 * logical blocks presented to the operating environment. VolumeSets may
 * overlap. However, the underlying PhysicalExtents within the overlap shall not
...
public class CIM_VolumeSet extends CIM_StorageVolume  {
	...
	public final static String CIM_CLASS_NAME = "CIM_VolumeSet"; //$NON-NLS-1$
	...
	*/
	...
	static {
		...
		for (int i = 0; i < CIM_StorageVolume.Java_Package_List.size(); i++) {
			if (((String)CIM_StorageVolume.Java_Package_List.elementAt(i)).equals("org.sblim.wbemsmt.schema.cim_2_14")){
				continue;
			}
