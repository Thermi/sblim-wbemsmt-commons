 ...
 * Devices that provide a classic bus master interface. Examples of Controllers
 * are USBControllers, SerialControllers, and so on. The Controller class is an
 * abstraction for Devices with a single protocol stack, which exist to control
 * communications (data, control, and reset) to downstream devices. Note that a
 * new abstract class (ProtocolController) has been created to model more
 * complex interface controllers such as SCSI.
 * 
 */
...
/**
 *  Controller is a superclass for grouping the miscellaneous control-related
 * Devices that provide a classic bus master interface. Examples of Controllers
 * are USBControllers, SerialControllers, and so on. The Controller class is an
 * abstraction for Devices with a single protocol stack, which exist to control
 * communications (data, control, and reset) to downstream devices. Note that a
...
public class CIM_Controller extends CIM_LogicalDevice  {
	...
	public final static String CIM_CLASS_NAME = "CIM_Controller"; //$NON-NLS-1$
