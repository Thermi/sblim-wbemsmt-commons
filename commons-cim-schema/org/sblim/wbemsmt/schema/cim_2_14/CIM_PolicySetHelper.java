...
public final class CIM_PolicySetHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_PolicySet.Java_Package_List.size(); i++) {
						if (!((String)(CIM_PolicySet.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_PolicySet.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_PolicySet.Java_Package_List.setElementAt((String)(CIM_PolicySet.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_PolicySet.");
						resultArrayList.add(new CIM_PolicySet(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_PolicySet.");
						resultArrayList.add(new CIM_PolicySet(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_PolicySet.");
						resultArrayList.add(new CIM_PolicySet(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_PolicySet getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_PolicySetHelper.getInstance(cimClient, new CIM_PolicySet(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PolicySet getInstance(CIMClient cimClient, CIM_PolicySet dataInstance) {
		...
		return CIM_PolicySetHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PolicySet getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_PolicySet dataInstance = null;
		try {
			dataInstance = new CIM_PolicySet(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_PolicySet.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_PolicySet.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_PolicySet.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_PolicySet.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Enabled(UnsignedInt16 Enabled) {
		return true;
	}

	public static boolean isValid_PolicyDecisionStrategy(UnsignedInt16 PolicyDecisionStrategy) {
		return true;
	}

	public static boolean isValid_PolicyRoles(String[] PolicyRoles) {
		return true;
	}
