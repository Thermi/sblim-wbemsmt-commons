...
public final class PRS_SolutionHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Solution.");
						resultArrayList.add(new PRS_Solution(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Solution.");
						resultArrayList.add(new PRS_Solution(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Solution.");
						resultArrayList.add(new PRS_Solution(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Solution getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_SolutionHelper.getInstance(cimClient, new PRS_Solution(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Solution getInstance(CIMClient cimClient, PRS_Solution dataInstance) {
		...
		return PRS_SolutionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Solution getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Solution dataInstance = null;
		try {
			dataInstance = new PRS_Solution(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Solution createInstance(CIMClient cimClient, PRS_Solution dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Solution newDataInstance = null;
		try {
			newDataInstance = PRS_SolutionHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Solution modifyInstance(CIMClient cimClient, PRS_Solution dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_SolutionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Solution dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Solution dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Solution.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Solution.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Solution.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Solution.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Solution.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_SolutionAbstract(String SolutionAbstract) {
		return true;
	}

	public static boolean isValid_SolutionType(UnsignedInt8 SolutionType) {
		return true;
	}
