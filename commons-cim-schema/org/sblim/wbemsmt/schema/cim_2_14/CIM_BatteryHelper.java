...
public final class CIM_BatteryHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_Battery.Java_Package_List.size(); i++) {
						if (!((String)(CIM_Battery.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_Battery.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_Battery.Java_Package_List.setElementAt((String)(CIM_Battery.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_Battery.");
						resultArrayList.add(new CIM_Battery(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_Battery.");
						resultArrayList.add(new CIM_Battery(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_Battery.");
						resultArrayList.add(new CIM_Battery(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_Battery getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_BatteryHelper.getInstance(cimClient, new CIM_Battery(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Battery getInstance(CIMClient cimClient, CIM_Battery dataInstance) {
		...
		return CIM_BatteryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Battery getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_Battery dataInstance = null;
		try {
			dataInstance = new CIM_Battery(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Battery createInstance(CIMClient cimClient, CIM_Battery dataInstance, boolean notifyDifferencies){ 
		...
		CIM_Battery newDataInstance = null;
		try {
			newDataInstance = CIM_BatteryHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Battery modifyInstance(CIMClient cimClient, CIM_Battery dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_BatteryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_Battery dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_Battery dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_Battery.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Battery.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Battery.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Battery.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Battery.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_BatteryStatus(UnsignedInt16 BatteryStatus) {
		return true;
	}

	public static boolean isValid_Chemistry(UnsignedInt16 Chemistry) {
		return true;
	}

	public static boolean isValid_DesignCapacity(UnsignedInt32 DesignCapacity) {
		return true;
	}

	public static boolean isValid_DesignVoltage(UnsignedInt64 DesignVoltage) {
		return true;
	}
	...
	public static boolean isValid_ExpectedLife(UnsignedInt32 ExpectedLife) {
		return true;
	}

	public static boolean isValid_FullChargeCapacity(UnsignedInt32 FullChargeCapacity) {
		return true;
	}

	public static boolean isValid_MaxRechargeTime(UnsignedInt32 MaxRechargeTime) {
		return true;
	}

	public static boolean isValid_SmartBatteryVersion(String SmartBatteryVersion) {
		return true;
	}

	public static boolean isValid_TimeOnBattery(UnsignedInt32 TimeOnBattery) {
		return true;
	}

	public static boolean isValid_TimeToFullCharge(UnsignedInt32 TimeToFullCharge) {
		return true;
	}
