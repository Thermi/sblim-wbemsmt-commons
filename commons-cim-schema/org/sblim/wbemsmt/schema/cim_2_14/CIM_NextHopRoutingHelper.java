...
public final class CIM_NextHopRoutingHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_NextHopRouting.Java_Package_List.size(); i++) {
						if (!((String)(CIM_NextHopRouting.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_NextHopRouting.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_NextHopRouting.Java_Package_List.setElementAt((String)(CIM_NextHopRouting.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_NextHopRouting.");
						resultArrayList.add(new CIM_NextHopRouting(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_NextHopRouting.");
						resultArrayList.add(new CIM_NextHopRouting(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_NextHopRouting.");
						resultArrayList.add(new CIM_NextHopRouting(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_NextHopRouting getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_NextHopRoutingHelper.getInstance(cimClient, new CIM_NextHopRouting(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_NextHopRouting getInstance(CIMClient cimClient, CIM_NextHopRouting dataInstance) {
		...
		return CIM_NextHopRoutingHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_NextHopRouting getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_NextHopRouting dataInstance = null;
		try {
			dataInstance = new CIM_NextHopRouting(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_NextHopRouting.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_NextHopRouting.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_NextHopRouting.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_NextHopRouting.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
