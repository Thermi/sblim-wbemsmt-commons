/** 
 * CIM_OSPFVirtualInterfaceHelper.java
 *
...
public final class CIM_OSPFVirtualInterfaceHelper {

	public final static String CIM_CLASS_NAME = "CIM_OSPFVirtualInterface";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_OSPFVirtualInterface(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_OSPFVirtualInterface.Java_Package_List.size(); i++) {
						if (!((String)(CIM_OSPFVirtualInterface.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_OSPFVirtualInterface.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_OSPFVirtualInterface.Java_Package_List.setElementAt((String)(CIM_OSPFVirtualInterface.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_OSPFVirtualInterface.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_OSPFVirtualInterface.");
						resultArrayList.add(new CIM_OSPFVirtualInterface(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_OSPFVirtualInterface.");
						resultArrayList.add(new CIM_OSPFVirtualInterface(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_OSPFVirtualInterface.");
						resultArrayList.add(new CIM_OSPFVirtualInterface(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_OSPFVirtualInterface getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_OSPFVirtualInterfaceHelper.getInstance(cimClient, new CIM_OSPFVirtualInterface(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_OSPFVirtualInterface getInstance(CIMClient cimClient, CIM_OSPFVirtualInterface dataInstance) {
		...
		return CIM_OSPFVirtualInterfaceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_OSPFVirtualInterface getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_OSPFVirtualInterface dataInstance = null;
		try {
			dataInstance = new CIM_OSPFVirtualInterface(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_OSPFVirtualInterface createInstance(CIMClient cimClient, CIM_OSPFVirtualInterface dataInstance, boolean notifyDifferencies){ 
		...
		CIM_OSPFVirtualInterface newDataInstance = null;
		try {
			newDataInstance = CIM_OSPFVirtualInterfaceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_OSPFVirtualInterface modifyInstance(CIMClient cimClient, CIM_OSPFVirtualInterface dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_OSPFVirtualInterfaceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_OSPFVirtualInterface dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_OSPFVirtualInterface dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_OSPFVirtualInterface.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_OSPFVirtualInterface.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_OSPFVirtualInterface.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_OSPFVirtualInterface.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_OSPFVirtualInterface.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
