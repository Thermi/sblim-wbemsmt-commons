 ...
 *
 *
 * Description:  This is a concrete subclass of the MeterService class that represents the
 * metering of network traffic using a token bucket meter. Two types of token
 * bucket meters are defined using this class - a simple, two-parameter bucket
 * meter, and a multi-stage meter. A simple token bucket usually has two
 * parameters, an average token rate and a burst size, and has two conformance
 * levels: 'conforming' and 'non-conforming'. This class also defines an excess
 * burst size, which enables the meter to have three conformance levels
 * ('conforming', 'partially conforming', and 'non-conforming'). In this case,
 * packets that exceed the excess burst size are deemed non-conforming, while
 * packets that exceed the smaller burst size but are less than the excess burst
...
public class CIM_TokenBucketMeterService extends CIM_MeterService  {
	...
	public final static String CIM_CLASS_NAME = "CIM_TokenBucketMeterService"; //$NON-NLS-1$
	...
	*/
	...
	static {
		...
		for (int i = 0; i < CIM_MeterService.Java_Package_List.size(); i++) {
			if (((String)CIM_MeterService.Java_Package_List.elementAt(i)).equals("org.sblim.wbemsmt.schema.cim_2_14")){
				continue;
			}
