/** 
 * PRS_ActivityTransactionHelper.java
 *
...
public final class PRS_ActivityTransactionHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new PRS_ActivityTransaction(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<PRS_ActivityTransaction.Java_Package_List.size(); i++) {
						if (!((String)(PRS_ActivityTransaction.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(PRS_ActivityTransaction.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							PRS_ActivityTransaction.Java_Package_List.setElementAt((String)(PRS_ActivityTransaction.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (PRS_ActivityTransaction.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_ActivityTransaction.");
						resultArrayList.add(new PRS_ActivityTransaction(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_ActivityTransaction.");
						resultArrayList.add(new PRS_ActivityTransaction(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_ActivityTransaction.");
						resultArrayList.add(new PRS_ActivityTransaction(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_ActivityTransaction getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_ActivityTransactionHelper.getInstance(cimClient, new PRS_ActivityTransaction(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_ActivityTransaction getInstance(CIMClient cimClient, PRS_ActivityTransaction dataInstance) {
		...
		return PRS_ActivityTransactionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_ActivityTransaction getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_ActivityTransaction dataInstance = null;
		try {
			dataInstance = new PRS_ActivityTransaction(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_ActivityTransaction createInstance(CIMClient cimClient, PRS_ActivityTransaction dataInstance, boolean notifyDifferencies){ 
		...
		PRS_ActivityTransaction newDataInstance = null;
		try {
			newDataInstance = PRS_ActivityTransactionHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_ActivityTransaction modifyInstance(CIMClient cimClient, PRS_ActivityTransaction dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_ActivityTransactionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_ActivityTransaction dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_ActivityTransaction dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_ActivityTransaction.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_ActivityTransaction.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_ActivityTransaction.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_ActivityTransaction.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_ActivityTransaction.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_PRS_Activity(PRS_Activity PRS_Activity) {
		return true;
	}

	public static boolean isValid_PRS_Transaction(PRS_Transaction PRS_Transaction) {
		return true;
	}
