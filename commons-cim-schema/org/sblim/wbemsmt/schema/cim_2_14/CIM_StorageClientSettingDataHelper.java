/** 
 * CIM_StorageClientSettingDataHelper.java
 *
...
public final class CIM_StorageClientSettingDataHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_StorageClientSettingData(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_StorageClientSettingData.Java_Package_List.size(); i++) {
						if (!((String)(CIM_StorageClientSettingData.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_StorageClientSettingData.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_StorageClientSettingData.Java_Package_List.setElementAt((String)(CIM_StorageClientSettingData.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_StorageClientSettingData.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_StorageClientSettingData.");
						resultArrayList.add(new CIM_StorageClientSettingData(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_StorageClientSettingData.");
						resultArrayList.add(new CIM_StorageClientSettingData(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_StorageClientSettingData.");
						resultArrayList.add(new CIM_StorageClientSettingData(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_StorageClientSettingData getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_StorageClientSettingDataHelper.getInstance(cimClient, new CIM_StorageClientSettingData(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_StorageClientSettingData getInstance(CIMClient cimClient, CIM_StorageClientSettingData dataInstance) {
		...
		return CIM_StorageClientSettingDataHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_StorageClientSettingData getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_StorageClientSettingData dataInstance = null;
		try {
			dataInstance = new CIM_StorageClientSettingData(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_StorageClientSettingData createInstance(CIMClient cimClient, CIM_StorageClientSettingData dataInstance, boolean notifyDifferencies){ 
		...
		CIM_StorageClientSettingData newDataInstance = null;
		try {
			newDataInstance = CIM_StorageClientSettingDataHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_StorageClientSettingData modifyInstance(CIMClient cimClient, CIM_StorageClientSettingData dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_StorageClientSettingDataHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_StorageClientSettingData dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_StorageClientSettingData dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_StorageClientSettingData.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_StorageClientSettingData.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_StorageClientSettingData.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_StorageClientSettingData.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_StorageClientSettingData.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ClientTypes(UnsignedInt16[] ClientTypes) {
		return true;
	}

	public static boolean isValid_OtherClientTypeDescriptions(String[] OtherClientTypeDescriptions) {
		return true;
	}
