...
public final class CIM_StorageExtentHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_StorageExtent.Java_Package_List.size(); i++) {
						if (!((String)(CIM_StorageExtent.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_StorageExtent.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_StorageExtent.Java_Package_List.setElementAt((String)(CIM_StorageExtent.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_StorageExtent.");
						resultArrayList.add(new CIM_StorageExtent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_StorageExtent.");
						resultArrayList.add(new CIM_StorageExtent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_StorageExtent.");
						resultArrayList.add(new CIM_StorageExtent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_StorageExtent getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_StorageExtentHelper.getInstance(cimClient, new CIM_StorageExtent(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_StorageExtent getInstance(CIMClient cimClient, CIM_StorageExtent dataInstance) {
		...
		return CIM_StorageExtentHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_StorageExtent getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_StorageExtent dataInstance = null;
		try {
			dataInstance = new CIM_StorageExtent(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_StorageExtent createInstance(CIMClient cimClient, CIM_StorageExtent dataInstance, boolean notifyDifferencies){ 
		...
		CIM_StorageExtent newDataInstance = null;
		try {
			newDataInstance = CIM_StorageExtentHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_StorageExtent modifyInstance(CIMClient cimClient, CIM_StorageExtent dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_StorageExtentHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_StorageExtent dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_StorageExtent dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_StorageExtent.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_StorageExtent.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_StorageExtent.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_StorageExtent.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_StorageExtent.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Access(UnsignedInt16 Access) {
		return true;
	}
	...
	public static boolean isValid_ConsumableBlocks(UnsignedInt64 ConsumableBlocks) {
		return true;
	}

	public static boolean isValid_DataOrganization(UnsignedInt16 DataOrganization) {
		return true;
	}

	public static boolean isValid_DataRedundancy(UnsignedInt16 DataRedundancy) {
		return true;
	}

	public static boolean isValid_DeltaReservation(UnsignedInt8 DeltaReservation) {
		return true;
	}
	...
	public static boolean isValid_ExtentStatus(UnsignedInt16[] ExtentStatus) {
		return true;
	}

	public static boolean isValid_IsBasedOnUnderlyingRedundancy(Boolean IsBasedOnUnderlyingRedundancy) {
		return true;
	}
	...
	public static boolean isValid_PackageRedundancy(UnsignedInt16 PackageRedundancy) {
		return true;
	}

	public static boolean isValid_Primordial(Boolean Primordial) {
		return true;
	}

	public static boolean isValid_Purpose(String Purpose) {
		return true;
	}

	public static boolean isValid_SequentialAccess(Boolean SequentialAccess) {
		return true;
	}
