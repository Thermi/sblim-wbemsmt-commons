...
public final class CIM_USBHubHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_USBHub.");
						resultArrayList.add(new CIM_USBHub(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_USBHub.");
						resultArrayList.add(new CIM_USBHub(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_USBHub.");
						resultArrayList.add(new CIM_USBHub(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_USBHub getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_USBHubHelper.getInstance(cimClient, new CIM_USBHub(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_USBHub getInstance(CIMClient cimClient, CIM_USBHub dataInstance) {
		...
		return CIM_USBHubHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_USBHub getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_USBHub dataInstance = null;
		try {
			dataInstance = new CIM_USBHub(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_USBHub createInstance(CIMClient cimClient, CIM_USBHub dataInstance, boolean notifyDifferencies){ 
		...
		CIM_USBHub newDataInstance = null;
		try {
			newDataInstance = CIM_USBHubHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_USBHub modifyInstance(CIMClient cimClient, CIM_USBHub dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_USBHubHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_USBHub dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_USBHub dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_USBHub.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_USBHub.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_USBHub.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_USBHub.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_USBHub.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_GangSwitched(Boolean GangSwitched) {
		return true;
	}

	public static boolean isValid_NumberOfPorts(UnsignedInt8 NumberOfPorts) {
		return true;
	}
