/** 
 * CIM_OrganizationalEntityHelper.java
 *
...
public final class CIM_OrganizationalEntityHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_OrganizationalEntity(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_OrganizationalEntity.");
						resultArrayList.add(new CIM_OrganizationalEntity(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_OrganizationalEntity.");
						resultArrayList.add(new CIM_OrganizationalEntity(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_OrganizationalEntity.");
						resultArrayList.add(new CIM_OrganizationalEntity(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_OrganizationalEntity getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_OrganizationalEntityHelper.getInstance(cimClient, new CIM_OrganizationalEntity(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_OrganizationalEntity getInstance(CIMClient cimClient, CIM_OrganizationalEntity dataInstance) {
		...
		return CIM_OrganizationalEntityHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_OrganizationalEntity getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_OrganizationalEntity dataInstance = null;
		try {
			dataInstance = new CIM_OrganizationalEntity(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_OrganizationalEntity.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_OrganizationalEntity.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_OrganizationalEntity.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_OrganizationalEntity.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
