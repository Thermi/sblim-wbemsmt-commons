/** 
 * CIM_SecurityServiceUsesAccountHelper.java
 *
...
public final class CIM_SecurityServiceUsesAccountHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_SecurityServiceUsesAccount(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_SecurityServiceUsesAccount.Java_Package_List.size(); i++) {
						if (!((String)(CIM_SecurityServiceUsesAccount.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_SecurityServiceUsesAccount.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_SecurityServiceUsesAccount.Java_Package_List.setElementAt((String)(CIM_SecurityServiceUsesAccount.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_SecurityServiceUsesAccount.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_SecurityServiceUsesAccount.");
						resultArrayList.add(new CIM_SecurityServiceUsesAccount(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_SecurityServiceUsesAccount.");
						resultArrayList.add(new CIM_SecurityServiceUsesAccount(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_SecurityServiceUsesAccount.");
						resultArrayList.add(new CIM_SecurityServiceUsesAccount(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_SecurityServiceUsesAccount getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_SecurityServiceUsesAccountHelper.getInstance(cimClient, new CIM_SecurityServiceUsesAccount(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SecurityServiceUsesAccount getInstance(CIMClient cimClient, CIM_SecurityServiceUsesAccount dataInstance) {
		...
		return CIM_SecurityServiceUsesAccountHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SecurityServiceUsesAccount getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_SecurityServiceUsesAccount dataInstance = null;
		try {
			dataInstance = new CIM_SecurityServiceUsesAccount(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_SecurityServiceUsesAccount createInstance(CIMClient cimClient, CIM_SecurityServiceUsesAccount dataInstance, boolean notifyDifferencies){ 
		...
		CIM_SecurityServiceUsesAccount newDataInstance = null;
		try {
			newDataInstance = CIM_SecurityServiceUsesAccountHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_SecurityServiceUsesAccount modifyInstance(CIMClient cimClient, CIM_SecurityServiceUsesAccount dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_SecurityServiceUsesAccountHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_SecurityServiceUsesAccount dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_SecurityServiceUsesAccount dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_SecurityServiceUsesAccount.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_SecurityServiceUsesAccount.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_SecurityServiceUsesAccount.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_SecurityServiceUsesAccount.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_SecurityServiceUsesAccount.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CIM_Account(CIM_Account CIM_Account) {
		return true;
	}

	public static boolean isValid_CIM_SecurityService(CIM_SecurityService CIM_SecurityService) {
		return true;
	}
