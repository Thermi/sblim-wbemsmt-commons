...
public final class CIM_PhysicalLinkHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_PhysicalLink.Java_Package_List.size(); i++) {
						if (!((String)(CIM_PhysicalLink.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_PhysicalLink.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_PhysicalLink.Java_Package_List.setElementAt((String)(CIM_PhysicalLink.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_PhysicalLink.");
						resultArrayList.add(new CIM_PhysicalLink(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_PhysicalLink.");
						resultArrayList.add(new CIM_PhysicalLink(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_PhysicalLink.");
						resultArrayList.add(new CIM_PhysicalLink(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_PhysicalLink getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_PhysicalLinkHelper.getInstance(cimClient, new CIM_PhysicalLink(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PhysicalLink getInstance(CIMClient cimClient, CIM_PhysicalLink dataInstance) {
		...
		return CIM_PhysicalLinkHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PhysicalLink getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_PhysicalLink dataInstance = null;
		try {
			dataInstance = new CIM_PhysicalLink(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_PhysicalLink createInstance(CIMClient cimClient, CIM_PhysicalLink dataInstance, boolean notifyDifferencies){ 
		...
		CIM_PhysicalLink newDataInstance = null;
		try {
			newDataInstance = CIM_PhysicalLinkHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_PhysicalLink modifyInstance(CIMClient cimClient, CIM_PhysicalLink dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_PhysicalLinkHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_PhysicalLink dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_PhysicalLink dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_PhysicalLink.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_PhysicalLink.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_PhysicalLink.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_PhysicalLink.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_PhysicalLink.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Length(Double Length) {
		return true;
	}

	public static boolean isValid_MaxLength(Double MaxLength) {
		return true;
	}
	...
	public static boolean isValid_Wired(Boolean Wired) {
		return true;
	}
