/** 
 * CIM_IPsecSAEndpointHelper.java
 *
...
public final class CIM_IPsecSAEndpointHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_IPsecSAEndpoint.Java_Package_List.size(); i++) {
						if (!((String)(CIM_IPsecSAEndpoint.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_IPsecSAEndpoint.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_IPsecSAEndpoint.Java_Package_List.setElementAt((String)(CIM_IPsecSAEndpoint.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_IPsecSAEndpoint.");
						resultArrayList.add(new CIM_IPsecSAEndpoint(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_IPsecSAEndpoint.");
						resultArrayList.add(new CIM_IPsecSAEndpoint(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_IPsecSAEndpoint.");
						resultArrayList.add(new CIM_IPsecSAEndpoint(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_IPsecSAEndpoint getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_IPsecSAEndpointHelper.getInstance(cimClient, new CIM_IPsecSAEndpoint(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_IPsecSAEndpoint getInstance(CIMClient cimClient, CIM_IPsecSAEndpoint dataInstance) {
		...
		return CIM_IPsecSAEndpointHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_IPsecSAEndpoint getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_IPsecSAEndpoint dataInstance = null;
		try {
			dataInstance = new CIM_IPsecSAEndpoint(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_IPsecSAEndpoint createInstance(CIMClient cimClient, CIM_IPsecSAEndpoint dataInstance, boolean notifyDifferencies){ 
		...
		CIM_IPsecSAEndpoint newDataInstance = null;
		try {
			newDataInstance = CIM_IPsecSAEndpointHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_IPsecSAEndpoint modifyInstance(CIMClient cimClient, CIM_IPsecSAEndpoint dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_IPsecSAEndpointHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_IPsecSAEndpoint dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_IPsecSAEndpoint dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_IPsecSAEndpoint.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_IPsecSAEndpoint.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_IPsecSAEndpoint.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_IPsecSAEndpoint.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_IPsecSAEndpoint.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_EncapsulationMode(UnsignedInt16 EncapsulationMode) {
		return true;
	}
	...
	public static boolean isValid_PFSInUse(Boolean PFSInUse) {
		return true;
	}
