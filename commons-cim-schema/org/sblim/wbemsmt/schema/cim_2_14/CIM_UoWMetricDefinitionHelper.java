/** 
 * CIM_UoWMetricDefinitionHelper.java
 *
...
public final class CIM_UoWMetricDefinitionHelper {

	public final static String CIM_CLASS_NAME = "CIM_UoWMetricDefinition";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_UoWMetricDefinition(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_UoWMetricDefinition.Java_Package_List.size(); i++) {
						if (!((String)(CIM_UoWMetricDefinition.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_UoWMetricDefinition.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_UoWMetricDefinition.Java_Package_List.setElementAt((String)(CIM_UoWMetricDefinition.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_UoWMetricDefinition.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_UoWMetricDefinition.");
						resultArrayList.add(new CIM_UoWMetricDefinition(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_UoWMetricDefinition.");
						resultArrayList.add(new CIM_UoWMetricDefinition(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_UoWMetricDefinition.");
						resultArrayList.add(new CIM_UoWMetricDefinition(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_UoWMetricDefinition getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_UoWMetricDefinitionHelper.getInstance(cimClient, new CIM_UoWMetricDefinition(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_UoWMetricDefinition getInstance(CIMClient cimClient, CIM_UoWMetricDefinition dataInstance) {
		...
		return CIM_UoWMetricDefinitionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_UoWMetricDefinition getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_UoWMetricDefinition dataInstance = null;
		try {
			dataInstance = new CIM_UoWMetricDefinition(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_UoWMetricDefinition createInstance(CIMClient cimClient, CIM_UoWMetricDefinition dataInstance, boolean notifyDifferencies){ 
		...
		CIM_UoWMetricDefinition newDataInstance = null;
		try {
			newDataInstance = CIM_UoWMetricDefinitionHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_UoWMetricDefinition modifyInstance(CIMClient cimClient, CIM_UoWMetricDefinition dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_UoWMetricDefinitionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_UoWMetricDefinition dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_UoWMetricDefinition dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_UoWMetricDefinition.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_UoWMetricDefinition.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_UoWMetricDefinition.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_UoWMetricDefinition.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_UoWMetricDefinition.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CIM_MetricDefinition(CIM_MetricDefinition CIM_MetricDefinition) {
		return true;
	}
