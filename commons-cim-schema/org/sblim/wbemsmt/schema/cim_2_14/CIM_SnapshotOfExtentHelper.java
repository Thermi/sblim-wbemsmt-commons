/** 
 * CIM_SnapshotOfExtentHelper.java
 *
...
public final class CIM_SnapshotOfExtentHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_SnapshotOfExtent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_SnapshotOfExtent.Java_Package_List.size(); i++) {
						if (!((String)(CIM_SnapshotOfExtent.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_SnapshotOfExtent.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_SnapshotOfExtent.Java_Package_List.setElementAt((String)(CIM_SnapshotOfExtent.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_SnapshotOfExtent.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_SnapshotOfExtent.");
						resultArrayList.add(new CIM_SnapshotOfExtent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_SnapshotOfExtent.");
						resultArrayList.add(new CIM_SnapshotOfExtent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_SnapshotOfExtent.");
						resultArrayList.add(new CIM_SnapshotOfExtent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_SnapshotOfExtent getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_SnapshotOfExtentHelper.getInstance(cimClient, new CIM_SnapshotOfExtent(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SnapshotOfExtent getInstance(CIMClient cimClient, CIM_SnapshotOfExtent dataInstance) {
		...
		return CIM_SnapshotOfExtentHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SnapshotOfExtent getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_SnapshotOfExtent dataInstance = null;
		try {
			dataInstance = new CIM_SnapshotOfExtent(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_SnapshotOfExtent createInstance(CIMClient cimClient, CIM_SnapshotOfExtent dataInstance, boolean notifyDifferencies){ 
		...
		CIM_SnapshotOfExtent newDataInstance = null;
		try {
			newDataInstance = CIM_SnapshotOfExtentHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_SnapshotOfExtent modifyInstance(CIMClient cimClient, CIM_SnapshotOfExtent dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_SnapshotOfExtentHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_SnapshotOfExtent dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_SnapshotOfExtent dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_SnapshotOfExtent.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_SnapshotOfExtent.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_SnapshotOfExtent.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_SnapshotOfExtent.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_SnapshotOfExtent.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_SnapshotType(UnsignedInt16 SnapshotType) {
		return true;
	}
	...
	public static boolean isValid_CIM_Snapshot(CIM_Snapshot CIM_Snapshot) {
		return true;
	}
