...
public final class CIM_ObjectManagerHelper {
	...
	*/	
	public final static CIM_ObjectManager getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ObjectManagerHelper.getInstance(cimClient, new CIM_ObjectManager(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ObjectManager getInstance(CIMClient cimClient, CIM_ObjectManager dataInstance) {
		...
		return CIM_ObjectManagerHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ObjectManager getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_ObjectManager(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_ObjectManager.getPackages();
	...
	*/	
	public final static CIM_ObjectManager createInstance(CIMClient cimClient, CIM_ObjectManager dataInstance, boolean notifyDifferencies){ 
		...
		CIM_ObjectManager newDataInstance = null;
		try {
			newDataInstance = CIM_ObjectManagerHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ObjectManager modifyInstance(CIMClient cimClient, CIM_ObjectManager dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_ObjectManagerHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_ObjectManager dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_ObjectManager dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_ObjectManager.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_ObjectManager.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_ObjectManager.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_ObjectManager.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_ObjectManager.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_GatherStatisticalData(Boolean GatherStatisticalData) {
		return true;
	}
