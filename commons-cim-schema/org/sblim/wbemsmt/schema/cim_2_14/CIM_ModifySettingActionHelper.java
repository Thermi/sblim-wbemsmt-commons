			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_ModifySettingAction.Java_Package_List.size(); i++) {
						if (!((String)(CIM_ModifySettingAction.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_ModifySettingAction.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_ModifySettingAction.Java_Package_List.setElementAt((String)(CIM_ModifySettingAction.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
