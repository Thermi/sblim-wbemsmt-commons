...
public final class CIM_SensorHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_Sensor.Java_Package_List.size(); i++) {
						if (!((String)(CIM_Sensor.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_Sensor.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_Sensor.Java_Package_List.setElementAt((String)(CIM_Sensor.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_Sensor.");
						resultArrayList.add(new CIM_Sensor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_Sensor.");
						resultArrayList.add(new CIM_Sensor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_Sensor.");
						resultArrayList.add(new CIM_Sensor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_Sensor getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_SensorHelper.getInstance(cimClient, new CIM_Sensor(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Sensor getInstance(CIMClient cimClient, CIM_Sensor dataInstance) {
		...
		return CIM_SensorHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Sensor getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_Sensor dataInstance = null;
		try {
			dataInstance = new CIM_Sensor(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Sensor.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Sensor.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Sensor.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Sensor.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CurrentState(String CurrentState) {
		return true;
	}

	public static boolean isValid_OtherSensorTypeDescription(String OtherSensorTypeDescription) {
		return true;
	}

	public static boolean isValid_PollingInterval(UnsignedInt64 PollingInterval) {
		return true;
	}

	public static boolean isValid_PossibleStates(String[] PossibleStates) {
		return true;
	}
