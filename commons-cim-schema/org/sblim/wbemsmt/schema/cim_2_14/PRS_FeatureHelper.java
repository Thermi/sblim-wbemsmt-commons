...
public final class PRS_FeatureHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Feature.");
						resultArrayList.add(new PRS_Feature(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Feature.");
						resultArrayList.add(new PRS_Feature(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Feature.");
						resultArrayList.add(new PRS_Feature(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Feature getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_FeatureHelper.getInstance(cimClient, new PRS_Feature(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Feature getInstance(CIMClient cimClient, PRS_Feature dataInstance) {
		...
		return PRS_FeatureHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Feature getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Feature dataInstance = null;
		try {
			dataInstance = new PRS_Feature(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Feature createInstance(CIMClient cimClient, PRS_Feature dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Feature newDataInstance = null;
		try {
			newDataInstance = PRS_FeatureHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Feature modifyInstance(CIMClient cimClient, PRS_Feature dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_FeatureHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Feature dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Feature dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Feature.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Feature.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Feature.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Feature.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Feature.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_DataType(UnsignedInt8 DataType) {
		return true;
	}

	public static boolean isValid_Prompt(String Prompt) {
		return true;
	}

	public static boolean isValid_ValidValues(String ValidValues) {
		return true;
	}
