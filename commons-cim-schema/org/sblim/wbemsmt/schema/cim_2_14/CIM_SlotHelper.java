...
public final class CIM_SlotHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_Slot.Java_Package_List.size(); i++) {
						if (!((String)(CIM_Slot.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_Slot.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_Slot.Java_Package_List.setElementAt((String)(CIM_Slot.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_Slot.");
						resultArrayList.add(new CIM_Slot(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_Slot.");
						resultArrayList.add(new CIM_Slot(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_Slot.");
						resultArrayList.add(new CIM_Slot(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_Slot getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_SlotHelper.getInstance(cimClient, new CIM_Slot(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Slot getInstance(CIMClient cimClient, CIM_Slot dataInstance) {
		...
		return CIM_SlotHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Slot getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_Slot dataInstance = null;
		try {
			dataInstance = new CIM_Slot(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Slot createInstance(CIMClient cimClient, CIM_Slot dataInstance, boolean notifyDifferencies){ 
		...
		CIM_Slot newDataInstance = null;
		try {
			newDataInstance = CIM_SlotHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Slot modifyInstance(CIMClient cimClient, CIM_Slot dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_SlotHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_Slot dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_Slot dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_Slot.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Slot.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Slot.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Slot.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Slot.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_HeightAllowed(Float HeightAllowed) {
		return true;
	}

	public static boolean isValid_LengthAllowed(Float LengthAllowed) {
		return true;
	}

	public static boolean isValid_MaxDataWidth(UnsignedInt16 MaxDataWidth) {
		return true;
	}

	public static boolean isValid_Number(UnsignedInt16 Number) {
		return true;
	}

	public static boolean isValid_OpenSwitch(Boolean OpenSwitch) {
		return true;
	}

	public static boolean isValid_Powered(Boolean Powered) {
		return true;
	}

	public static boolean isValid_PoweredOn(Boolean PoweredOn) {
		return true;
	}
	...
	public static boolean isValid_SpecialPurpose(Boolean SpecialPurpose) {
		return true;
	}

	public static boolean isValid_SupportsHotPlug(Boolean SupportsHotPlug) {
		return true;
	}

	public static boolean isValid_ThermalRating(UnsignedInt32 ThermalRating) {
		return true;
	}

	public static boolean isValid_VccMixedVoltageSupport(UnsignedInt16[] VccMixedVoltageSupport) {
		return true;
	}

	public static boolean isValid_VppMixedVoltageSupport(UnsignedInt16[] VppMixedVoltageSupport) {
		return true;
	}
