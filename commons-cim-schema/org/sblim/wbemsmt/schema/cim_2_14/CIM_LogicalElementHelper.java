...
public final class CIM_LogicalElementHelper {
	...
	*/	
	public final static CIM_LogicalElement getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_LogicalElementHelper.getInstance(cimClient, new CIM_LogicalElement(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LogicalElement getInstance(CIMClient cimClient, CIM_LogicalElement dataInstance) {
		...
		return CIM_LogicalElementHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LogicalElement getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_LogicalElement(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_LogicalElement.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_LogicalElement.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_LogicalElement.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_LogicalElement.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
