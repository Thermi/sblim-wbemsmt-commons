/** 
 * CIM_UnitOfWorkDefinitionHelper.java
 *
...
public final class CIM_UnitOfWorkDefinitionHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_UnitOfWorkDefinition.Java_Package_List.size(); i++) {
						if (!((String)(CIM_UnitOfWorkDefinition.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_UnitOfWorkDefinition.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_UnitOfWorkDefinition.Java_Package_List.setElementAt((String)(CIM_UnitOfWorkDefinition.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_UnitOfWorkDefinition.");
						resultArrayList.add(new CIM_UnitOfWorkDefinition(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_UnitOfWorkDefinition.");
						resultArrayList.add(new CIM_UnitOfWorkDefinition(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_UnitOfWorkDefinition.");
						resultArrayList.add(new CIM_UnitOfWorkDefinition(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_UnitOfWorkDefinition getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_UnitOfWorkDefinitionHelper.getInstance(cimClient, new CIM_UnitOfWorkDefinition(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_UnitOfWorkDefinition getInstance(CIMClient cimClient, CIM_UnitOfWorkDefinition dataInstance) {
		...
		return CIM_UnitOfWorkDefinitionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_UnitOfWorkDefinition getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_UnitOfWorkDefinition dataInstance = null;
		try {
			dataInstance = new CIM_UnitOfWorkDefinition(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_UnitOfWorkDefinition createInstance(CIMClient cimClient, CIM_UnitOfWorkDefinition dataInstance, boolean notifyDifferencies){ 
		...
		CIM_UnitOfWorkDefinition newDataInstance = null;
		try {
			newDataInstance = CIM_UnitOfWorkDefinitionHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_UnitOfWorkDefinition modifyInstance(CIMClient cimClient, CIM_UnitOfWorkDefinition dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_UnitOfWorkDefinitionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_UnitOfWorkDefinition dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_UnitOfWorkDefinition dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_UnitOfWorkDefinition.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_UnitOfWorkDefinition.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_UnitOfWorkDefinition.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_UnitOfWorkDefinition.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_UnitOfWorkDefinition.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Context(String Context) {
		return true;
	}
	...
	public static boolean isValid_TraceLevelTypeId(String TraceLevelTypeId) {
		return true;
	}
