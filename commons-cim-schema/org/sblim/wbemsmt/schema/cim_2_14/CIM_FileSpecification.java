 ...
 *
 *
 * Description:  FileSpecification identifies a file that is either to be on or off the system.
 * The file is to be located in the directory identified in FileName, or by the
 * CIM_Directory SpecificationFile association. When the Invoke methods are
 * executed, it is expected that they will use a combination of information to
 * check for file existence. Therefore, any of the properties with a NULL value
 * are not checked. So, if only the FileName and MD5Checksum properties have
 * values, they are the only ones considered by the Invoke methods.
 * 
 */
...
/**
 *  FileSpecification identifies a file that is either to be on or off the system.
 * The file is to be located in the directory identified in FileName, or by the
 * CIM_Directory SpecificationFile association. When the Invoke methods are
 * executed, it is expected that they will use a combination of information to
 * check for file existence. Therefore, any of the properties with a NULL value
...
public class CIM_FileSpecification extends CIM_Check  {
	...
	public final static String CIM_CLASS_NAME = "CIM_FileSpecification"; //$NON-NLS-1$
