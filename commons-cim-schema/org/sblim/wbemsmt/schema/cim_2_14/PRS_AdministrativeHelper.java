...
public final class PRS_AdministrativeHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<PRS_Administrative.Java_Package_List.size(); i++) {
						if (!((String)(PRS_Administrative.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(PRS_Administrative.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							PRS_Administrative.Java_Package_List.setElementAt((String)(PRS_Administrative.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Administrative.");
						resultArrayList.add(new PRS_Administrative(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Administrative.");
						resultArrayList.add(new PRS_Administrative(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Administrative.");
						resultArrayList.add(new PRS_Administrative(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Administrative getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_AdministrativeHelper.getInstance(cimClient, new PRS_Administrative(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Administrative getInstance(CIMClient cimClient, PRS_Administrative dataInstance) {
		...
		return PRS_AdministrativeHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Administrative getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Administrative dataInstance = null;
		try {
			dataInstance = new PRS_Administrative(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Administrative createInstance(CIMClient cimClient, PRS_Administrative dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Administrative newDataInstance = null;
		try {
			newDataInstance = PRS_AdministrativeHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Administrative modifyInstance(CIMClient cimClient, PRS_Administrative dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_AdministrativeHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Administrative dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Administrative dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Administrative.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Administrative.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Administrative.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Administrative.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Administrative.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ComplianceLevel(UnsignedInt16 ComplianceLevel) {
		return true;
	}

	public static boolean isValid_Copyright(String Copyright) {
		return true;
	}

	public static boolean isValid_Disclaimer(String Disclaimer) {
		return true;
	}

	public static boolean isValid_Distribution(UnsignedInt8 Distribution) {
		return true;
	}

	public static boolean isValid_DocumentStatus(UnsignedInt8 DocumentStatus) {
		return true;
	}

	public static boolean isValid_Language(String Language) {
		return true;
	}

	public static boolean isValid_Rights(String Rights) {
		return true;
	}
