...
public final class PRS_AttachmentHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<PRS_Attachment.Java_Package_List.size(); i++) {
						if (!((String)(PRS_Attachment.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(PRS_Attachment.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							PRS_Attachment.Java_Package_List.setElementAt((String)(PRS_Attachment.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Attachment.");
						resultArrayList.add(new PRS_Attachment(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Attachment.");
						resultArrayList.add(new PRS_Attachment(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Attachment.");
						resultArrayList.add(new PRS_Attachment(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Attachment getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_AttachmentHelper.getInstance(cimClient, new PRS_Attachment(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Attachment getInstance(CIMClient cimClient, PRS_Attachment dataInstance) {
		...
		return PRS_AttachmentHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Attachment getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Attachment dataInstance = null;
		try {
			dataInstance = new PRS_Attachment(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Attachment createInstance(CIMClient cimClient, PRS_Attachment dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Attachment newDataInstance = null;
		try {
			newDataInstance = PRS_AttachmentHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Attachment modifyInstance(CIMClient cimClient, PRS_Attachment dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_AttachmentHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Attachment dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Attachment dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Attachment.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Attachment.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Attachment.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Attachment.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Attachment.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AttachmentName(String AttachmentName) {
		return true;
	}

	public static boolean isValid_AttachmentObject(String AttachmentObject) {
		return true;
	}

	public static boolean isValid_AttachmentReference(String AttachmentReference) {
		return true;
	}

	public static boolean isValid_AttachmentSize(String AttachmentSize) {
		return true;
	}

	public static boolean isValid_AttachmentType(UnsignedInt8 AttachmentType) {
		return true;
	}

	public static boolean isValid_Format(String Format) {
		return true;
	}

	public static boolean isValid_Protocol(String Protocol) {
		return true;
	}
