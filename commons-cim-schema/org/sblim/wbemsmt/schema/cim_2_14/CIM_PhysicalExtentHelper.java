...
public final class CIM_PhysicalExtentHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_PhysicalExtent.Java_Package_List.size(); i++) {
						if (!((String)(CIM_PhysicalExtent.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_PhysicalExtent.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_PhysicalExtent.Java_Package_List.setElementAt((String)(CIM_PhysicalExtent.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_PhysicalExtent.");
						resultArrayList.add(new CIM_PhysicalExtent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_PhysicalExtent.");
						resultArrayList.add(new CIM_PhysicalExtent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_PhysicalExtent.");
						resultArrayList.add(new CIM_PhysicalExtent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_PhysicalExtent getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_PhysicalExtentHelper.getInstance(cimClient, new CIM_PhysicalExtent(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PhysicalExtent getInstance(CIMClient cimClient, CIM_PhysicalExtent dataInstance) {
		...
		return CIM_PhysicalExtentHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PhysicalExtent getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_PhysicalExtent dataInstance = null;
		try {
			dataInstance = new CIM_PhysicalExtent(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_PhysicalExtent createInstance(CIMClient cimClient, CIM_PhysicalExtent dataInstance, boolean notifyDifferencies){ 
		...
		CIM_PhysicalExtent newDataInstance = null;
		try {
			newDataInstance = CIM_PhysicalExtentHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_PhysicalExtent modifyInstance(CIMClient cimClient, CIM_PhysicalExtent dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_PhysicalExtentHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_PhysicalExtent dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_PhysicalExtent dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_PhysicalExtent.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_PhysicalExtent.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_PhysicalExtent.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_PhysicalExtent.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_PhysicalExtent.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
