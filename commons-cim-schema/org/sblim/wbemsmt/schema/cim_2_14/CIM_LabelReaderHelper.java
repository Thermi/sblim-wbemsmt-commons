...
public final class CIM_LabelReaderHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_LabelReader.Java_Package_List.size(); i++) {
						if (!((String)(CIM_LabelReader.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_LabelReader.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_LabelReader.Java_Package_List.setElementAt((String)(CIM_LabelReader.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_LabelReader.");
						resultArrayList.add(new CIM_LabelReader(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_LabelReader.");
						resultArrayList.add(new CIM_LabelReader(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_LabelReader.");
						resultArrayList.add(new CIM_LabelReader(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_LabelReader getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_LabelReaderHelper.getInstance(cimClient, new CIM_LabelReader(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LabelReader getInstance(CIMClient cimClient, CIM_LabelReader dataInstance) {
		...
		return CIM_LabelReaderHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LabelReader getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_LabelReader dataInstance = null;
		try {
			dataInstance = new CIM_LabelReader(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_LabelReader createInstance(CIMClient cimClient, CIM_LabelReader dataInstance, boolean notifyDifferencies){ 
		...
		CIM_LabelReader newDataInstance = null;
		try {
			newDataInstance = CIM_LabelReaderHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_LabelReader modifyInstance(CIMClient cimClient, CIM_LabelReader dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_LabelReaderHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_LabelReader dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_LabelReader dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_LabelReader.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_LabelReader.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_LabelReader.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_LabelReader.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_LabelReader.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_SupportedFormats(UnsignedInt16[] SupportedFormats) {
		return true;
	}

	public static boolean isValid_Technology(UnsignedInt16 Technology) {
		return true;
	}
