...
public final class CIM_ConcreteComponentHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_ConcreteComponent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_ConcreteComponent.Java_Package_List.size(); i++) {
						if (!((String)(CIM_ConcreteComponent.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_ConcreteComponent.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_ConcreteComponent.Java_Package_List.setElementAt((String)(CIM_ConcreteComponent.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_ConcreteComponent.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static CIM_ConcreteComponent getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ConcreteComponentHelper.getInstance(cimClient, new CIM_ConcreteComponent(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ConcreteComponent getInstance(CIMClient cimClient, CIM_ConcreteComponent dataInstance) {
		...
		return CIM_ConcreteComponentHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ConcreteComponent getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_ConcreteComponent(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ConcreteComponent createInstance(CIMClient cimClient, CIM_ConcreteComponent dataInstance, boolean notifyDifferencies){ 
		...
		CIM_ConcreteComponent newDataInstance = null;
		try {
			newDataInstance = CIM_ConcreteComponentHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ConcreteComponent modifyInstance(CIMClient cimClient, CIM_ConcreteComponent dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_ConcreteComponentHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_ConcreteComponent dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_ConcreteComponent dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_ConcreteComponent.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_ConcreteComponent.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_ConcreteComponent.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_ConcreteComponent.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_ConcreteComponent.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
