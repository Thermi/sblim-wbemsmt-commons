...
public final class CIM_ComponentHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_Component(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_Component.Java_Package_List.size(); i++) {
						if (!((String)(CIM_Component.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_Component.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_Component.Java_Package_List.setElementAt((String)(CIM_Component.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_Component.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static CIM_Component getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ComponentHelper.getInstance(cimClient, new CIM_Component(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Component getInstance(CIMClient cimClient, CIM_Component dataInstance) {
		...
		return CIM_ComponentHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Component getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_Component(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Component.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Component.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Component.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Component.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
