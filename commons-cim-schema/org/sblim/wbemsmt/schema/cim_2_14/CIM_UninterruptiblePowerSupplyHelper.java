/** 
 * CIM_UninterruptiblePowerSupplyHelper.java
 *
...
public final class CIM_UninterruptiblePowerSupplyHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_UninterruptiblePowerSupply(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_UninterruptiblePowerSupply.Java_Package_List.size(); i++) {
						if (!((String)(CIM_UninterruptiblePowerSupply.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_UninterruptiblePowerSupply.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_UninterruptiblePowerSupply.Java_Package_List.setElementAt((String)(CIM_UninterruptiblePowerSupply.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_UninterruptiblePowerSupply.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_UninterruptiblePowerSupply.");
						resultArrayList.add(new CIM_UninterruptiblePowerSupply(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_UninterruptiblePowerSupply.");
						resultArrayList.add(new CIM_UninterruptiblePowerSupply(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_UninterruptiblePowerSupply.");
						resultArrayList.add(new CIM_UninterruptiblePowerSupply(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_UninterruptiblePowerSupply getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_UninterruptiblePowerSupplyHelper.getInstance(cimClient, new CIM_UninterruptiblePowerSupply(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_UninterruptiblePowerSupply getInstance(CIMClient cimClient, CIM_UninterruptiblePowerSupply dataInstance) {
		...
		return CIM_UninterruptiblePowerSupplyHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_UninterruptiblePowerSupply getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_UninterruptiblePowerSupply dataInstance = null;
		try {
			dataInstance = new CIM_UninterruptiblePowerSupply(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_UninterruptiblePowerSupply createInstance(CIMClient cimClient, CIM_UninterruptiblePowerSupply dataInstance, boolean notifyDifferencies){ 
		...
		CIM_UninterruptiblePowerSupply newDataInstance = null;
		try {
			newDataInstance = CIM_UninterruptiblePowerSupplyHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_UninterruptiblePowerSupply modifyInstance(CIMClient cimClient, CIM_UninterruptiblePowerSupply dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_UninterruptiblePowerSupplyHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_UninterruptiblePowerSupply dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_UninterruptiblePowerSupply dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_UninterruptiblePowerSupply.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_UninterruptiblePowerSupply.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_UninterruptiblePowerSupply.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_UninterruptiblePowerSupply.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_UninterruptiblePowerSupply.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_EstimatedChargeRemaining(UnsignedInt16 EstimatedChargeRemaining) {
		return true;
	}

	public static boolean isValid_EstimatedRunTime(UnsignedInt32 EstimatedRunTime) {
		return true;
	}

	public static boolean isValid_Range1InputVoltageHigh(UnsignedInt32 Range1InputVoltageHigh) {
		return true;
	}

	public static boolean isValid_Range1InputVoltageLow(UnsignedInt32 Range1InputVoltageLow) {
		return true;
	}

	public static boolean isValid_Range2InputVoltageHigh(UnsignedInt32 Range2InputVoltageHigh) {
		return true;
	}

	public static boolean isValid_Range2InputVoltageLow(UnsignedInt32 Range2InputVoltageLow) {
		return true;
	}

	public static boolean isValid_RemainingCapacityStatus(UnsignedInt16 RemainingCapacityStatus) {
		return true;
	}

	public static boolean isValid_TimeOnBackup(UnsignedInt32 TimeOnBackup) {
		return true;
	}
