/** 
 * CIM_FibrePortEventCountersHelper.java
 *
...
public final class CIM_FibrePortEventCountersHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_FibrePortEventCounters(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_FibrePortEventCounters.Java_Package_List.size(); i++) {
						if (!((String)(CIM_FibrePortEventCounters.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_FibrePortEventCounters.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_FibrePortEventCounters.Java_Package_List.setElementAt((String)(CIM_FibrePortEventCounters.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_FibrePortEventCounters.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_FibrePortEventCounters.");
						resultArrayList.add(new CIM_FibrePortEventCounters(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_FibrePortEventCounters.");
						resultArrayList.add(new CIM_FibrePortEventCounters(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_FibrePortEventCounters.");
						resultArrayList.add(new CIM_FibrePortEventCounters(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_FibrePortEventCounters getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_FibrePortEventCountersHelper.getInstance(cimClient, new CIM_FibrePortEventCounters(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_FibrePortEventCounters getInstance(CIMClient cimClient, CIM_FibrePortEventCounters dataInstance) {
		...
		return CIM_FibrePortEventCountersHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_FibrePortEventCounters getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_FibrePortEventCounters dataInstance = null;
		try {
			dataInstance = new CIM_FibrePortEventCounters(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_FibrePortEventCounters createInstance(CIMClient cimClient, CIM_FibrePortEventCounters dataInstance, boolean notifyDifferencies){ 
		...
		CIM_FibrePortEventCounters newDataInstance = null;
		try {
			newDataInstance = CIM_FibrePortEventCountersHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_FibrePortEventCounters modifyInstance(CIMClient cimClient, CIM_FibrePortEventCounters dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_FibrePortEventCountersHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_FibrePortEventCounters dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_FibrePortEventCounters dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_FibrePortEventCounters.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_FibrePortEventCounters.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_FibrePortEventCounters.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_FibrePortEventCounters.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_FibrePortEventCounters.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_EOFAbortsReceived(UnsignedInt64 EOFAbortsReceived) {
		return true;
	}

	public static boolean isValid_EOFAbortsTransmitted(UnsignedInt64 EOFAbortsTransmitted) {
		return true;
	}

	public static boolean isValid_PLOGIsReceived(UnsignedInt64 PLOGIsReceived) {
		return true;
	}

	public static boolean isValid_PLOGIsRejected(UnsignedInt64 PLOGIsRejected) {
		return true;
	}

	public static boolean isValid_PLOGIsSent(UnsignedInt64 PLOGIsSent) {
		return true;
	}

	public static boolean isValid_PLOGOsReceived(UnsignedInt64 PLOGOsReceived) {
		return true;
	}

	public static boolean isValid_PLOGOsRejected(UnsignedInt64 PLOGOsRejected) {
		return true;
	}

	public static boolean isValid_PLOGOsSent(UnsignedInt64 PLOGOsSent) {
		return true;
	}
