...
public final class CIM_PersonHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_Person.Java_Package_List.size(); i++) {
						if (!((String)(CIM_Person.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_Person.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_Person.Java_Package_List.setElementAt((String)(CIM_Person.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_Person.");
						resultArrayList.add(new CIM_Person(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_Person.");
						resultArrayList.add(new CIM_Person(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_Person.");
						resultArrayList.add(new CIM_Person(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_Person getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_PersonHelper.getInstance(cimClient, new CIM_Person(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Person getInstance(CIMClient cimClient, CIM_Person dataInstance) {
		...
		return CIM_PersonHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Person getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_Person dataInstance = null;
		try {
			dataInstance = new CIM_Person(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Person createInstance(CIMClient cimClient, CIM_Person dataInstance, boolean notifyDifferencies){ 
		...
		CIM_Person newDataInstance = null;
		try {
			newDataInstance = CIM_PersonHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Person modifyInstance(CIMClient cimClient, CIM_Person dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_PersonHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_Person dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_Person dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_Person.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Person.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Person.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Person.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Person.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
