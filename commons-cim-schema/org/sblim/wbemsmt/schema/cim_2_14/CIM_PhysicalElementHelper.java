/** 
 * CIM_PhysicalElementHelper.java
 *
...
public final class CIM_PhysicalElementHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_PhysicalElement.");
						resultArrayList.add(new CIM_PhysicalElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_PhysicalElement.");
						resultArrayList.add(new CIM_PhysicalElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_PhysicalElement.");
						resultArrayList.add(new CIM_PhysicalElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_PhysicalElement getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_PhysicalElementHelper.getInstance(cimClient, new CIM_PhysicalElement(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PhysicalElement getInstance(CIMClient cimClient, CIM_PhysicalElement dataInstance) {
		...
		return CIM_PhysicalElementHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PhysicalElement getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_PhysicalElement dataInstance = null;
		try {
			dataInstance = new CIM_PhysicalElement(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_PhysicalElement.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_PhysicalElement.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_PhysicalElement.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_PhysicalElement.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CanBeFRUed(Boolean CanBeFRUed) {
		return true;
	}
	...
	public static boolean isValid_ManufactureDate(Calendar ManufactureDate) {
		return true;
	}
	...
	public static boolean isValid_Model(String Model) {
		return true;
	}
	...
	public static boolean isValid_PartNumber(String PartNumber) {
		return true;
	}
	...
	public static boolean isValid_SKU(String SKU) {
		return true;
	}
	...
	public static boolean isValid_UserTracking(String UserTracking) {
		return true;
	}

	public static boolean isValid_VendorEquipmentType(String VendorEquipmentType) {
		return true;
	}
