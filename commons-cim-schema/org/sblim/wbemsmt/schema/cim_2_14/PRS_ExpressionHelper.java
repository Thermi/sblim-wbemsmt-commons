...
public final class PRS_ExpressionHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<PRS_Expression.Java_Package_List.size(); i++) {
						if (!((String)(PRS_Expression.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(PRS_Expression.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							PRS_Expression.Java_Package_List.setElementAt((String)(PRS_Expression.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Expression.");
						resultArrayList.add(new PRS_Expression(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Expression.");
						resultArrayList.add(new PRS_Expression(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Expression.");
						resultArrayList.add(new PRS_Expression(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Expression getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_ExpressionHelper.getInstance(cimClient, new PRS_Expression(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Expression getInstance(CIMClient cimClient, PRS_Expression dataInstance) {
		...
		return PRS_ExpressionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Expression getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Expression dataInstance = null;
		try {
			dataInstance = new PRS_Expression(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Expression createInstance(CIMClient cimClient, PRS_Expression dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Expression newDataInstance = null;
		try {
			newDataInstance = PRS_ExpressionHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Expression modifyInstance(CIMClient cimClient, PRS_Expression dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_ExpressionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Expression dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Expression dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Expression.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Expression.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Expression.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Expression.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Expression.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Relation(String Relation) {
		return true;
	}
