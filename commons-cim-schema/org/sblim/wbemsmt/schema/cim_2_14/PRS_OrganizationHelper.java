...
public final class PRS_OrganizationHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Organization.");
						resultArrayList.add(new PRS_Organization(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Organization.");
						resultArrayList.add(new PRS_Organization(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Organization.");
						resultArrayList.add(new PRS_Organization(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Organization getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_OrganizationHelper.getInstance(cimClient, new PRS_Organization(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Organization getInstance(CIMClient cimClient, PRS_Organization dataInstance) {
		...
		return PRS_OrganizationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Organization getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Organization dataInstance = null;
		try {
			dataInstance = new PRS_Organization(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Organization createInstance(CIMClient cimClient, PRS_Organization dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Organization newDataInstance = null;
		try {
			newDataInstance = PRS_OrganizationHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Organization modifyInstance(CIMClient cimClient, PRS_Organization dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_OrganizationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Organization dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Organization dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Organization.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Organization.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Organization.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Organization.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Organization.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Business(String Business) {
		return true;
	}
