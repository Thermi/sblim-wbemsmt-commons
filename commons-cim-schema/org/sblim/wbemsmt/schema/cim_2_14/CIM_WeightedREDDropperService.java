 ...
 *
 *
 * Description:  This class represents a logical entity that resides in the data forwarding path
 * of a network device. It describes the ability to drop network traffic using a
 * Weighted Random Early Detection (WRED) algorithm. Like RED, the purpose of
 * WRED is to avoid congestion (as opposed to managing congestion). This
 * modification of the basic RED algorithm enables packets belonging to
 * different traffic classes to be dropped at different queue depths. This
 * algorithm also enables discard to be done based on different information
 * contained in the packet header, such as IP Precedence, RSVP session
 * parameters, or even on other factors not directly encoded in the packet
 * header, such as the queue depth. Implementation of this class has shown that
 * weighting should not be a property of the class (but on an association to
 * it), and that there is insufficient data to process the drop metric. The
...
/**
 *  This class represents a logical entity that resides in the data forwarding path
 * of a network device. It describes the ability to drop network traffic using a
 * Weighted Random Early Detection (WRED) algorithm. Like RED, the purpose of
 * WRED is to avoid congestion (as opposed to managing congestion). This
 * modification of the basic RED algorithm enables packets belonging to
...
public class CIM_WeightedREDDropperService extends CIM_DropperService  {
	...
	public final static String CIM_CLASS_NAME = "CIM_WeightedREDDropperService"; //$NON-NLS-1$
