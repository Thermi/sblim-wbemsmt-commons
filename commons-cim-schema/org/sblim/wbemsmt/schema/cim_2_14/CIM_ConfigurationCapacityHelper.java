/** 
 * CIM_ConfigurationCapacityHelper.java
 *
...
public final class CIM_ConfigurationCapacityHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_ConfigurationCapacity(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_ConfigurationCapacity.Java_Package_List.size(); i++) {
						if (!((String)(CIM_ConfigurationCapacity.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_ConfigurationCapacity.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_ConfigurationCapacity.Java_Package_List.setElementAt((String)(CIM_ConfigurationCapacity.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_ConfigurationCapacity.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_ConfigurationCapacity.");
						resultArrayList.add(new CIM_ConfigurationCapacity(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_ConfigurationCapacity.");
						resultArrayList.add(new CIM_ConfigurationCapacity(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_ConfigurationCapacity.");
						resultArrayList.add(new CIM_ConfigurationCapacity(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_ConfigurationCapacity getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ConfigurationCapacityHelper.getInstance(cimClient, new CIM_ConfigurationCapacity(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ConfigurationCapacity getInstance(CIMClient cimClient, CIM_ConfigurationCapacity dataInstance) {
		...
		return CIM_ConfigurationCapacityHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ConfigurationCapacity getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_ConfigurationCapacity dataInstance = null;
		try {
			dataInstance = new CIM_ConfigurationCapacity(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ConfigurationCapacity createInstance(CIMClient cimClient, CIM_ConfigurationCapacity dataInstance, boolean notifyDifferencies){ 
		...
		CIM_ConfigurationCapacity newDataInstance = null;
		try {
			newDataInstance = CIM_ConfigurationCapacityHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ConfigurationCapacity modifyInstance(CIMClient cimClient, CIM_ConfigurationCapacity dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_ConfigurationCapacityHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_ConfigurationCapacity dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_ConfigurationCapacity dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_ConfigurationCapacity.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_ConfigurationCapacity.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_ConfigurationCapacity.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_ConfigurationCapacity.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_ConfigurationCapacity.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Increment(UnsignedInt32 Increment) {
		return true;
	}

	public static boolean isValid_MaximumCapacity(UnsignedInt64 MaximumCapacity) {
		return true;
	}

	public static boolean isValid_MinimumCapacity(UnsignedInt64 MinimumCapacity) {
		return true;
	}
	...
	public static boolean isValid_ObjectType(UnsignedInt16 ObjectType) {
		return true;
	}
