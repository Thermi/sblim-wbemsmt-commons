...
public final class CIM_ServiceAccessBySAPHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_ServiceAccessBySAP(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_ServiceAccessBySAP.Java_Package_List.size(); i++) {
						if (!((String)(CIM_ServiceAccessBySAP.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_ServiceAccessBySAP.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_ServiceAccessBySAP.Java_Package_List.setElementAt((String)(CIM_ServiceAccessBySAP.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_ServiceAccessBySAP.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static CIM_ServiceAccessBySAP getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ServiceAccessBySAPHelper.getInstance(cimClient, new CIM_ServiceAccessBySAP(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ServiceAccessBySAP getInstance(CIMClient cimClient, CIM_ServiceAccessBySAP dataInstance) {
		...
		return CIM_ServiceAccessBySAPHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ServiceAccessBySAP getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_ServiceAccessBySAP(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ServiceAccessBySAP createInstance(CIMClient cimClient, CIM_ServiceAccessBySAP dataInstance, boolean notifyDifferencies){ 
		...
		CIM_ServiceAccessBySAP newDataInstance = null;
		try {
			newDataInstance = CIM_ServiceAccessBySAPHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ServiceAccessBySAP modifyInstance(CIMClient cimClient, CIM_ServiceAccessBySAP dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_ServiceAccessBySAPHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_ServiceAccessBySAP dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_ServiceAccessBySAP dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_ServiceAccessBySAP.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_ServiceAccessBySAP.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_ServiceAccessBySAP.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_ServiceAccessBySAP.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_ServiceAccessBySAP.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
