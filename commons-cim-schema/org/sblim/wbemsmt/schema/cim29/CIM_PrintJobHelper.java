...
public final class CIM_PrintJobHelper {

	public final static String CIM_CLASS_NAME = "CIM_PrintJob";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_PrintJob.Java_Package_List.size(); i++) {
						if (!((String)(CIM_PrintJob.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_PrintJob.Java_Package_List.get(i))).endsWith(".")) {
							CIM_PrintJob.Java_Package_List.setElementAt((String)(CIM_PrintJob.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_PrintJob.");
						resultArrayList.add(new CIM_PrintJob(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_PrintJob.");
						resultArrayList.add(new CIM_PrintJob(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_PrintJob.");
						resultArrayList.add(new CIM_PrintJob(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_PrintJob getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_PrintJobHelper.getInstance(cimClient, new CIM_PrintJob(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PrintJob getInstance(CIMClient cimClient, CIM_PrintJob dataInstance) {
		...
		return CIM_PrintJobHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PrintJob getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_PrintJob dataInstance = null;
		try {
			dataInstance = new CIM_PrintJob(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_PrintJob createInstance(CIMClient cimClient, CIM_PrintJob dataInstance, boolean notifyDifferencies){ 
		...
		CIM_PrintJob newDataInstance = null;
		try {
			newDataInstance = CIM_PrintJobHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_PrintJob modifyInstance(CIMClient cimClient, CIM_PrintJob dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_PrintJobHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_PrintJob dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_PrintJob dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_PrintJob.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_PrintJob.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_PrintJob.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_PrintJob.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_PrintJob.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CharSet(String CharSet) {
		return true;
	}

	public static boolean isValid_Copies(UnsignedInt32 Copies) {
		return true;
	}

	public static boolean isValid_Finishing(UnsignedInt16[] Finishing) {
		return true;
	}
	...
	public static boolean isValid_JobID(String JobID) {
		return true;
	}

	public static boolean isValid_JobOrigination(String JobOrigination) {
		return true;
	}

	public static boolean isValid_JobSize(UnsignedInt32 JobSize) {
		return true;
	}

	public static boolean isValid_JobStatus(String JobStatus) {
		return true;
	}

	public static boolean isValid_Language(UnsignedInt16 Language) {
		return true;
	}

	public static boolean isValid_MimeTypes(String[] MimeTypes) {
		return true;
	}

	public static boolean isValid_NaturalLanguage(String NaturalLanguage) {
		return true;
	}

	public static boolean isValid_NumberUp(UnsignedInt32 NumberUp) {
		return true;
	}

	public static boolean isValid_PrintJobStatus(UnsignedInt16 PrintJobStatus) {
		return true;
	}

	public static boolean isValid_QueueCreationClassName(String QueueCreationClassName) {
		return true;
	}

	public static boolean isValid_QueueName(String QueueName) {
		return true;
	}

	public static boolean isValid_RequiredJobSheets(String[] RequiredJobSheets) {
		return true;
	}

	public static boolean isValid_RequiredPaperType(String RequiredPaperType) {
		return true;
	}

	public static boolean isValid_SchedulingInformation(String SchedulingInformation) {
		return true;
	}
	...
	public static boolean isValid_TimeCompleted(Calendar TimeCompleted) {
		return true;
	}
