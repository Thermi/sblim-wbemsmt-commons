/** 
 * CIM_ClusteringServiceHelper.java
 *
...
public final class CIM_ClusteringServiceHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_ClusteringService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = CIM_ClusteringServiceHelper.findClass(cimClient, cimInstance);
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_ClusteringService.");
						resultArrayList.add(new CIM_ClusteringService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_ClusteringService.");
						resultArrayList.add(new CIM_ClusteringService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_ClusteringService.");
						resultArrayList.add(new CIM_ClusteringService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_ClusteringService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ClusteringServiceHelper.getInstance(cimClient, new CIM_ClusteringService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ClusteringService getInstance(CIMClient cimClient, CIM_ClusteringService dataInstance) {
		...
		return CIM_ClusteringServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ClusteringService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_ClusteringService dataInstance = null;
		try {
			dataInstance = new CIM_ClusteringService(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_ClusteringService.getPackages();
	...
	*/	
	public final static CIM_ClusteringService createInstance(CIMClient cimClient, CIM_ClusteringService dataInstance, boolean notifyDifferencies){ 
		...
		CIM_ClusteringService newDataInstance = null;
		try {
			newDataInstance = CIM_ClusteringServiceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ClusteringService modifyInstance(CIMClient cimClient, CIM_ClusteringService dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_ClusteringServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_ClusteringService dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_ClusteringService dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_ClusteringService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_ClusteringService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_ClusteringService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_ClusteringService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_ClusteringService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
