...
public final class CIM_ProcessorHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_Processor.Java_Package_List.size(); i++) {
						if (!((String)(CIM_Processor.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_Processor.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_Processor.Java_Package_List.setElementAt((String)(CIM_Processor.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_Processor.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_Processor.");
						resultArrayList.add(new CIM_Processor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_Processor.");
						resultArrayList.add(new CIM_Processor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_Processor.");
						resultArrayList.add(new CIM_Processor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_Processor getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ProcessorHelper.getInstance(cimClient, new CIM_Processor(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Processor getInstance(CIMClient cimClient, CIM_Processor dataInstance) {
		...
		return CIM_ProcessorHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Processor getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_Processor dataInstance = null;
		try {
			dataInstance = new CIM_Processor(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Processor createInstance(CIMClient cimClient, CIM_Processor dataInstance, boolean notifyDifferencies){ 
		...
		CIM_Processor newDataInstance = null;
		try {
			newDataInstance = CIM_ProcessorHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Processor modifyInstance(CIMClient cimClient, CIM_Processor dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_ProcessorHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_Processor dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_Processor dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_Processor.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Processor.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Processor.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Processor.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Processor.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AddressWidth(UnsignedInt16 AddressWidth) {
		return true;
	}

	public static boolean isValid_CPUStatus(UnsignedInt16 CPUStatus) {
		return true;
	}

	public static boolean isValid_CurrentClockSpeed(UnsignedInt32 CurrentClockSpeed) {
		return true;
	}

	public static boolean isValid_DataWidth(UnsignedInt16 DataWidth) {
		return true;
	}

	public static boolean isValid_Family(UnsignedInt16 Family) {
		return true;
	}

	public static boolean isValid_LoadPercentage(UnsignedInt16 LoadPercentage) {
		return true;
	}

	public static boolean isValid_MaxClockSpeed(UnsignedInt32 MaxClockSpeed) {
		return true;
	}

	public static boolean isValid_OtherFamilyDescription(String OtherFamilyDescription) {
		return true;
	}

	public static boolean isValid_Role(String Role) {
		return true;
	}

	public static boolean isValid_Stepping(String Stepping) {
		return true;
	}

	public static boolean isValid_UniqueID(String UniqueID) {
		return true;
	}

	public static boolean isValid_UpgradeMethod(UnsignedInt16 UpgradeMethod) {
		return true;
	}
