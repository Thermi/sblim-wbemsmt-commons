...
public final class CIM_ClusterHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_Cluster.");
						resultArrayList.add(new CIM_Cluster(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_Cluster.");
						resultArrayList.add(new CIM_Cluster(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_Cluster.");
						resultArrayList.add(new CIM_Cluster(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_Cluster getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ClusterHelper.getInstance(cimClient, new CIM_Cluster(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Cluster getInstance(CIMClient cimClient, CIM_Cluster dataInstance) {
		...
		return CIM_ClusterHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Cluster getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_Cluster dataInstance = null;
		try {
			dataInstance = new CIM_Cluster(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_Cluster.getPackages();
	...
	*/	
	public final static CIM_Cluster createInstance(CIMClient cimClient, CIM_Cluster dataInstance, boolean notifyDifferencies){ 
		...
		CIM_Cluster newDataInstance = null;
		try {
			newDataInstance = CIM_ClusterHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Cluster modifyInstance(CIMClient cimClient, CIM_Cluster dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_ClusterHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_Cluster dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_Cluster dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_Cluster.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Cluster.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Cluster.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Cluster.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Cluster.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ClusterState(UnsignedInt16 ClusterState) {
		return true;
	}

	public static boolean isValid_Interconnect(String Interconnect) {
		return true;
	}

	public static boolean isValid_InterconnectAddress(String InterconnectAddress) {
		return true;
	}

	public static boolean isValid_MaxNumberOfNodes(UnsignedInt32 MaxNumberOfNodes) {
		return true;
	}

	public static boolean isValid_Types(UnsignedInt16[] Types) {
		return true;
	}
