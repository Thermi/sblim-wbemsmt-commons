...
public final class CIM_LogicalFileHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_LogicalFile.Java_Package_List.size(); i++) {
						if (!((String)(CIM_LogicalFile.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_LogicalFile.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_LogicalFile.Java_Package_List.setElementAt((String)(CIM_LogicalFile.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_LogicalFile.");
						resultArrayList.add(new CIM_LogicalFile(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_LogicalFile.");
						resultArrayList.add(new CIM_LogicalFile(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_LogicalFile.");
						resultArrayList.add(new CIM_LogicalFile(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_LogicalFile getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_LogicalFileHelper.getInstance(cimClient, new CIM_LogicalFile(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LogicalFile getInstance(CIMClient cimClient, CIM_LogicalFile dataInstance) {
		...
		return CIM_LogicalFileHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LogicalFile getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_LogicalFile dataInstance = null;
		try {
			dataInstance = new CIM_LogicalFile(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_LogicalFile createInstance(CIMClient cimClient, CIM_LogicalFile dataInstance, boolean notifyDifferencies){ 
		...
		CIM_LogicalFile newDataInstance = null;
		try {
			newDataInstance = CIM_LogicalFileHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_LogicalFile modifyInstance(CIMClient cimClient, CIM_LogicalFile dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_LogicalFileHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_LogicalFile dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_LogicalFile dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_LogicalFile.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_LogicalFile.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_LogicalFile.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_LogicalFile.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_LogicalFile.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CreationDate(Calendar CreationDate) {
		return true;
	}

	public static boolean isValid_EncryptionMethod(String EncryptionMethod) {
		return true;
	}

	public static boolean isValid_Executable(Boolean Executable) {
		return true;
	}
	...
	public static boolean isValid_FileSize(UnsignedInt64 FileSize) {
		return true;
	}

	public static boolean isValid_InUseCount(UnsignedInt64 InUseCount) {
		return true;
	}
	...
	public static boolean isValid_LastModified(Calendar LastModified) {
		return true;
	}
	...
	public static boolean isValid_Readable(Boolean Readable) {
		return true;
	}

	public static boolean isValid_Writeable(Boolean Writeable) {
		return true;
	}
