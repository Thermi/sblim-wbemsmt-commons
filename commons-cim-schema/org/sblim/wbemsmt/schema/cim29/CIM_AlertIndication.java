...
2 and 0 - Information and Unknown (respectively) follow common usage. Literally, the AlertIndication is purely informational or its severity is simply unknown.
	...
	static {
		...
		for (int i = 0; i < CIM_ProcessIndication.CIM_PropertyNameList.size(); i++) {
			if (((String)CIM_ProcessIndication.CIM_PropertyNameList.elementAt(i)).equals(CIM_PROPERTY_ALERTTYPE)||
				...
				((String)CIM_ProcessIndication.CIM_PropertyNameList.elementAt(i)).equals(CIM_PROPERTY_OTHERSEVERITY)||
		...
		for (int i = 0; i < CIM_ProcessIndication.CIM_PropertyList.size(); i++) {
			if (((CIMProperty)CIM_ProcessIndication.CIM_PropertyList.get(i)).getName().equals(CIM_PROPERTY_ALERTTYPE)||
				...
				((CIMProperty)CIM_ProcessIndication.CIM_PropertyList.get(i)).getName().equals(CIM_PROPERTY_OTHERSEVERITY)||
	...
	*/	
	...
	public static void addPackage(String packagename) {
        if (packagename != null) {
            ...
            CIM_AlertIndication.Java_Package_List.add(packagename);
    ...
    public static String[] getPackages() {
        return (String[]) CIM_AlertIndication.Java_Package_List.toArray(new String[CIM_AlertIndication.Java_Package_List.size()]);
    }
	...
	*/	
	...
	public String get_OtherSeverity() {
		...
		CIMProperty currentProperty = this.cimInstance.getProperty(CIM_AlertIndication.CIM_PROPERTY_OTHERSEVERITY);
        
		if (currentProperty == null) {
			throw new CIMException(CIMException.CIM_ERR_NO_SUCH_PROPERTY, "The property " + CIM_AlertIndication.CIM_PROPERTY_OTHERSEVERITY + " could not be found");
    		
		} else if (currentProperty.getType() == null || currentProperty.getType().getType() != CIMDataType.STRING) {
			throw new CIMException(CIMException.CIM_ERR_TYPE_MISMATCH, "The property " + CIM_AlertIndication.CIM_PROPERTY_OTHERSEVERITY + " is not of expected type " + CIMDataType.getPredefinedType(CIMDataType.STRING) + ".");
		}
	...
	public void set_OtherSeverity(String newValue) {
		...
		} else if (!CIM_AlertIndicationHelper.isValid_OtherSeverity(newValue)) {
			throw new InvalidParameterException("The value " + newValue + " is not valid for property " + CIM_AlertIndication.CIM_PROPERTY_OTHERSEVERITY);
