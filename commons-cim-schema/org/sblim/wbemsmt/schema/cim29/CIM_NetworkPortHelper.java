...
public final class CIM_NetworkPortHelper {
	...
	*/	
	public final static CIM_NetworkPort getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_NetworkPortHelper.getInstance(cimClient, new CIM_NetworkPort(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_NetworkPort getInstance(CIMClient cimClient, CIM_NetworkPort dataInstance) {
		...
		return CIM_NetworkPortHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_NetworkPort getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_NetworkPort(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_NetworkPort.getPackages();
	...
	*/	
	public final static CIM_NetworkPort createInstance(CIMClient cimClient, CIM_NetworkPort dataInstance, boolean notifyDifferencies){ 
		...
		CIM_NetworkPort newDataInstance = null;
		try {
			newDataInstance = CIM_NetworkPortHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_NetworkPort modifyInstance(CIMClient cimClient, CIM_NetworkPort dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_NetworkPortHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_NetworkPort dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_NetworkPort dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_NetworkPort.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_NetworkPort.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_NetworkPort.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_NetworkPort.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_NetworkPort.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ActiveMaximumTransmissionUnit(UnsignedInt64 ActiveMaximumTransmissionUnit) {
		return true;
	}

	public static boolean isValid_AutoSense(Boolean AutoSense) {
		return true;
	}

	public static boolean isValid_FullDuplex(Boolean FullDuplex) {
		return true;
	}

	public static boolean isValid_LinkTechnology(UnsignedInt16 LinkTechnology) {
		return true;
	}
	...
	public static boolean isValid_OtherLinkTechnology(String OtherLinkTechnology) {
		return true;
	}

	public static boolean isValid_OtherNetworkPortType(String OtherNetworkPortType) {
		return true;
	}

	public static boolean isValid_PermanentAddress(String PermanentAddress) {
		return true;
	}

	public static boolean isValid_PortNumber(UnsignedInt16 PortNumber) {
		return true;
	}

	public static boolean isValid_PortType(UnsignedInt16 PortType) {
		return true;
	}

	public static boolean isValid_Speed(UnsignedInt64 Speed) {
		return true;
	}

	public static boolean isValid_SupportedMaximumTransmissionUnit(UnsignedInt64 SupportedMaximumTransmissionUnit) {
		return true;
	}
