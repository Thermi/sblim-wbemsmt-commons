...
public final class CIM_IndicationFilterHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_IndicationFilter(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_IndicationFilter getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_IndicationFilterHelper.getInstance(cimClient, new CIM_IndicationFilter(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_IndicationFilter getInstance(CIMClient cimClient, CIM_IndicationFilter dataInstance) {
		...
		return CIM_IndicationFilterHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_IndicationFilter getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_IndicationFilter(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_IndicationFilter.getPackages();
	...
	*/	
	public final static CIM_IndicationFilter createInstance(CIMClient cimClient, CIM_IndicationFilter dataInstance, boolean notifyDifferencies){ 
		...
		CIM_IndicationFilter newDataInstance = null;
		try {
			newDataInstance = CIM_IndicationFilterHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_IndicationFilter modifyInstance(CIMClient cimClient, CIM_IndicationFilter dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_IndicationFilterHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_IndicationFilter dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_IndicationFilter dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_IndicationFilter.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_IndicationFilter.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_IndicationFilter.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_IndicationFilter.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_IndicationFilter.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Query(String Query) {
		return true;
	}

	public static boolean isValid_QueryLanguage(String QueryLanguage) {
		return true;
	}

	public static boolean isValid_SourceNamespace(String SourceNamespace) {
		return true;
	}
