...
public final class CIM_FileSystemHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_FileSystem.");
						resultArrayList.add(new CIM_FileSystem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_FileSystem.");
						resultArrayList.add(new CIM_FileSystem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_FileSystem.");
						resultArrayList.add(new CIM_FileSystem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_FileSystem getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_FileSystemHelper.getInstance(cimClient, new CIM_FileSystem(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_FileSystem getInstance(CIMClient cimClient, CIM_FileSystem dataInstance) {
		...
		return CIM_FileSystemHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_FileSystem getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_FileSystem dataInstance = null;
		try {
			dataInstance = new CIM_FileSystem(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_FileSystem.getPackages();
	...
	*/	
	public final static CIM_FileSystem createInstance(CIMClient cimClient, CIM_FileSystem dataInstance, boolean notifyDifferencies){ 
		...
		CIM_FileSystem newDataInstance = null;
		try {
			newDataInstance = CIM_FileSystemHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_FileSystem modifyInstance(CIMClient cimClient, CIM_FileSystem dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_FileSystemHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_FileSystem dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_FileSystem dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_FileSystem.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_FileSystem.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_FileSystem.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_FileSystem.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_FileSystem.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AvailableSpace(UnsignedInt64 AvailableSpace) {
		return true;
	}
	...
	public static boolean isValid_CasePreserved(Boolean CasePreserved) {
		return true;
	}
	...
	public static boolean isValid_ClusterSize(UnsignedInt32 ClusterSize) {
		return true;
	}

	public static boolean isValid_CodeSet(UnsignedInt16[] CodeSet) {
		return true;
	}
	...
	public static boolean isValid_FileSystemSize(UnsignedInt64 FileSystemSize) {
		return true;
	}

	public static boolean isValid_FileSystemType(String FileSystemType) {
		return true;
	}

	public static boolean isValid_MaxFileNameLength(UnsignedInt32 MaxFileNameLength) {
		return true;
	}
	...
	public static boolean isValid_NumberOfFiles(UnsignedInt64 NumberOfFiles) {
		return true;
	}
	...
	public static boolean isValid_Root(String Root) {
		return true;
	}
