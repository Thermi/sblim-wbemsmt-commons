/** 
 * CIM_FibrePortActiveLoginHelper.java
 *
...
public final class CIM_FibrePortActiveLoginHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_FibrePortActiveLogin(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_FibrePortActiveLogin.Java_Package_List.size(); i++) {
						if (!((String)(CIM_FibrePortActiveLogin.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_FibrePortActiveLogin.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_FibrePortActiveLogin.Java_Package_List.setElementAt((String)(CIM_FibrePortActiveLogin.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_FibrePortActiveLogin.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_FibrePortActiveLogin.");
						resultArrayList.add(new CIM_FibrePortActiveLogin(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_FibrePortActiveLogin.");
						resultArrayList.add(new CIM_FibrePortActiveLogin(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_FibrePortActiveLogin.");
						resultArrayList.add(new CIM_FibrePortActiveLogin(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_FibrePortActiveLogin getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_FibrePortActiveLoginHelper.getInstance(cimClient, new CIM_FibrePortActiveLogin(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_FibrePortActiveLogin getInstance(CIMClient cimClient, CIM_FibrePortActiveLogin dataInstance) {
		...
		return CIM_FibrePortActiveLoginHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_FibrePortActiveLogin getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_FibrePortActiveLogin dataInstance = null;
		try {
			dataInstance = new CIM_FibrePortActiveLogin(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_FibrePortActiveLogin createInstance(CIMClient cimClient, CIM_FibrePortActiveLogin dataInstance, boolean notifyDifferencies){ 
		...
		CIM_FibrePortActiveLogin newDataInstance = null;
		try {
			newDataInstance = CIM_FibrePortActiveLoginHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_FibrePortActiveLogin modifyInstance(CIMClient cimClient, CIM_FibrePortActiveLogin dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_FibrePortActiveLoginHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_FibrePortActiveLogin dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_FibrePortActiveLogin dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_FibrePortActiveLogin.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_FibrePortActiveLogin.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_FibrePortActiveLogin.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_FibrePortActiveLogin.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_FibrePortActiveLogin.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ACKModel(UnsignedInt16 ACKModel) {
		return true;
	}

	public static boolean isValid_BufferToBufferModel(UnsignedInt16 BufferToBufferModel) {
		return true;
	}

	public static boolean isValid_NegotiatedCOS(UnsignedInt16 NegotiatedCOS) {
		return true;
	}

	public static boolean isValid_NegotiatedFrameSize(UnsignedInt64 NegotiatedFrameSize) {
		return true;
	}

	public static boolean isValid_NegotiatedSpeed(UnsignedInt64 NegotiatedSpeed) {
		return true;
	}

	public static boolean isValid_OriginatorBufferCredit(UnsignedInt64 OriginatorBufferCredit) {
		return true;
	}

	public static boolean isValid_OriginatorEndCredit(UnsignedInt64 OriginatorEndCredit) {
		return true;
	}

	public static boolean isValid_ResponderBufferCredit(UnsignedInt64 ResponderBufferCredit) {
		return true;
	}

	public static boolean isValid_ResponderEndCredit(UnsignedInt64 ResponderEndCredit) {
		return true;
	}

	public static boolean isValid_CIM_FibrePort_1(CIM_FibrePort CIM_FibrePort_1) {
		return true;
	}

	public static boolean isValid_CIM_FibrePort_2(CIM_FibrePort CIM_FibrePort_2) {
		return true;
	}
