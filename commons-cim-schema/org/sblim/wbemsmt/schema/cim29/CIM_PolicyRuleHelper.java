...
public final class CIM_PolicyRuleHelper {

	public final static String CIM_CLASS_NAME = "CIM_PolicyRule";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_PolicyRule.Java_Package_List.size(); i++) {
						if (!((String)(CIM_PolicyRule.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_PolicyRule.Java_Package_List.get(i))).endsWith(".")) {
							CIM_PolicyRule.Java_Package_List.setElementAt((String)(CIM_PolicyRule.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_PolicyRule.");
						resultArrayList.add(new CIM_PolicyRule(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_PolicyRule.");
						resultArrayList.add(new CIM_PolicyRule(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_PolicyRule.");
						resultArrayList.add(new CIM_PolicyRule(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_PolicyRule getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_PolicyRuleHelper.getInstance(cimClient, new CIM_PolicyRule(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PolicyRule getInstance(CIMClient cimClient, CIM_PolicyRule dataInstance) {
		...
		return CIM_PolicyRuleHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PolicyRule getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_PolicyRule dataInstance = null;
		try {
			dataInstance = new CIM_PolicyRule(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_PolicyRule createInstance(CIMClient cimClient, CIM_PolicyRule dataInstance, boolean notifyDifferencies){ 
		...
		CIM_PolicyRule newDataInstance = null;
		try {
			newDataInstance = CIM_PolicyRuleHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_PolicyRule modifyInstance(CIMClient cimClient, CIM_PolicyRule dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_PolicyRuleHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_PolicyRule dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_PolicyRule dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_PolicyRule.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_PolicyRule.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_PolicyRule.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_PolicyRule.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_PolicyRule.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ExecutionStrategy(UnsignedInt16 ExecutionStrategy) {
		return true;
	}

	public static boolean isValid_Mandatory(Boolean Mandatory) {
		return true;
	}

	public static boolean isValid_PolicyRuleName(String PolicyRuleName) {
		return true;
	}
	...
	public static boolean isValid_RuleUsage(String RuleUsage) {
		return true;
	}

	public static boolean isValid_SequencedActions(UnsignedInt16 SequencedActions) {
		return true;
	}
