/** 
 * CIM_LocalFileSystemHelper.java
 *
...
public final class CIM_LocalFileSystemHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					Class clazz = CIM_LocalFileSystemHelper.findClass(cimClient, cimInstance);
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_LocalFileSystem.");
						resultArrayList.add(new CIM_LocalFileSystem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_LocalFileSystem.");
						resultArrayList.add(new CIM_LocalFileSystem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_LocalFileSystem.");
						resultArrayList.add(new CIM_LocalFileSystem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_LocalFileSystem getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_LocalFileSystemHelper.getInstance(cimClient, new CIM_LocalFileSystem(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LocalFileSystem getInstance(CIMClient cimClient, CIM_LocalFileSystem dataInstance) {
		...
		return CIM_LocalFileSystemHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LocalFileSystem getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_LocalFileSystem dataInstance = null;
		try {
			dataInstance = new CIM_LocalFileSystem(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_LocalFileSystem.getPackages();
	...
	*/	
	public final static CIM_LocalFileSystem createInstance(CIMClient cimClient, CIM_LocalFileSystem dataInstance, boolean notifyDifferencies){ 
		...
		CIM_LocalFileSystem newDataInstance = null;
		try {
			newDataInstance = CIM_LocalFileSystemHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_LocalFileSystem modifyInstance(CIMClient cimClient, CIM_LocalFileSystem dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_LocalFileSystemHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_LocalFileSystem dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_LocalFileSystem dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_LocalFileSystem.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_LocalFileSystem.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_LocalFileSystem.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_LocalFileSystem.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_LocalFileSystem.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
