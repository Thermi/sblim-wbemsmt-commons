...
public final class CIM_RackHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_Rack.");
						resultArrayList.add(new CIM_Rack(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_Rack.");
						resultArrayList.add(new CIM_Rack(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_Rack.");
						resultArrayList.add(new CIM_Rack(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_Rack getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_RackHelper.getInstance(cimClient, new CIM_Rack(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Rack getInstance(CIMClient cimClient, CIM_Rack dataInstance) {
		...
		return CIM_RackHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Rack getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_Rack dataInstance = null;
		try {
			dataInstance = new CIM_Rack(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_Rack.getPackages();
	...
	*/	
	public final static CIM_Rack createInstance(CIMClient cimClient, CIM_Rack dataInstance, boolean notifyDifferencies){ 
		...
		CIM_Rack newDataInstance = null;
		try {
			newDataInstance = CIM_RackHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Rack modifyInstance(CIMClient cimClient, CIM_Rack dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_RackHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_Rack dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_Rack dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_Rack.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Rack.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Rack.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Rack.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Rack.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CountryDesignation(String CountryDesignation) {
		return true;
	}
	...
	public static boolean isValid_TypeOfRack(UnsignedInt16 TypeOfRack) {
		return true;
	}
