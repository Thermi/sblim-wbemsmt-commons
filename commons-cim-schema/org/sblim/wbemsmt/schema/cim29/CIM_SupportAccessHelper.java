...
public final class CIM_SupportAccessHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_SupportAccess.Java_Package_List.size(); i++) {
						if (!((String)(CIM_SupportAccess.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_SupportAccess.Java_Package_List.get(i))).endsWith(".")) {
							CIM_SupportAccess.Java_Package_List.setElementAt((String)(CIM_SupportAccess.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_SupportAccess.");
						resultArrayList.add(new CIM_SupportAccess(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_SupportAccess.");
						resultArrayList.add(new CIM_SupportAccess(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_SupportAccess.");
						resultArrayList.add(new CIM_SupportAccess(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_SupportAccess getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_SupportAccessHelper.getInstance(cimClient, new CIM_SupportAccess(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SupportAccess getInstance(CIMClient cimClient, CIM_SupportAccess dataInstance) {
		...
		return CIM_SupportAccessHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SupportAccess getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_SupportAccess dataInstance = null;
		try {
			dataInstance = new CIM_SupportAccess(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_SupportAccess createInstance(CIMClient cimClient, CIM_SupportAccess dataInstance, boolean notifyDifferencies){ 
		...
		CIM_SupportAccess newDataInstance = null;
		try {
			newDataInstance = CIM_SupportAccessHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_SupportAccess modifyInstance(CIMClient cimClient, CIM_SupportAccess dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_SupportAccessHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_SupportAccess dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_SupportAccess dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_SupportAccess.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_SupportAccess.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_SupportAccess.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_SupportAccess.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_SupportAccess.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CommunicationInfo(String CommunicationInfo) {
		return true;
	}

	public static boolean isValid_CommunicationMode(UnsignedInt16 CommunicationMode) {
		return true;
	}
	...
	public static boolean isValid_Locale(String Locale) {
		return true;
	}

	public static boolean isValid_SupportAccessId(String SupportAccessId) {
		return true;
	}
