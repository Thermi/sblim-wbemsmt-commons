/** 
 * CIM_MethodParametersHelper.java
 *
...
public final class CIM_MethodParametersHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_MethodParameters.");
						resultArrayList.add(new CIM_MethodParameters(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_MethodParameters.");
						resultArrayList.add(new CIM_MethodParameters(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_MethodParameters.");
						resultArrayList.add(new CIM_MethodParameters(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_MethodParameters getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_MethodParametersHelper.getInstance(cimClient, new CIM_MethodParameters(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_MethodParameters getInstance(CIMClient cimClient, CIM_MethodParameters dataInstance) {
		...
		return CIM_MethodParametersHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_MethodParameters getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_MethodParameters dataInstance = null;
		try {
			dataInstance = new CIM_MethodParameters(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_MethodParameters.getPackages();
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_MethodParameters.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_MethodParameters.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_MethodParameters.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_MethodParameters.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_MethodParametersId(String MethodParametersId) {
		return true;
	}
