...
public final class CIM_AdminDomainHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_AdminDomain.Java_Package_List.size(); i++) {
						if (!((String)(CIM_AdminDomain.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_AdminDomain.Java_Package_List.get(i))).endsWith(".")) {
							CIM_AdminDomain.Java_Package_List.setElementAt((String)(CIM_AdminDomain.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_AdminDomain.");
						resultArrayList.add(new CIM_AdminDomain(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_AdminDomain.");
						resultArrayList.add(new CIM_AdminDomain(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_AdminDomain.");
						resultArrayList.add(new CIM_AdminDomain(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_AdminDomain getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_AdminDomainHelper.getInstance(cimClient, new CIM_AdminDomain(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_AdminDomain getInstance(CIMClient cimClient, CIM_AdminDomain dataInstance) {
		...
		return CIM_AdminDomainHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_AdminDomain getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_AdminDomain dataInstance = null;
		try {
			dataInstance = new CIM_AdminDomain(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_AdminDomain createInstance(CIMClient cimClient, CIM_AdminDomain dataInstance, boolean notifyDifferencies){ 
		...
		CIM_AdminDomain newDataInstance = null;
		try {
			newDataInstance = CIM_AdminDomainHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_AdminDomain modifyInstance(CIMClient cimClient, CIM_AdminDomain dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_AdminDomainHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_AdminDomain dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_AdminDomain dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_AdminDomain.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_AdminDomain.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_AdminDomain.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_AdminDomain.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_AdminDomain.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
