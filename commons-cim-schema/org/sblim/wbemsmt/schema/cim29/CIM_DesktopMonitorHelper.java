...
public final class CIM_DesktopMonitorHelper {

	public final static String CIM_CLASS_NAME = "CIM_DesktopMonitor";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_DesktopMonitor.Java_Package_List.size(); i++) {
						if (!((String)(CIM_DesktopMonitor.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_DesktopMonitor.Java_Package_List.get(i))).endsWith(".")) {
							CIM_DesktopMonitor.Java_Package_List.setElementAt((String)(CIM_DesktopMonitor.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_DesktopMonitor.");
						resultArrayList.add(new CIM_DesktopMonitor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_DesktopMonitor.");
						resultArrayList.add(new CIM_DesktopMonitor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_DesktopMonitor.");
						resultArrayList.add(new CIM_DesktopMonitor(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_DesktopMonitor getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_DesktopMonitorHelper.getInstance(cimClient, new CIM_DesktopMonitor(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DesktopMonitor getInstance(CIMClient cimClient, CIM_DesktopMonitor dataInstance) {
		...
		return CIM_DesktopMonitorHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DesktopMonitor getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_DesktopMonitor dataInstance = null;
		try {
			dataInstance = new CIM_DesktopMonitor(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_DesktopMonitor createInstance(CIMClient cimClient, CIM_DesktopMonitor dataInstance, boolean notifyDifferencies){ 
		...
		CIM_DesktopMonitor newDataInstance = null;
		try {
			newDataInstance = CIM_DesktopMonitorHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_DesktopMonitor modifyInstance(CIMClient cimClient, CIM_DesktopMonitor dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_DesktopMonitorHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_DesktopMonitor dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_DesktopMonitor dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_DesktopMonitor.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_DesktopMonitor.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_DesktopMonitor.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_DesktopMonitor.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_DesktopMonitor.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Bandwidth(UnsignedInt32 Bandwidth) {
		return true;
	}

	public static boolean isValid_DisplayType(UnsignedInt16 DisplayType) {
		return true;
	}

	public static boolean isValid_ScreenHeight(UnsignedInt32 ScreenHeight) {
		return true;
	}

	public static boolean isValid_ScreenWidth(UnsignedInt32 ScreenWidth) {
		return true;
	}
