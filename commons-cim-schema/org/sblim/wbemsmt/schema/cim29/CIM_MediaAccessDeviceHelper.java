...
public final class CIM_MediaAccessDeviceHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					Class clazz = CIM_MediaAccessDeviceHelper.findClass(cimClient, cimInstance);
					if (clazz == null) {
	...
	*/	
	public final static CIM_MediaAccessDevice getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_MediaAccessDeviceHelper.getInstance(cimClient, new CIM_MediaAccessDevice(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_MediaAccessDevice getInstance(CIMClient cimClient, CIM_MediaAccessDevice dataInstance) {
		...
		return CIM_MediaAccessDeviceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_MediaAccessDevice getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_MediaAccessDevice(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_MediaAccessDevice.getPackages();
	...
	*/	
	public final static CIM_MediaAccessDevice createInstance(CIMClient cimClient, CIM_MediaAccessDevice dataInstance, boolean notifyDifferencies){ 
		...
		CIM_MediaAccessDevice newDataInstance = null;
		try {
			newDataInstance = CIM_MediaAccessDeviceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_MediaAccessDevice modifyInstance(CIMClient cimClient, CIM_MediaAccessDevice dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_MediaAccessDeviceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_MediaAccessDevice dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_MediaAccessDevice dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_MediaAccessDevice.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_MediaAccessDevice.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_MediaAccessDevice.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_MediaAccessDevice.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_MediaAccessDevice.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CompressionMethod(String CompressionMethod) {
		return true;
	}

	public static boolean isValid_DefaultBlockSize(UnsignedInt64 DefaultBlockSize) {
		return true;
	}
	...
	public static boolean isValid_LastCleaned(Calendar LastCleaned) {
		return true;
	}

	public static boolean isValid_LoadTime(UnsignedInt64 LoadTime) {
		return true;
	}

	public static boolean isValid_MaxAccessTime(UnsignedInt64 MaxAccessTime) {
		return true;
	}

	public static boolean isValid_MaxBlockSize(UnsignedInt64 MaxBlockSize) {
		return true;
	}

	public static boolean isValid_MaxMediaSize(UnsignedInt64 MaxMediaSize) {
		return true;
	}

	public static boolean isValid_MaxUnitsBeforeCleaning(UnsignedInt64 MaxUnitsBeforeCleaning) {
		return true;
	}

	public static boolean isValid_MediaIsLocked(Boolean MediaIsLocked) {
		return true;
	}

	public static boolean isValid_MinBlockSize(UnsignedInt64 MinBlockSize) {
		return true;
	}
	...
	public static boolean isValid_NeedsCleaning(Boolean NeedsCleaning) {
		return true;
	}

	public static boolean isValid_NumberOfMediaSupported(UnsignedInt32 NumberOfMediaSupported) {
		return true;
	}

	public static boolean isValid_Security(UnsignedInt16 Security) {
		return true;
	}
	...
	public static boolean isValid_UncompressedDataRate(UnsignedInt32 UncompressedDataRate) {
		return true;
	}

	public static boolean isValid_UnitsDescription(String UnitsDescription) {
		return true;
	}

	public static boolean isValid_UnitsUsed(UnsignedInt64 UnitsUsed) {
		return true;
	}

	public static boolean isValid_UnloadTime(UnsignedInt64 UnloadTime) {
		return true;
	}
