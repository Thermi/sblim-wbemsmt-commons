...
public final class CIM_LANEndpointHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_LANEndpoint.Java_Package_List.size(); i++) {
						if (!((String)(CIM_LANEndpoint.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_LANEndpoint.Java_Package_List.get(i))).endsWith(".")) {
							CIM_LANEndpoint.Java_Package_List.setElementAt((String)(CIM_LANEndpoint.Java_Package_List.get(i)) + ("."), i);
						}
	...
	*/	
	public final static CIM_LANEndpoint getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_LANEndpointHelper.getInstance(cimClient, new CIM_LANEndpoint(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LANEndpoint getInstance(CIMClient cimClient, CIM_LANEndpoint dataInstance) {
		...
		return CIM_LANEndpointHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LANEndpoint getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_LANEndpoint(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_LANEndpoint createInstance(CIMClient cimClient, CIM_LANEndpoint dataInstance, boolean notifyDifferencies){ 
		...
		CIM_LANEndpoint newDataInstance = null;
		try {
			newDataInstance = CIM_LANEndpointHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_LANEndpoint modifyInstance(CIMClient cimClient, CIM_LANEndpoint dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_LANEndpointHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_LANEndpoint dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_LANEndpoint dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_LANEndpoint.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_LANEndpoint.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_LANEndpoint.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_LANEndpoint.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_LANEndpoint.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AliasAddresses(String[] AliasAddresses) {
		return true;
	}

	public static boolean isValid_GroupAddresses(String[] GroupAddresses) {
		return true;
	}
	...
	public static boolean isValid_LANType(UnsignedInt16 LANType) {
		return true;
	}
	...
	public static boolean isValid_OtherLANType(String OtherLANType) {
		return true;
	}
