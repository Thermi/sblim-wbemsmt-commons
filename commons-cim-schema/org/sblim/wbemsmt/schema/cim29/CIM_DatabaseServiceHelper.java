/** 
 * CIM_DatabaseServiceHelper.java
 *
...
public final class CIM_DatabaseServiceHelper {

	public final static String CIM_CLASS_NAME = "CIM_DatabaseService";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_DatabaseService.Java_Package_List.size(); i++) {
						if (!((String)(CIM_DatabaseService.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_DatabaseService.Java_Package_List.get(i))).endsWith(".")) {
							CIM_DatabaseService.Java_Package_List.setElementAt((String)(CIM_DatabaseService.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_DatabaseService.");
						resultArrayList.add(new CIM_DatabaseService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_DatabaseService.");
						resultArrayList.add(new CIM_DatabaseService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_DatabaseService.");
						resultArrayList.add(new CIM_DatabaseService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_DatabaseService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_DatabaseServiceHelper.getInstance(cimClient, new CIM_DatabaseService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DatabaseService getInstance(CIMClient cimClient, CIM_DatabaseService dataInstance) {
		...
		return CIM_DatabaseServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_DatabaseService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_DatabaseService dataInstance = null;
		try {
			dataInstance = new CIM_DatabaseService(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_DatabaseService createInstance(CIMClient cimClient, CIM_DatabaseService dataInstance, boolean notifyDifferencies){ 
		...
		CIM_DatabaseService newDataInstance = null;
		try {
			newDataInstance = CIM_DatabaseServiceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_DatabaseService modifyInstance(CIMClient cimClient, CIM_DatabaseService dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_DatabaseServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_DatabaseService dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_DatabaseService dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_DatabaseService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_DatabaseService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_DatabaseService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_DatabaseService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_DatabaseService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ConnectionLimit(UnsignedInt32 ConnectionLimit) {
		return true;
	}

	public static boolean isValid_LastStatusChangeTime(Calendar LastStatusChangeTime) {
		return true;
	}
	...
	public static boolean isValid_StartupTime(Calendar StartupTime) {
		return true;
	}
