...
public final class PRS_AgreementHelper {

	public final static String CIM_CLASS_NAME = "PRS_Agreement";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<PRS_Agreement.Java_Package_List.size(); i++) {
						if (!((String)(PRS_Agreement.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(PRS_Agreement.Java_Package_List.get(i))).endsWith(".")) {
							PRS_Agreement.Java_Package_List.setElementAt((String)(PRS_Agreement.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Agreement.");
						resultArrayList.add(new PRS_Agreement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Agreement.");
						resultArrayList.add(new PRS_Agreement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Agreement.");
						resultArrayList.add(new PRS_Agreement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Agreement getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_AgreementHelper.getInstance(cimClient, new PRS_Agreement(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Agreement getInstance(CIMClient cimClient, PRS_Agreement dataInstance) {
		...
		return PRS_AgreementHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Agreement getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Agreement dataInstance = null;
		try {
			dataInstance = new PRS_Agreement(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Agreement createInstance(CIMClient cimClient, PRS_Agreement dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Agreement newDataInstance = null;
		try {
			newDataInstance = PRS_AgreementHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Agreement modifyInstance(CIMClient cimClient, PRS_Agreement dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_AgreementHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Agreement dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Agreement dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Agreement.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Agreement.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Agreement.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Agreement.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Agreement.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AgreementType(String AgreementType) {
		return true;
	}

	public static boolean isValid_ContractID(String ContractID) {
		return true;
	}
