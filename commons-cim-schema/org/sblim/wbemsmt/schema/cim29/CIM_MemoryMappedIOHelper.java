...
public final class CIM_MemoryMappedIOHelper {

	public final static String CIM_CLASS_NAME = "CIM_MemoryMappedIO";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_MemoryMappedIO.Java_Package_List.size(); i++) {
						if (!((String)(CIM_MemoryMappedIO.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_MemoryMappedIO.Java_Package_List.get(i))).endsWith(".")) {
							CIM_MemoryMappedIO.Java_Package_List.setElementAt((String)(CIM_MemoryMappedIO.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_MemoryMappedIO.");
						resultArrayList.add(new CIM_MemoryMappedIO(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_MemoryMappedIO.");
						resultArrayList.add(new CIM_MemoryMappedIO(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_MemoryMappedIO.");
						resultArrayList.add(new CIM_MemoryMappedIO(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_MemoryMappedIO getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_MemoryMappedIOHelper.getInstance(cimClient, new CIM_MemoryMappedIO(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_MemoryMappedIO getInstance(CIMClient cimClient, CIM_MemoryMappedIO dataInstance) {
		...
		return CIM_MemoryMappedIOHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_MemoryMappedIO getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_MemoryMappedIO dataInstance = null;
		try {
			dataInstance = new CIM_MemoryMappedIO(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_MemoryMappedIO createInstance(CIMClient cimClient, CIM_MemoryMappedIO dataInstance, boolean notifyDifferencies){ 
		...
		CIM_MemoryMappedIO newDataInstance = null;
		try {
			newDataInstance = CIM_MemoryMappedIOHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_MemoryMappedIO modifyInstance(CIMClient cimClient, CIM_MemoryMappedIO dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_MemoryMappedIOHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_MemoryMappedIO dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_MemoryMappedIO dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_MemoryMappedIO.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_MemoryMappedIO.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_MemoryMappedIO.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_MemoryMappedIO.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_MemoryMappedIO.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_MappedResource(UnsignedInt16 MappedResource) {
		return true;
	}
