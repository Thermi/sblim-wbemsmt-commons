...
public final class CIM_ConfigurationHelper {
	...
	*/	
	public final static CIM_Configuration getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ConfigurationHelper.getInstance(cimClient, new CIM_Configuration(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Configuration getInstance(CIMClient cimClient, CIM_Configuration dataInstance) {
		...
		return CIM_ConfigurationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Configuration getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_Configuration(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Configuration createInstance(CIMClient cimClient, CIM_Configuration dataInstance, boolean notifyDifferencies){ 
		...
		CIM_Configuration newDataInstance = null;
		try {
			newDataInstance = CIM_ConfigurationHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Configuration modifyInstance(CIMClient cimClient, CIM_Configuration dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_ConfigurationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_Configuration dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_Configuration dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_Configuration.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Configuration.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Configuration.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Configuration.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Configuration.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
