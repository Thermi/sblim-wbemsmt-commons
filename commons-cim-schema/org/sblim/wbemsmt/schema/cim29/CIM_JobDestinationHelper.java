...
public final class CIM_JobDestinationHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_JobDestination.");
						resultArrayList.add(new CIM_JobDestination(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_JobDestination.");
						resultArrayList.add(new CIM_JobDestination(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_JobDestination.");
						resultArrayList.add(new CIM_JobDestination(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_JobDestination getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_JobDestinationHelper.getInstance(cimClient, new CIM_JobDestination(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_JobDestination getInstance(CIMClient cimClient, CIM_JobDestination dataInstance) {
		...
		return CIM_JobDestinationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_JobDestination getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_JobDestination(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_JobDestination.getPackages();
	...
	*/	
	public final static CIM_JobDestination createInstance(CIMClient cimClient, CIM_JobDestination dataInstance, boolean notifyDifferencies){ 
		...
		CIM_JobDestination newDataInstance = null;
		try {
			newDataInstance = CIM_JobDestinationHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_JobDestination modifyInstance(CIMClient cimClient, CIM_JobDestination dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_JobDestinationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_JobDestination dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_JobDestination dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_JobDestination.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_JobDestination.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_JobDestination.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_JobDestination.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_JobDestination.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
