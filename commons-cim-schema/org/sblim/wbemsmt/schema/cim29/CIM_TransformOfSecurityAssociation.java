 ...
 *
 *
 * Description: TransformOfSecurityAssociation maps a SecurityAssociation Endpoint with the transform that it uses. For security, no keying material of the SA is exposed.
 * 
 */
...
public class CIM_TransformOfSecurityAssociation extends CIM_ElementSettingData  {
	...
	static {
		...
		for (int i = 0; i < CIM_ElementSettingData.Java_Package_List.size(); i++) {
			if (((String)CIM_ElementSettingData.Java_Package_List.elementAt(i)).equals("org.sblim.wbemsmt.schema.cim29")){
				continue;
			}
