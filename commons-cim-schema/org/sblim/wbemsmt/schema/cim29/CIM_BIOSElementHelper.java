...
public final class CIM_BIOSElementHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_BIOSElement.");
						resultArrayList.add(new CIM_BIOSElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_BIOSElement.");
						resultArrayList.add(new CIM_BIOSElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_BIOSElement.");
						resultArrayList.add(new CIM_BIOSElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_BIOSElement getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_BIOSElementHelper.getInstance(cimClient, new CIM_BIOSElement(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_BIOSElement getInstance(CIMClient cimClient, CIM_BIOSElement dataInstance) {
		...
		return CIM_BIOSElementHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_BIOSElement getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_BIOSElement dataInstance = null;
		try {
			dataInstance = new CIM_BIOSElement(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_BIOSElement.getPackages();
	...
	*/	
	public final static CIM_BIOSElement createInstance(CIMClient cimClient, CIM_BIOSElement dataInstance, boolean notifyDifferencies){ 
		...
		CIM_BIOSElement newDataInstance = null;
		try {
			newDataInstance = CIM_BIOSElementHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_BIOSElement modifyInstance(CIMClient cimClient, CIM_BIOSElement dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_BIOSElementHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_BIOSElement dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_BIOSElement dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_BIOSElement.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_BIOSElement.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_BIOSElement.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_BIOSElement.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_BIOSElement.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CurrentLanguage(String CurrentLanguage) {
		return true;
	}

	public static boolean isValid_ListOfLanguages(String[] ListOfLanguages) {
		return true;
	}

	public static boolean isValid_LoadUtilityInformation(String LoadUtilityInformation) {
		return true;
	}

	public static boolean isValid_LoadedEndingAddress(UnsignedInt64 LoadedEndingAddress) {
		return true;
	}

	public static boolean isValid_LoadedStartingAddress(UnsignedInt64 LoadedStartingAddress) {
		return true;
	}
	...
	public static boolean isValid_PrimaryBIOS(Boolean PrimaryBIOS) {
		return true;
	}
