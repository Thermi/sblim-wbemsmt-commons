...
public final class PRS_RevisionHelper {

	public final static String CIM_CLASS_NAME = "PRS_Revision";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<PRS_Revision.Java_Package_List.size(); i++) {
						if (!((String)(PRS_Revision.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(PRS_Revision.Java_Package_List.get(i))).endsWith(".")) {
							PRS_Revision.Java_Package_List.setElementAt((String)(PRS_Revision.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Revision.");
						resultArrayList.add(new PRS_Revision(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Revision.");
						resultArrayList.add(new PRS_Revision(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Revision.");
						resultArrayList.add(new PRS_Revision(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Revision getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_RevisionHelper.getInstance(cimClient, new PRS_Revision(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Revision getInstance(CIMClient cimClient, PRS_Revision dataInstance) {
		...
		return PRS_RevisionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Revision getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Revision dataInstance = null;
		try {
			dataInstance = new PRS_Revision(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Revision createInstance(CIMClient cimClient, PRS_Revision dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Revision newDataInstance = null;
		try {
			newDataInstance = PRS_RevisionHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Revision modifyInstance(CIMClient cimClient, PRS_Revision dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_RevisionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Revision dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Revision dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Revision.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Revision.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Revision.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Revision.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Revision.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ActivityName(UnsignedInt8 ActivityName) {
		return true;
	}

	public static boolean isValid_ActivityType(UnsignedInt8 ActivityType) {
		return true;
	}
	...
	public static boolean isValid_EditorialStatus(UnsignedInt8 EditorialStatus) {
		return true;
	}

	public static boolean isValid_RevisionDate(Calendar RevisionDate) {
		return true;
	}

	public static boolean isValid_TechnicalStatus(UnsignedInt8 TechnicalStatus) {
		return true;
	}
