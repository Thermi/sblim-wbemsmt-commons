/** 
 * CIM_REDDropperServiceHelper.java
 *
...
public final class CIM_REDDropperServiceHelper {

	public final static String CIM_CLASS_NAME = "CIM_REDDropperService";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_REDDropperService.Java_Package_List.size(); i++) {
						if (!((String)(CIM_REDDropperService.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_REDDropperService.Java_Package_List.get(i))).endsWith(".")) {
							CIM_REDDropperService.Java_Package_List.setElementAt((String)(CIM_REDDropperService.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_REDDropperService.");
						resultArrayList.add(new CIM_REDDropperService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_REDDropperService.");
						resultArrayList.add(new CIM_REDDropperService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_REDDropperService.");
						resultArrayList.add(new CIM_REDDropperService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_REDDropperService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_REDDropperServiceHelper.getInstance(cimClient, new CIM_REDDropperService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_REDDropperService getInstance(CIMClient cimClient, CIM_REDDropperService dataInstance) {
		...
		return CIM_REDDropperServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_REDDropperService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_REDDropperService dataInstance = null;
		try {
			dataInstance = new CIM_REDDropperService(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_REDDropperService createInstance(CIMClient cimClient, CIM_REDDropperService dataInstance, boolean notifyDifferencies){ 
		...
		CIM_REDDropperService newDataInstance = null;
		try {
			newDataInstance = CIM_REDDropperServiceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_REDDropperService modifyInstance(CIMClient cimClient, CIM_REDDropperService dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_REDDropperServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_REDDropperService dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_REDDropperService dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_REDDropperService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_REDDropperService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_REDDropperService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_REDDropperService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_REDDropperService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_MaxQueueThreshold(UnsignedInt32 MaxQueueThreshold) {
		return true;
	}

	public static boolean isValid_MinQueueThreshold(UnsignedInt32 MinQueueThreshold) {
		return true;
	}

	public static boolean isValid_StartProbability(UnsignedInt32 StartProbability) {
		return true;
	}

	public static boolean isValid_StopProbability(UnsignedInt32 StopProbability) {
		return true;
	}

	public static boolean isValid_ThresholdUnits(UnsignedInt16 ThresholdUnits) {
		return true;
	}
