...
public final class PRS_AddressHelper {

	public final static String CIM_CLASS_NAME = "PRS_Address";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<PRS_Address.Java_Package_List.size(); i++) {
						if (!((String)(PRS_Address.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(PRS_Address.Java_Package_List.get(i))).endsWith(".")) {
							PRS_Address.Java_Package_List.setElementAt((String)(PRS_Address.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Address.");
						resultArrayList.add(new PRS_Address(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Address.");
						resultArrayList.add(new PRS_Address(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Address.");
						resultArrayList.add(new PRS_Address(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Address getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_AddressHelper.getInstance(cimClient, new PRS_Address(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Address getInstance(CIMClient cimClient, PRS_Address dataInstance) {
		...
		return PRS_AddressHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Address getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Address dataInstance = null;
		try {
			dataInstance = new PRS_Address(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Address createInstance(CIMClient cimClient, PRS_Address dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Address newDataInstance = null;
		try {
			newDataInstance = PRS_AddressHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Address modifyInstance(CIMClient cimClient, PRS_Address dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_AddressHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Address dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Address dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Address.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Address.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Address.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Address.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Address.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Address1(String Address1) {
		return true;
	}

	public static boolean isValid_Address2(String Address2) {
		return true;
	}

	public static boolean isValid_Address3(String Address3) {
		return true;
	}

	public static boolean isValid_Address4(String Address4) {
		return true;
	}

	public static boolean isValid_AddressType(UnsignedInt8 AddressType) {
		return true;
	}

	public static boolean isValid_City(String City) {
		return true;
	}

	public static boolean isValid_Country(String Country) {
		return true;
	}
	...
	public static boolean isValid_Region(String Region) {
		return true;
	}

	public static boolean isValid_TimeZone(String TimeZone) {
		return true;
	}
