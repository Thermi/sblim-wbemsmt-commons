...
public final class CIM_PhysicalMemoryHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_PhysicalMemory.");
						resultArrayList.add(new CIM_PhysicalMemory(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_PhysicalMemory.");
						resultArrayList.add(new CIM_PhysicalMemory(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_PhysicalMemory.");
						resultArrayList.add(new CIM_PhysicalMemory(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_PhysicalMemory getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_PhysicalMemoryHelper.getInstance(cimClient, new CIM_PhysicalMemory(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PhysicalMemory getInstance(CIMClient cimClient, CIM_PhysicalMemory dataInstance) {
		...
		return CIM_PhysicalMemoryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PhysicalMemory getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_PhysicalMemory dataInstance = null;
		try {
			dataInstance = new CIM_PhysicalMemory(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_PhysicalMemory.getPackages();
	...
	*/	
	public final static CIM_PhysicalMemory createInstance(CIMClient cimClient, CIM_PhysicalMemory dataInstance, boolean notifyDifferencies){ 
		...
		CIM_PhysicalMemory newDataInstance = null;
		try {
			newDataInstance = CIM_PhysicalMemoryHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_PhysicalMemory modifyInstance(CIMClient cimClient, CIM_PhysicalMemory dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_PhysicalMemoryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_PhysicalMemory dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_PhysicalMemory dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_PhysicalMemory.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_PhysicalMemory.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_PhysicalMemory.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_PhysicalMemory.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_PhysicalMemory.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_BankLabel(String BankLabel) {
		return true;
	}
	...
	public static boolean isValid_FormFactor(UnsignedInt16 FormFactor) {
		return true;
	}

	public static boolean isValid_InterleavePosition(UnsignedInt32 InterleavePosition) {
		return true;
	}
	...
	public static boolean isValid_PositionInRow(UnsignedInt32 PositionInRow) {
		return true;
	}

	public static boolean isValid_Speed(UnsignedInt32 Speed) {
		return true;
	}

	public static boolean isValid_TotalWidth(UnsignedInt16 TotalWidth) {
		return true;
	}
