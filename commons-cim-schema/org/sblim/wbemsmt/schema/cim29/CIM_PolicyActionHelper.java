...
public final class CIM_PolicyActionHelper {

	public final static String CIM_CLASS_NAME = "CIM_PolicyAction";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_PolicyAction.Java_Package_List.size(); i++) {
						if (!((String)(CIM_PolicyAction.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_PolicyAction.Java_Package_List.get(i))).endsWith(".")) {
							CIM_PolicyAction.Java_Package_List.setElementAt((String)(CIM_PolicyAction.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_PolicyAction.");
						resultArrayList.add(new CIM_PolicyAction(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_PolicyAction.");
						resultArrayList.add(new CIM_PolicyAction(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_PolicyAction.");
						resultArrayList.add(new CIM_PolicyAction(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_PolicyAction getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_PolicyActionHelper.getInstance(cimClient, new CIM_PolicyAction(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PolicyAction getInstance(CIMClient cimClient, CIM_PolicyAction dataInstance) {
		...
		return CIM_PolicyActionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PolicyAction getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_PolicyAction dataInstance = null;
		try {
			dataInstance = new CIM_PolicyAction(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_PolicyAction.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_PolicyAction.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_PolicyAction.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_PolicyAction.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_DoActionLogging(Boolean DoActionLogging) {
		return true;
	}

	public static boolean isValid_PolicyActionName(String PolicyActionName) {
		return true;
	}
