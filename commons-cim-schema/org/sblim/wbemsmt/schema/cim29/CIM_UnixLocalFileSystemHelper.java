/** 
 * CIM_UnixLocalFileSystemHelper.java
 *
...
public final class CIM_UnixLocalFileSystemHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_UnixLocalFileSystem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_UnixLocalFileSystem.Java_Package_List.size(); i++) {
						if (!((String)(CIM_UnixLocalFileSystem.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_UnixLocalFileSystem.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_UnixLocalFileSystem.Java_Package_List.setElementAt((String)(CIM_UnixLocalFileSystem.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_UnixLocalFileSystem.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_UnixLocalFileSystem.");
						resultArrayList.add(new CIM_UnixLocalFileSystem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_UnixLocalFileSystem.");
						resultArrayList.add(new CIM_UnixLocalFileSystem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_UnixLocalFileSystem.");
						resultArrayList.add(new CIM_UnixLocalFileSystem(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_UnixLocalFileSystem getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_UnixLocalFileSystemHelper.getInstance(cimClient, new CIM_UnixLocalFileSystem(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_UnixLocalFileSystem getInstance(CIMClient cimClient, CIM_UnixLocalFileSystem dataInstance) {
		...
		return CIM_UnixLocalFileSystemHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_UnixLocalFileSystem getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_UnixLocalFileSystem dataInstance = null;
		try {
			dataInstance = new CIM_UnixLocalFileSystem(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_UnixLocalFileSystem createInstance(CIMClient cimClient, CIM_UnixLocalFileSystem dataInstance, boolean notifyDifferencies){ 
		...
		CIM_UnixLocalFileSystem newDataInstance = null;
		try {
			newDataInstance = CIM_UnixLocalFileSystemHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_UnixLocalFileSystem modifyInstance(CIMClient cimClient, CIM_UnixLocalFileSystem dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_UnixLocalFileSystemHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_UnixLocalFileSystem dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_UnixLocalFileSystem dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_UnixLocalFileSystem.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_UnixLocalFileSystem.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_UnixLocalFileSystem.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_UnixLocalFileSystem.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_UnixLocalFileSystem.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_FSReservedCapacity(UnsignedInt64 FSReservedCapacity) {
		return true;
	}

	public static boolean isValid_FreeInodes(UnsignedInt64 FreeInodes) {
		return true;
	}

	public static boolean isValid_TotalInodes(UnsignedInt64 TotalInodes) {
		return true;
	}
