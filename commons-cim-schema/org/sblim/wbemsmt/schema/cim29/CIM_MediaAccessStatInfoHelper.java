/** 
 * CIM_MediaAccessStatInfoHelper.java
 *
...
public final class CIM_MediaAccessStatInfoHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_MediaAccessStatInfo(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_MediaAccessStatInfo.Java_Package_List.size(); i++) {
						if (!((String)(CIM_MediaAccessStatInfo.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_MediaAccessStatInfo.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_MediaAccessStatInfo.Java_Package_List.setElementAt((String)(CIM_MediaAccessStatInfo.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_MediaAccessStatInfo.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_MediaAccessStatInfo.");
						resultArrayList.add(new CIM_MediaAccessStatInfo(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_MediaAccessStatInfo.");
						resultArrayList.add(new CIM_MediaAccessStatInfo(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_MediaAccessStatInfo.");
						resultArrayList.add(new CIM_MediaAccessStatInfo(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_MediaAccessStatInfo getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_MediaAccessStatInfoHelper.getInstance(cimClient, new CIM_MediaAccessStatInfo(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_MediaAccessStatInfo getInstance(CIMClient cimClient, CIM_MediaAccessStatInfo dataInstance) {
		...
		return CIM_MediaAccessStatInfoHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_MediaAccessStatInfo getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_MediaAccessStatInfo dataInstance = null;
		try {
			dataInstance = new CIM_MediaAccessStatInfo(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_MediaAccessStatInfo createInstance(CIMClient cimClient, CIM_MediaAccessStatInfo dataInstance, boolean notifyDifferencies){ 
		...
		CIM_MediaAccessStatInfo newDataInstance = null;
		try {
			newDataInstance = CIM_MediaAccessStatInfoHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_MediaAccessStatInfo modifyInstance(CIMClient cimClient, CIM_MediaAccessStatInfo dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_MediaAccessStatInfoHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_MediaAccessStatInfo dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_MediaAccessStatInfo dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_MediaAccessStatInfo.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_MediaAccessStatInfo.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_MediaAccessStatInfo.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_MediaAccessStatInfo.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_MediaAccessStatInfo.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ReadOperations(UnsignedInt64 ReadOperations) {
		return true;
	}

	public static boolean isValid_RecoveredReadOperations(UnsignedInt64 RecoveredReadOperations) {
		return true;
	}

	public static boolean isValid_RecoveredSeekOperations(UnsignedInt64 RecoveredSeekOperations) {
		return true;
	}

	public static boolean isValid_RecoveredWriteOperations(UnsignedInt64 RecoveredWriteOperations) {
		return true;
	}

	public static boolean isValid_UnrecoverableReadOperations(UnsignedInt64 UnrecoverableReadOperations) {
		return true;
	}

	public static boolean isValid_UnrecoverableSeekOperations(UnsignedInt64 UnrecoverableSeekOperations) {
		return true;
	}

	public static boolean isValid_UnrecoverableWriteOperations(UnsignedInt64 UnrecoverableWriteOperations) {
		return true;
	}

	public static boolean isValid_WriteOperations(UnsignedInt64 WriteOperations) {
		return true;
	}
