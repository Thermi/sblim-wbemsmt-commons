...
public final class CIM_BGPServiceHelper {

	public final static String CIM_CLASS_NAME = "CIM_BGPService";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_BGPService.Java_Package_List.size(); i++) {
						if (!((String)(CIM_BGPService.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_BGPService.Java_Package_List.get(i))).endsWith(".")) {
							CIM_BGPService.Java_Package_List.setElementAt((String)(CIM_BGPService.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_BGPService.");
						resultArrayList.add(new CIM_BGPService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_BGPService.");
						resultArrayList.add(new CIM_BGPService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_BGPService.");
						resultArrayList.add(new CIM_BGPService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_BGPService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_BGPServiceHelper.getInstance(cimClient, new CIM_BGPService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_BGPService getInstance(CIMClient cimClient, CIM_BGPService dataInstance) {
		...
		return CIM_BGPServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_BGPService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_BGPService dataInstance = null;
		try {
			dataInstance = new CIM_BGPService(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_BGPService createInstance(CIMClient cimClient, CIM_BGPService dataInstance, boolean notifyDifferencies){ 
		...
		CIM_BGPService newDataInstance = null;
		try {
			newDataInstance = CIM_BGPServiceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_BGPService modifyInstance(CIMClient cimClient, CIM_BGPService dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_BGPServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_BGPService dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_BGPService dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_BGPService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_BGPService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_BGPService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_BGPService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_BGPService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_BGPVersion(UnsignedInt8 BGPVersion) {
		return true;
	}

	public static boolean isValid_LastErrorCode(UnsignedInt16 LastErrorCode) {
		return true;
	}

	public static boolean isValid_LastMessageErrorSubCode(UnsignedInt16 LastMessageErrorSubCode) {
		return true;
	}

	public static boolean isValid_LastOpenErrorSubCode(UnsignedInt16 LastOpenErrorSubCode) {
		return true;
	}

	public static boolean isValid_LastUpdateErrorSubCode(UnsignedInt16 LastUpdateErrorSubCode) {
		return true;
	}
