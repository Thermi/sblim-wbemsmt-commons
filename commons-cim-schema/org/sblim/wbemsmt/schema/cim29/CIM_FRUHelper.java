...
public final class CIM_FRUHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_FRU.");
						resultArrayList.add(new CIM_FRU(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_FRU.");
						resultArrayList.add(new CIM_FRU(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_FRU.");
						resultArrayList.add(new CIM_FRU(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_FRU getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_FRUHelper.getInstance(cimClient, new CIM_FRU(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_FRU getInstance(CIMClient cimClient, CIM_FRU dataInstance) {
		...
		return CIM_FRUHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_FRU getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_FRU dataInstance = null;
		try {
			dataInstance = new CIM_FRU(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_FRU createInstance(CIMClient cimClient, CIM_FRU dataInstance, boolean notifyDifferencies){ 
		...
		CIM_FRU newDataInstance = null;
		try {
			newDataInstance = CIM_FRUHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_FRU modifyInstance(CIMClient cimClient, CIM_FRU dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_FRUHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_FRU dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_FRU dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_FRU.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_FRU.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_FRU.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_FRU.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_FRU.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CustomerReplaceable(Boolean CustomerReplaceable) {
		return true;
	}
	...
	public static boolean isValid_FRUNumber(String FRUNumber) {
		return true;
	}

	public static boolean isValid_IdentifyingNumber(String IdentifyingNumber) {
		return true;
	}
	...
	public static boolean isValid_RevisionLevel(String RevisionLevel) {
		return true;
	}
