...
public final class CIM_CollectionOfMSEsHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					Class clazz = CIM_CollectionOfMSEsHelper.findClass(cimClient, cimInstance);
					if (clazz == null) {
	...
	*/	
	public final static CIM_CollectionOfMSEs getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_CollectionOfMSEsHelper.getInstance(cimClient, new CIM_CollectionOfMSEs(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_CollectionOfMSEs getInstance(CIMClient cimClient, CIM_CollectionOfMSEs dataInstance) {
		...
		return CIM_CollectionOfMSEsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_CollectionOfMSEs getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_CollectionOfMSEs(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_CollectionOfMSEs.getPackages();
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_CollectionOfMSEs.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_CollectionOfMSEs.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_CollectionOfMSEs.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_CollectionOfMSEs.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
