...
public final class CIM_NFSHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_NFS(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_NFS.Java_Package_List.size(); i++) {
						if (!((String)(CIM_NFS.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_NFS.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_NFS.Java_Package_List.setElementAt((String)(CIM_NFS.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_NFS.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_NFS.");
						resultArrayList.add(new CIM_NFS(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_NFS.");
						resultArrayList.add(new CIM_NFS(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_NFS.");
						resultArrayList.add(new CIM_NFS(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_NFS getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_NFSHelper.getInstance(cimClient, new CIM_NFS(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_NFS getInstance(CIMClient cimClient, CIM_NFS dataInstance) {
		...
		return CIM_NFSHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_NFS getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_NFS dataInstance = null;
		try {
			dataInstance = new CIM_NFS(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_NFS createInstance(CIMClient cimClient, CIM_NFS dataInstance, boolean notifyDifferencies){ 
		...
		CIM_NFS newDataInstance = null;
		try {
			newDataInstance = CIM_NFSHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_NFS modifyInstance(CIMClient cimClient, CIM_NFS dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_NFSHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_NFS dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_NFS dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_NFS.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_NFS.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_NFS.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_NFS.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_NFS.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AttributeCaching(Boolean AttributeCaching) {
		return true;
	}

	public static boolean isValid_AttributeCachingForDirectoriesMax(UnsignedInt16 AttributeCachingForDirectoriesMax) {
		return true;
	}

	public static boolean isValid_AttributeCachingForDirectoriesMin(UnsignedInt16 AttributeCachingForDirectoriesMin) {
		return true;
	}

	public static boolean isValid_AttributeCachingForRegularFilesMax(UnsignedInt16 AttributeCachingForRegularFilesMax) {
		return true;
	}

	public static boolean isValid_AttributeCachingForRegularFilesMin(UnsignedInt16 AttributeCachingForRegularFilesMin) {
		return true;
	}

	public static boolean isValid_ForegroundMount(Boolean ForegroundMount) {
		return true;
	}

	public static boolean isValid_HardMount(Boolean HardMount) {
		return true;
	}

	public static boolean isValid_Interrupt(Boolean Interrupt) {
		return true;
	}

	public static boolean isValid_MountFailureRetries(UnsignedInt16 MountFailureRetries) {
		return true;
	}

	public static boolean isValid_ReadBufferSize(UnsignedInt64 ReadBufferSize) {
		return true;
	}

	public static boolean isValid_RetransmissionAttempts(UnsignedInt16 RetransmissionAttempts) {
		return true;
	}

	public static boolean isValid_RetransmissionTimeout(UnsignedInt32 RetransmissionTimeout) {
		return true;
	}

	public static boolean isValid_ServerCommunicationPort(UnsignedInt32 ServerCommunicationPort) {
		return true;
	}

	public static boolean isValid_WriteBufferSize(UnsignedInt64 WriteBufferSize) {
		return true;
	}
