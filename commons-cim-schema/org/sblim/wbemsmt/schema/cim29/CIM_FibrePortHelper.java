...
public final class CIM_FibrePortHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_FibrePort.");
						resultArrayList.add(new CIM_FibrePort(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_FibrePort.");
						resultArrayList.add(new CIM_FibrePort(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_FibrePort.");
						resultArrayList.add(new CIM_FibrePort(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_FibrePort getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_FibrePortHelper.getInstance(cimClient, new CIM_FibrePort(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_FibrePort getInstance(CIMClient cimClient, CIM_FibrePort dataInstance) {
		...
		return CIM_FibrePortHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_FibrePort getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_FibrePort dataInstance = null;
		try {
			dataInstance = new CIM_FibrePort(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_FibrePort.getPackages();
	...
	*/	
	public final static CIM_FibrePort createInstance(CIMClient cimClient, CIM_FibrePort dataInstance, boolean notifyDifferencies){ 
		...
		CIM_FibrePort newDataInstance = null;
		try {
			newDataInstance = CIM_FibrePortHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_FibrePort modifyInstance(CIMClient cimClient, CIM_FibrePort dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_FibrePortHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_FibrePort dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_FibrePort dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_FibrePort.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_FibrePort.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_FibrePort.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_FibrePort.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_FibrePort.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AddressIdentifier(UnsignedInt32 AddressIdentifier) {
		return true;
	}

	public static boolean isValid_AliasAddresses(UnsignedInt32[] AliasAddresses) {
		return true;
	}

	public static boolean isValid_BypassedState(UnsignedInt16 BypassedState) {
		return true;
	}
	...
	public static boolean isValid_ConnectedMedia(UnsignedInt16 ConnectedMedia) {
		return true;
	}

	public static boolean isValid_CurrentPortType(UnsignedInt16 CurrentPortType) {
		return true;
	}

	public static boolean isValid_CurrentVersion(String CurrentVersion) {
		return true;
	}

	public static boolean isValid_ElasticityBufferOverruns(UnsignedInt64 ElasticityBufferOverruns) {
		return true;
	}

	public static boolean isValid_ElasticityBufferUnderruns(UnsignedInt64 ElasticityBufferUnderruns) {
		return true;
	}

	public static boolean isValid_EnabledPortTypes(UnsignedInt16[] EnabledPortTypes) {
		return true;
	}

	public static boolean isValid_EnabledVersions(String[] EnabledVersions) {
		return true;
	}
	...
	public static boolean isValid_PortTypeVersions(String[] PortTypeVersions) {
		return true;
	}

	public static boolean isValid_ReceiverTransmitterTimeout(UnsignedInt64 ReceiverTransmitterTimeout) {
		return true;
	}

	public static boolean isValid_SupportedPortTypes(UnsignedInt16[] SupportedPortTypes) {
		return true;
	}
