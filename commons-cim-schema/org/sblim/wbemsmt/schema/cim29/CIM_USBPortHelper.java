...
public final class CIM_USBPortHelper {

	public final static String CIM_CLASS_NAME = "CIM_USBPort";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_USBPort.Java_Package_List.size(); i++) {
						if (!((String)(CIM_USBPort.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_USBPort.Java_Package_List.get(i))).endsWith(".")) {
							CIM_USBPort.Java_Package_List.setElementAt((String)(CIM_USBPort.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_USBPort.");
						resultArrayList.add(new CIM_USBPort(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_USBPort.");
						resultArrayList.add(new CIM_USBPort(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_USBPort.");
						resultArrayList.add(new CIM_USBPort(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_USBPort getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_USBPortHelper.getInstance(cimClient, new CIM_USBPort(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_USBPort getInstance(CIMClient cimClient, CIM_USBPort dataInstance) {
		...
		return CIM_USBPortHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_USBPort getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_USBPort dataInstance = null;
		try {
			dataInstance = new CIM_USBPort(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_USBPort createInstance(CIMClient cimClient, CIM_USBPort dataInstance, boolean notifyDifferencies){ 
		...
		CIM_USBPort newDataInstance = null;
		try {
			newDataInstance = CIM_USBPortHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_USBPort modifyInstance(CIMClient cimClient, CIM_USBPort dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_USBPortHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_USBPort dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_USBPort dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_USBPort.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_USBPort.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_USBPort.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_USBPort.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_USBPort.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Overcurrent(Boolean Overcurrent) {
		return true;
	}

	public static boolean isValid_Power(UnsignedInt16 Power) {
		return true;
	}
	...
	public static boolean isValid_Suspended(Boolean Suspended) {
		return true;
	}
