/** 
 * CIM_CompositeExtentHelper.java
 *
...
public final class CIM_CompositeExtentHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_CompositeExtent.");
						resultArrayList.add(new CIM_CompositeExtent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_CompositeExtent.");
						resultArrayList.add(new CIM_CompositeExtent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_CompositeExtent.");
						resultArrayList.add(new CIM_CompositeExtent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_CompositeExtent getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_CompositeExtentHelper.getInstance(cimClient, new CIM_CompositeExtent(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_CompositeExtent getInstance(CIMClient cimClient, CIM_CompositeExtent dataInstance) {
		...
		return CIM_CompositeExtentHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_CompositeExtent getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_CompositeExtent dataInstance = null;
		try {
			dataInstance = new CIM_CompositeExtent(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_CompositeExtent.getPackages();
	...
	*/	
	public final static CIM_CompositeExtent createInstance(CIMClient cimClient, CIM_CompositeExtent dataInstance, boolean notifyDifferencies){ 
		...
		CIM_CompositeExtent newDataInstance = null;
		try {
			newDataInstance = CIM_CompositeExtentHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_CompositeExtent modifyInstance(CIMClient cimClient, CIM_CompositeExtent dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_CompositeExtentHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_CompositeExtent dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_CompositeExtent dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_CompositeExtent.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_CompositeExtent.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_CompositeExtent.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_CompositeExtent.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_CompositeExtent.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ExtentInterleaveDepth(UnsignedInt64 ExtentInterleaveDepth) {
		return true;
	}

	public static boolean isValid_ExtentStripeLength(UnsignedInt64 ExtentStripeLength) {
		return true;
	}
