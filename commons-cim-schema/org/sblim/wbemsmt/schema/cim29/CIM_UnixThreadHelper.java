...
public final class CIM_UnixThreadHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_UnixThread(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_UnixThread.Java_Package_List.size(); i++) {
						if (!((String)(CIM_UnixThread.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_UnixThread.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_UnixThread.Java_Package_List.setElementAt((String)(CIM_UnixThread.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_UnixThread.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_UnixThread.");
						resultArrayList.add(new CIM_UnixThread(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_UnixThread.");
						resultArrayList.add(new CIM_UnixThread(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_UnixThread.");
						resultArrayList.add(new CIM_UnixThread(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_UnixThread getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_UnixThreadHelper.getInstance(cimClient, new CIM_UnixThread(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_UnixThread getInstance(CIMClient cimClient, CIM_UnixThread dataInstance) {
		...
		return CIM_UnixThreadHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_UnixThread getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_UnixThread dataInstance = null;
		try {
			dataInstance = new CIM_UnixThread(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_UnixThread createInstance(CIMClient cimClient, CIM_UnixThread dataInstance, boolean notifyDifferencies){ 
		...
		CIM_UnixThread newDataInstance = null;
		try {
			newDataInstance = CIM_UnixThreadHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_UnixThread modifyInstance(CIMClient cimClient, CIM_UnixThread dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_UnixThreadHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_UnixThread dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_UnixThread dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_UnixThread.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_UnixThread.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_UnixThread.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_UnixThread.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_UnixThread.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CancelState(UnsignedInt16 CancelState) {
		return true;
	}

	public static boolean isValid_CancelType(UnsignedInt16 CancelType) {
		return true;
	}

	public static boolean isValid_ConcurrencyLevel(UnsignedInt64 ConcurrencyLevel) {
		return true;
	}

	public static boolean isValid_ContentionScope(UnsignedInt16 ContentionScope) {
		return true;
	}

	public static boolean isValid_DetachState(UnsignedInt16 DetachState) {
		return true;
	}

	public static boolean isValid_GuardSize(String GuardSize) {
		return true;
	}

	public static boolean isValid_InheritSched(UnsignedInt16 InheritSched) {
		return true;
	}

	public static boolean isValid_OtherSchedPolicy(String OtherSchedPolicy) {
		return true;
	}

	public static boolean isValid_SchedPolicy(UnsignedInt16 SchedPolicy) {
		return true;
	}

	public static boolean isValid_StackSize(String StackSize) {
		return true;
	}
