...
public final class CIM_MeterServiceHelper {

	public final static String CIM_CLASS_NAME = "CIM_MeterService";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_MeterService.Java_Package_List.size(); i++) {
						if (!((String)(CIM_MeterService.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_MeterService.Java_Package_List.get(i))).endsWith(".")) {
							CIM_MeterService.Java_Package_List.setElementAt((String)(CIM_MeterService.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_MeterService.");
						resultArrayList.add(new CIM_MeterService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_MeterService.");
						resultArrayList.add(new CIM_MeterService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_MeterService.");
						resultArrayList.add(new CIM_MeterService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_MeterService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_MeterServiceHelper.getInstance(cimClient, new CIM_MeterService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_MeterService getInstance(CIMClient cimClient, CIM_MeterService dataInstance) {
		...
		return CIM_MeterServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_MeterService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_MeterService dataInstance = null;
		try {
			dataInstance = new CIM_MeterService(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_MeterService createInstance(CIMClient cimClient, CIM_MeterService dataInstance, boolean notifyDifferencies){ 
		...
		CIM_MeterService newDataInstance = null;
		try {
			newDataInstance = CIM_MeterServiceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_MeterService modifyInstance(CIMClient cimClient, CIM_MeterService dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_MeterServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_MeterService dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_MeterService dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_MeterService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_MeterService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_MeterService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_MeterService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_MeterService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ConformanceLevels(UnsignedInt16 ConformanceLevels) {
		return true;
	}

	public static boolean isValid_MeterType(UnsignedInt16 MeterType) {
		return true;
	}

	public static boolean isValid_OtherMeterType(String OtherMeterType) {
		return true;
	}
