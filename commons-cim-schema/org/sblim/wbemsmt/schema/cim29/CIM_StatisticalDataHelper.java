/** 
 * CIM_StatisticalDataHelper.java
 *
...
public final class CIM_StatisticalDataHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_StatisticalData.Java_Package_List.size(); i++) {
						if (!((String)(CIM_StatisticalData.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_StatisticalData.Java_Package_List.get(i))).endsWith(".")) {
							CIM_StatisticalData.Java_Package_List.setElementAt((String)(CIM_StatisticalData.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_StatisticalData.");
						resultArrayList.add(new CIM_StatisticalData(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_StatisticalData.");
						resultArrayList.add(new CIM_StatisticalData(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_StatisticalData.");
						resultArrayList.add(new CIM_StatisticalData(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_StatisticalData getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_StatisticalDataHelper.getInstance(cimClient, new CIM_StatisticalData(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_StatisticalData getInstance(CIMClient cimClient, CIM_StatisticalData dataInstance) {
		...
		return CIM_StatisticalDataHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_StatisticalData getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_StatisticalData dataInstance = null;
		try {
			dataInstance = new CIM_StatisticalData(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_StatisticalData.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_StatisticalData.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_StatisticalData.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_StatisticalData.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_StatisticTime(Calendar StatisticTime) {
		return true;
	}
