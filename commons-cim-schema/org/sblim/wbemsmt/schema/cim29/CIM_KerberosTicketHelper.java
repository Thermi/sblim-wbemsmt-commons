...
public final class CIM_KerberosTicketHelper {

	public final static String CIM_CLASS_NAME = "CIM_KerberosTicket";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_KerberosTicket.Java_Package_List.size(); i++) {
						if (!((String)(CIM_KerberosTicket.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_KerberosTicket.Java_Package_List.get(i))).endsWith(".")) {
							CIM_KerberosTicket.Java_Package_List.setElementAt((String)(CIM_KerberosTicket.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_KerberosTicket.");
						resultArrayList.add(new CIM_KerberosTicket(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_KerberosTicket.");
						resultArrayList.add(new CIM_KerberosTicket(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_KerberosTicket.");
						resultArrayList.add(new CIM_KerberosTicket(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_KerberosTicket getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_KerberosTicketHelper.getInstance(cimClient, new CIM_KerberosTicket(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_KerberosTicket getInstance(CIMClient cimClient, CIM_KerberosTicket dataInstance) {
		...
		return CIM_KerberosTicketHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_KerberosTicket getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_KerberosTicket dataInstance = null;
		try {
			dataInstance = new CIM_KerberosTicket(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_KerberosTicket createInstance(CIMClient cimClient, CIM_KerberosTicket dataInstance, boolean notifyDifferencies){ 
		...
		CIM_KerberosTicket newDataInstance = null;
		try {
			newDataInstance = CIM_KerberosTicketHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_KerberosTicket modifyInstance(CIMClient cimClient, CIM_KerberosTicket dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_KerberosTicketHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_KerberosTicket dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_KerberosTicket dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_KerberosTicket.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_KerberosTicket.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_KerberosTicket.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_KerberosTicket.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_KerberosTicket.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AccessesService(String AccessesService) {
		return true;
	}

	public static boolean isValid_RemoteID(String RemoteID) {
		return true;
	}
	...
	public static boolean isValid_TicketType(UnsignedInt16 TicketType) {
		return true;
	}
