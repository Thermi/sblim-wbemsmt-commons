 ...
 *
 *
 * Description: Database services perform tasks for a database. Examples of tasks that are performed by a database service include providing user access to the database, performing data and metadata manipulation, database administration, and maintenance operations. 
One or more database services can perform tasks for a database. The database services can reside on the same computer system as the database or on different computer systems. Database services may be implemented as one or more processes.
 * 
 */
...
public class CIM_DatabaseService extends CIM_Service  {
	...
	*/
	...
	/**
	*	The LastStatusChangeTime property is deprecated in lieu of the TimeOfLastStateChange property that is inherited from CIM_EnabledLogicalElement. The date and time when the status of the database service last changed.
	*/
	public final static String CIM_PROPERTY_LASTSTATUSCHANGETIME = "LastStatusChangeTime"; //$NON-NLS-1$
	/**
	*	This property contains the operational status of the database service. OK means that the database service is operational and available for general use. Stopped means that the service is unavailable and cannot be used. Service implies an administrative state of unavailability. Degraded means that the database service is operating at a less than optimal level. Starting means that the database service is in the process of becoming operational. 
