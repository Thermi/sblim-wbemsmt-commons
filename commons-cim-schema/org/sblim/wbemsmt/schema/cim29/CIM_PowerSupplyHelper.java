...
public final class CIM_PowerSupplyHelper {

	public final static String CIM_CLASS_NAME = "CIM_PowerSupply";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_PowerSupply.Java_Package_List.size(); i++) {
						if (!((String)(CIM_PowerSupply.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_PowerSupply.Java_Package_List.get(i))).endsWith(".")) {
							CIM_PowerSupply.Java_Package_List.setElementAt((String)(CIM_PowerSupply.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_PowerSupply.");
						resultArrayList.add(new CIM_PowerSupply(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_PowerSupply.");
						resultArrayList.add(new CIM_PowerSupply(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_PowerSupply.");
						resultArrayList.add(new CIM_PowerSupply(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_PowerSupply getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_PowerSupplyHelper.getInstance(cimClient, new CIM_PowerSupply(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PowerSupply getInstance(CIMClient cimClient, CIM_PowerSupply dataInstance) {
		...
		return CIM_PowerSupplyHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PowerSupply getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_PowerSupply dataInstance = null;
		try {
			dataInstance = new CIM_PowerSupply(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_PowerSupply createInstance(CIMClient cimClient, CIM_PowerSupply dataInstance, boolean notifyDifferencies){ 
		...
		CIM_PowerSupply newDataInstance = null;
		try {
			newDataInstance = CIM_PowerSupplyHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_PowerSupply modifyInstance(CIMClient cimClient, CIM_PowerSupply dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_PowerSupplyHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_PowerSupply dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_PowerSupply dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_PowerSupply.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_PowerSupply.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_PowerSupply.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_PowerSupply.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_PowerSupply.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ActiveInputVoltage(UnsignedInt16 ActiveInputVoltage) {
		return true;
	}

	public static boolean isValid_IsSwitchingSupply(Boolean IsSwitchingSupply) {
		return true;
	}

	public static boolean isValid_Range1InputFrequencyHigh(UnsignedInt32 Range1InputFrequencyHigh) {
		return true;
	}

	public static boolean isValid_Range1InputFrequencyLow(UnsignedInt32 Range1InputFrequencyLow) {
		return true;
	}
	...
	public static boolean isValid_Range2InputFrequencyHigh(UnsignedInt32 Range2InputFrequencyHigh) {
		return true;
	}

	public static boolean isValid_Range2InputFrequencyLow(UnsignedInt32 Range2InputFrequencyLow) {
		return true;
	}
	...
	public static boolean isValid_TotalOutputPower(UnsignedInt32 TotalOutputPower) {
		return true;
	}

	public static boolean isValid_TypeOfRangeSwitching(UnsignedInt16 TypeOfRangeSwitching) {
		return true;
	}
