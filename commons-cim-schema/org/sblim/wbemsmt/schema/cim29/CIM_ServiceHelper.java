...
public final class CIM_ServiceHelper {
	...
	*/	
	public final static CIM_Service getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ServiceHelper.getInstance(cimClient, new CIM_Service(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Service getInstance(CIMClient cimClient, CIM_Service dataInstance) {
		...
		return CIM_ServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Service getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_Service(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Service.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Service.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Service.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Service.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_StartMode(String StartMode) {
		return true;
	}
