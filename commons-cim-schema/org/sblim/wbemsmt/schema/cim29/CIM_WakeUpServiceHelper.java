...
public final class CIM_WakeUpServiceHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_WakeUpService.");
						resultArrayList.add(new CIM_WakeUpService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_WakeUpService.");
						resultArrayList.add(new CIM_WakeUpService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_WakeUpService.");
						resultArrayList.add(new CIM_WakeUpService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_WakeUpService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_WakeUpServiceHelper.getInstance(cimClient, new CIM_WakeUpService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_WakeUpService getInstance(CIMClient cimClient, CIM_WakeUpService dataInstance) {
		...
		return CIM_WakeUpServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_WakeUpService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_WakeUpService dataInstance = null;
		try {
			dataInstance = new CIM_WakeUpService(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_WakeUpService.getPackages();
	...
	*/	
	public final static CIM_WakeUpService createInstance(CIMClient cimClient, CIM_WakeUpService dataInstance, boolean notifyDifferencies){ 
		...
		CIM_WakeUpService newDataInstance = null;
		try {
			newDataInstance = CIM_WakeUpServiceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_WakeUpService modifyInstance(CIMClient cimClient, CIM_WakeUpService dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_WakeUpServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_WakeUpService dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_WakeUpService dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_WakeUpService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_WakeUpService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_WakeUpService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_WakeUpService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_WakeUpService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_FilterData(UnsignedInt8[] FilterData) {
		return true;
	}

	public static boolean isValid_FilterType(UnsignedInt16 FilterType) {
		return true;
	}

	public static boolean isValid_OtherFilterTypeDescription(String OtherFilterTypeDescription) {
		return true;
	}

	public static boolean isValid_OtherWakeUpTypeDescription(String OtherWakeUpTypeDescription) {
		return true;
	}
