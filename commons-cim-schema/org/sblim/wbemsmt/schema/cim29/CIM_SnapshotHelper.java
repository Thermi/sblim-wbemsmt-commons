...
public final class CIM_SnapshotHelper {

	public final static String CIM_CLASS_NAME = "CIM_Snapshot";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_Snapshot.Java_Package_List.size(); i++) {
						if (!((String)(CIM_Snapshot.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_Snapshot.Java_Package_List.get(i))).endsWith(".")) {
							CIM_Snapshot.Java_Package_List.setElementAt((String)(CIM_Snapshot.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_Snapshot.");
						resultArrayList.add(new CIM_Snapshot(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_Snapshot.");
						resultArrayList.add(new CIM_Snapshot(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_Snapshot.");
						resultArrayList.add(new CIM_Snapshot(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_Snapshot getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_SnapshotHelper.getInstance(cimClient, new CIM_Snapshot(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Snapshot getInstance(CIMClient cimClient, CIM_Snapshot dataInstance) {
		...
		return CIM_SnapshotHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Snapshot getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_Snapshot dataInstance = null;
		try {
			dataInstance = new CIM_Snapshot(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Snapshot createInstance(CIMClient cimClient, CIM_Snapshot dataInstance, boolean notifyDifferencies){ 
		...
		CIM_Snapshot newDataInstance = null;
		try {
			newDataInstance = CIM_SnapshotHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Snapshot modifyInstance(CIMClient cimClient, CIM_Snapshot dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_SnapshotHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_Snapshot dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_Snapshot dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_Snapshot.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Snapshot.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Snapshot.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Snapshot.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Snapshot.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_IsSynced(Boolean IsSynced) {
		return true;
	}

	public static boolean isValid_SyncTime(Calendar SyncTime) {
		return true;
	}
