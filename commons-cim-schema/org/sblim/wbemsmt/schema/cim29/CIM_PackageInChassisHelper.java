/** 
 * CIM_PackageInChassisHelper.java
 *
...
public final class CIM_PackageInChassisHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_PackageInChassis(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_PackageInChassis.Java_Package_List.size(); i++) {
						if (!((String)(CIM_PackageInChassis.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_PackageInChassis.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_PackageInChassis.Java_Package_List.setElementAt((String)(CIM_PackageInChassis.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_PackageInChassis.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_PackageInChassis.");
						resultArrayList.add(new CIM_PackageInChassis(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_PackageInChassis.");
						resultArrayList.add(new CIM_PackageInChassis(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_PackageInChassis.");
						resultArrayList.add(new CIM_PackageInChassis(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_PackageInChassis getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_PackageInChassisHelper.getInstance(cimClient, new CIM_PackageInChassis(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PackageInChassis getInstance(CIMClient cimClient, CIM_PackageInChassis dataInstance) {
		...
		return CIM_PackageInChassisHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PackageInChassis getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_PackageInChassis dataInstance = null;
		try {
			dataInstance = new CIM_PackageInChassis(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_PackageInChassis createInstance(CIMClient cimClient, CIM_PackageInChassis dataInstance, boolean notifyDifferencies){ 
		...
		CIM_PackageInChassis newDataInstance = null;
		try {
			newDataInstance = CIM_PackageInChassisHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_PackageInChassis modifyInstance(CIMClient cimClient, CIM_PackageInChassis dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_PackageInChassisHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_PackageInChassis dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_PackageInChassis dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_PackageInChassis.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_PackageInChassis.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_PackageInChassis.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_PackageInChassis.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_PackageInChassis.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CIM_Chassis(CIM_Chassis CIM_Chassis) {
		return true;
	}
