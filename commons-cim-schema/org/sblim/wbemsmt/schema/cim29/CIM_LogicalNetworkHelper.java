...
public final class CIM_LogicalNetworkHelper {

	public final static String CIM_CLASS_NAME = "CIM_LogicalNetwork";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_LogicalNetwork.Java_Package_List.size(); i++) {
						if (!((String)(CIM_LogicalNetwork.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_LogicalNetwork.Java_Package_List.get(i))).endsWith(".")) {
							CIM_LogicalNetwork.Java_Package_List.setElementAt((String)(CIM_LogicalNetwork.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_LogicalNetwork.");
						resultArrayList.add(new CIM_LogicalNetwork(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_LogicalNetwork.");
						resultArrayList.add(new CIM_LogicalNetwork(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_LogicalNetwork.");
						resultArrayList.add(new CIM_LogicalNetwork(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_LogicalNetwork getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_LogicalNetworkHelper.getInstance(cimClient, new CIM_LogicalNetwork(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LogicalNetwork getInstance(CIMClient cimClient, CIM_LogicalNetwork dataInstance) {
		...
		return CIM_LogicalNetworkHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LogicalNetwork getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_LogicalNetwork dataInstance = null;
		try {
			dataInstance = new CIM_LogicalNetwork(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_LogicalNetwork createInstance(CIMClient cimClient, CIM_LogicalNetwork dataInstance, boolean notifyDifferencies){ 
		...
		CIM_LogicalNetwork newDataInstance = null;
		try {
			newDataInstance = CIM_LogicalNetworkHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_LogicalNetwork modifyInstance(CIMClient cimClient, CIM_LogicalNetwork dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_LogicalNetworkHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_LogicalNetwork dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_LogicalNetwork dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_LogicalNetwork.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_LogicalNetwork.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_LogicalNetwork.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_LogicalNetwork.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_LogicalNetwork.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_NetworkType(UnsignedInt16 NetworkType) {
		return true;
	}
