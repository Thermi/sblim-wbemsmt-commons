...
public final class CIM_ProductHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_Product.Java_Package_List.size(); i++) {
						if (!((String)(CIM_Product.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_Product.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_Product.Java_Package_List.setElementAt((String)(CIM_Product.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_Product.");
						resultArrayList.add(new CIM_Product(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_Product.");
						resultArrayList.add(new CIM_Product(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_Product.");
						resultArrayList.add(new CIM_Product(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_Product getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ProductHelper.getInstance(cimClient, new CIM_Product(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Product getInstance(CIMClient cimClient, CIM_Product dataInstance) {
		...
		return CIM_ProductHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Product getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_Product dataInstance = null;
		try {
			dataInstance = new CIM_Product(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Product createInstance(CIMClient cimClient, CIM_Product dataInstance, boolean notifyDifferencies){ 
		...
		CIM_Product newDataInstance = null;
		try {
			newDataInstance = CIM_ProductHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Product modifyInstance(CIMClient cimClient, CIM_Product dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_ProductHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_Product dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_Product dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_Product.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Product.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Product.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Product.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Product.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_SKUNumber(String SKUNumber) {
		return true;
	}
	...
	public static boolean isValid_WarrantyDuration(UnsignedInt32 WarrantyDuration) {
		return true;
	}

	public static boolean isValid_WarrantyStartDate(Calendar WarrantyStartDate) {
		return true;
	}
