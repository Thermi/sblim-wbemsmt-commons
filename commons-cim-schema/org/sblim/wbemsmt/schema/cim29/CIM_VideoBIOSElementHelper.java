/** 
 * CIM_VideoBIOSElementHelper.java
 *
...
public final class CIM_VideoBIOSElementHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_VideoBIOSElement.");
						resultArrayList.add(new CIM_VideoBIOSElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_VideoBIOSElement.");
						resultArrayList.add(new CIM_VideoBIOSElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_VideoBIOSElement.");
						resultArrayList.add(new CIM_VideoBIOSElement(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_VideoBIOSElement getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_VideoBIOSElementHelper.getInstance(cimClient, new CIM_VideoBIOSElement(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_VideoBIOSElement getInstance(CIMClient cimClient, CIM_VideoBIOSElement dataInstance) {
		...
		return CIM_VideoBIOSElementHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_VideoBIOSElement getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_VideoBIOSElement dataInstance = null;
		try {
			dataInstance = new CIM_VideoBIOSElement(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_VideoBIOSElement.getPackages();
	...
	*/	
	public final static CIM_VideoBIOSElement createInstance(CIMClient cimClient, CIM_VideoBIOSElement dataInstance, boolean notifyDifferencies){ 
		...
		CIM_VideoBIOSElement newDataInstance = null;
		try {
			newDataInstance = CIM_VideoBIOSElementHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_VideoBIOSElement modifyInstance(CIMClient cimClient, CIM_VideoBIOSElement dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_VideoBIOSElementHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_VideoBIOSElement dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_VideoBIOSElement dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_VideoBIOSElement.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_VideoBIOSElement.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_VideoBIOSElement.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_VideoBIOSElement.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_VideoBIOSElement.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_IsShadowed(Boolean IsShadowed) {
		return true;
	}
