...
public final class CIM_ScopedSettingDataHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					Class clazz = CIM_ScopedSettingDataHelper.findClass(cimClient, cimInstance);
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_ScopedSettingData.");
						resultArrayList.add(new CIM_ScopedSettingData(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_ScopedSettingData.");
						resultArrayList.add(new CIM_ScopedSettingData(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_ScopedSettingData.");
						resultArrayList.add(new CIM_ScopedSettingData(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_ScopedSettingData getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ScopedSettingDataHelper.getInstance(cimClient, new CIM_ScopedSettingData(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ScopedSettingData getInstance(CIMClient cimClient, CIM_ScopedSettingData dataInstance) {
		...
		return CIM_ScopedSettingDataHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ScopedSettingData getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_ScopedSettingData(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_ScopedSettingData.getPackages();
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_ScopedSettingData.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_ScopedSettingData.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_ScopedSettingData.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_ScopedSettingData.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
