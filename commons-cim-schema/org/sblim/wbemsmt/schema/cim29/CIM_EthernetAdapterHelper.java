/** 
 * CIM_EthernetAdapterHelper.java
 *
...
public final class CIM_EthernetAdapterHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_EthernetAdapter(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_EthernetAdapter.Java_Package_List.size(); i++) {
						if (!((String)(CIM_EthernetAdapter.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_EthernetAdapter.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_EthernetAdapter.Java_Package_List.setElementAt((String)(CIM_EthernetAdapter.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_EthernetAdapter.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_EthernetAdapter.");
						resultArrayList.add(new CIM_EthernetAdapter(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_EthernetAdapter.");
						resultArrayList.add(new CIM_EthernetAdapter(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_EthernetAdapter.");
						resultArrayList.add(new CIM_EthernetAdapter(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_EthernetAdapter getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_EthernetAdapterHelper.getInstance(cimClient, new CIM_EthernetAdapter(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_EthernetAdapter getInstance(CIMClient cimClient, CIM_EthernetAdapter dataInstance) {
		...
		return CIM_EthernetAdapterHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_EthernetAdapter getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_EthernetAdapter dataInstance = null;
		try {
			dataInstance = new CIM_EthernetAdapter(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_EthernetAdapter createInstance(CIMClient cimClient, CIM_EthernetAdapter dataInstance, boolean notifyDifferencies){ 
		...
		CIM_EthernetAdapter newDataInstance = null;
		try {
			newDataInstance = CIM_EthernetAdapterHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_EthernetAdapter modifyInstance(CIMClient cimClient, CIM_EthernetAdapter dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_EthernetAdapterHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_EthernetAdapter dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_EthernetAdapter dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_EthernetAdapter.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_EthernetAdapter.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_EthernetAdapter.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_EthernetAdapter.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_EthernetAdapter.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_TotalPacketsReceived(UnsignedInt64 TotalPacketsReceived) {
		return true;
	}

	public static boolean isValid_TotalPacketsTransmitted(UnsignedInt64 TotalPacketsTransmitted) {
		return true;
	}
