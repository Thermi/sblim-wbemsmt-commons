/** 
 * CIM_PolicyConditionHelper.java
 *
...
public final class CIM_PolicyConditionHelper {

	public final static String CIM_CLASS_NAME = "CIM_PolicyCondition";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_PolicyCondition.Java_Package_List.size(); i++) {
						if (!((String)(CIM_PolicyCondition.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_PolicyCondition.Java_Package_List.get(i))).endsWith(".")) {
							CIM_PolicyCondition.Java_Package_List.setElementAt((String)(CIM_PolicyCondition.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_PolicyCondition.");
						resultArrayList.add(new CIM_PolicyCondition(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_PolicyCondition.");
						resultArrayList.add(new CIM_PolicyCondition(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_PolicyCondition.");
						resultArrayList.add(new CIM_PolicyCondition(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_PolicyCondition getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_PolicyConditionHelper.getInstance(cimClient, new CIM_PolicyCondition(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PolicyCondition getInstance(CIMClient cimClient, CIM_PolicyCondition dataInstance) {
		...
		return CIM_PolicyConditionHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_PolicyCondition getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_PolicyCondition dataInstance = null;
		try {
			dataInstance = new CIM_PolicyCondition(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_PolicyCondition.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_PolicyCondition.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_PolicyCondition.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_PolicyCondition.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_PolicyConditionName(String PolicyConditionName) {
		return true;
	}

	public static boolean isValid_PolicyRuleCreationClassName(String PolicyRuleCreationClassName) {
		return true;
	}
