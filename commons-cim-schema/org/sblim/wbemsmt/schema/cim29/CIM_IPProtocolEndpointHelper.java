...
public final class CIM_IPProtocolEndpointHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_IPProtocolEndpoint.Java_Package_List.size(); i++) {
						if (!((String)(CIM_IPProtocolEndpoint.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_IPProtocolEndpoint.Java_Package_List.get(i))).endsWith(".")) {
							CIM_IPProtocolEndpoint.Java_Package_List.setElementAt((String)(CIM_IPProtocolEndpoint.Java_Package_List.get(i)) + ("."), i);
						}
	...
	*/	
	public final static CIM_IPProtocolEndpoint getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_IPProtocolEndpointHelper.getInstance(cimClient, new CIM_IPProtocolEndpoint(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_IPProtocolEndpoint getInstance(CIMClient cimClient, CIM_IPProtocolEndpoint dataInstance) {
		...
		return CIM_IPProtocolEndpointHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_IPProtocolEndpoint getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_IPProtocolEndpoint(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_IPProtocolEndpoint createInstance(CIMClient cimClient, CIM_IPProtocolEndpoint dataInstance, boolean notifyDifferencies){ 
		...
		CIM_IPProtocolEndpoint newDataInstance = null;
		try {
			newDataInstance = CIM_IPProtocolEndpointHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_IPProtocolEndpoint modifyInstance(CIMClient cimClient, CIM_IPProtocolEndpoint dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_IPProtocolEndpointHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_IPProtocolEndpoint dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_IPProtocolEndpoint dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_IPProtocolEndpoint.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_IPProtocolEndpoint.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_IPProtocolEndpoint.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_IPProtocolEndpoint.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_IPProtocolEndpoint.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_IPVersionSupport(UnsignedInt16 IPVersionSupport) {
		return true;
	}
	...
	public static boolean isValid_IPv6Address(String IPv6Address) {
		return true;
	}
