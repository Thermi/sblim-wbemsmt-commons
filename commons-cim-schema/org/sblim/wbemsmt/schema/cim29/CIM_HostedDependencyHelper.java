...
public final class CIM_HostedDependencyHelper {

	public final static String CIM_CLASS_NAME = "CIM_HostedDependency";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_HostedDependency(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					...
					for (int i=0; clazz==null && i<CIM_HostedDependency.Java_Package_List.size(); i++) {
						if (!((String)(CIM_HostedDependency.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_HostedDependency.Java_Package_List.get(i))).endsWith(".")) {
							CIM_HostedDependency.Java_Package_List.setElementAt((String)(CIM_HostedDependency.Java_Package_List.get(i)) + ("."), i);
						}
	...
	*/	
	public final static CIM_HostedDependency getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_HostedDependencyHelper.getInstance(cimClient, new CIM_HostedDependency(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_HostedDependency getInstance(CIMClient cimClient, CIM_HostedDependency dataInstance) {
		...
		return CIM_HostedDependencyHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_HostedDependency getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_HostedDependency(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_HostedDependency createInstance(CIMClient cimClient, CIM_HostedDependency dataInstance, boolean notifyDifferencies){ 
		...
		CIM_HostedDependency newDataInstance = null;
		try {
			newDataInstance = CIM_HostedDependencyHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_HostedDependency modifyInstance(CIMClient cimClient, CIM_HostedDependency dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_HostedDependencyHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_HostedDependency dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_HostedDependency dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_HostedDependency.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_HostedDependency.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_HostedDependency.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_HostedDependency.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_HostedDependency.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
