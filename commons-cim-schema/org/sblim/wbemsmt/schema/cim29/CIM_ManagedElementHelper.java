...
public final class CIM_ManagedElementHelper {
	...
	*/	
	public final static CIM_ManagedElement getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ManagedElementHelper.getInstance(cimClient, new CIM_ManagedElement(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ManagedElement getInstance(CIMClient cimClient, CIM_ManagedElement dataInstance) {
		...
		return CIM_ManagedElementHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ManagedElement getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_ManagedElement(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_ManagedElement.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_ManagedElement.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_ManagedElement.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_ManagedElement.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Caption(String Caption) {
		return true;
	}
