/** 
 * CIM_MediaPhysicalStatInfoHelper.java
 *
...
public final class CIM_MediaPhysicalStatInfoHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_MediaPhysicalStatInfo(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_MediaPhysicalStatInfo.Java_Package_List.size(); i++) {
						if (!((String)(CIM_MediaPhysicalStatInfo.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_MediaPhysicalStatInfo.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_MediaPhysicalStatInfo.Java_Package_List.setElementAt((String)(CIM_MediaPhysicalStatInfo.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_MediaPhysicalStatInfo.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_MediaPhysicalStatInfo.");
						resultArrayList.add(new CIM_MediaPhysicalStatInfo(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_MediaPhysicalStatInfo.");
						resultArrayList.add(new CIM_MediaPhysicalStatInfo(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_MediaPhysicalStatInfo.");
						resultArrayList.add(new CIM_MediaPhysicalStatInfo(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_MediaPhysicalStatInfo getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_MediaPhysicalStatInfoHelper.getInstance(cimClient, new CIM_MediaPhysicalStatInfo(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_MediaPhysicalStatInfo getInstance(CIMClient cimClient, CIM_MediaPhysicalStatInfo dataInstance) {
		...
		return CIM_MediaPhysicalStatInfoHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_MediaPhysicalStatInfo getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_MediaPhysicalStatInfo dataInstance = null;
		try {
			dataInstance = new CIM_MediaPhysicalStatInfo(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_MediaPhysicalStatInfo createInstance(CIMClient cimClient, CIM_MediaPhysicalStatInfo dataInstance, boolean notifyDifferencies){ 
		...
		CIM_MediaPhysicalStatInfo newDataInstance = null;
		try {
			newDataInstance = CIM_MediaPhysicalStatInfoHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_MediaPhysicalStatInfo modifyInstance(CIMClient cimClient, CIM_MediaPhysicalStatInfo dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_MediaPhysicalStatInfoHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_MediaPhysicalStatInfo dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_MediaPhysicalStatInfo dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_MediaPhysicalStatInfo.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_MediaPhysicalStatInfo.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_MediaPhysicalStatInfo.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_MediaPhysicalStatInfo.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_MediaPhysicalStatInfo.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_PickFailures(UnsignedInt64 PickFailures) {
		return true;
	}

	public static boolean isValid_PickRetries(UnsignedInt64 PickRetries) {
		return true;
	}

	public static boolean isValid_PickSuccesses(UnsignedInt64 PickSuccesses) {
		return true;
	}

	public static boolean isValid_PutFailures(UnsignedInt64 PutFailures) {
		return true;
	}

	public static boolean isValid_PutRetries(UnsignedInt64 PutRetries) {
		return true;
	}

	public static boolean isValid_PutSuccesses(UnsignedInt64 PutSuccesses) {
		return true;
	}

	public static boolean isValid_ScanFailures(UnsignedInt64 ScanFailures) {
		return true;
	}

	public static boolean isValid_ScanRetries(UnsignedInt64 ScanRetries) {
		return true;
	}

	public static boolean isValid_ScanSuccesses(UnsignedInt64 ScanSuccesses) {
		return true;
	}
