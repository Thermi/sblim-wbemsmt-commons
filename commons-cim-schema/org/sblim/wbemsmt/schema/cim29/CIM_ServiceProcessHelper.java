...
public final class CIM_ServiceProcessHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_ServiceProcess(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_ServiceProcess.Java_Package_List.size(); i++) {
						if (!((String)(CIM_ServiceProcess.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_ServiceProcess.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_ServiceProcess.Java_Package_List.setElementAt((String)(CIM_ServiceProcess.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_ServiceProcess.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_ServiceProcess.");
						resultArrayList.add(new CIM_ServiceProcess(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_ServiceProcess.");
						resultArrayList.add(new CIM_ServiceProcess(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_ServiceProcess.");
						resultArrayList.add(new CIM_ServiceProcess(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_ServiceProcess getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ServiceProcessHelper.getInstance(cimClient, new CIM_ServiceProcess(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ServiceProcess getInstance(CIMClient cimClient, CIM_ServiceProcess dataInstance) {
		...
		return CIM_ServiceProcessHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ServiceProcess getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_ServiceProcess dataInstance = null;
		try {
			dataInstance = new CIM_ServiceProcess(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ServiceProcess createInstance(CIMClient cimClient, CIM_ServiceProcess dataInstance, boolean notifyDifferencies){ 
		...
		CIM_ServiceProcess newDataInstance = null;
		try {
			newDataInstance = CIM_ServiceProcessHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ServiceProcess modifyInstance(CIMClient cimClient, CIM_ServiceProcess dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_ServiceProcessHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_ServiceProcess dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_ServiceProcess dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_ServiceProcess.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_ServiceProcess.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_ServiceProcess.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_ServiceProcess.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_ServiceProcess.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ExecutionType(UnsignedInt16 ExecutionType) {
		return true;
	}
