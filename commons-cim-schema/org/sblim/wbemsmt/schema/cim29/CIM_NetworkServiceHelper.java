...
public final class CIM_NetworkServiceHelper {

	public final static String CIM_CLASS_NAME = "CIM_NetworkService";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_NetworkService.Java_Package_List.size(); i++) {
						if (!((String)(CIM_NetworkService.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_NetworkService.Java_Package_List.get(i))).endsWith(".")) {
							CIM_NetworkService.Java_Package_List.setElementAt((String)(CIM_NetworkService.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_NetworkService.");
						resultArrayList.add(new CIM_NetworkService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_NetworkService.");
						resultArrayList.add(new CIM_NetworkService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_NetworkService.");
						resultArrayList.add(new CIM_NetworkService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_NetworkService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_NetworkServiceHelper.getInstance(cimClient, new CIM_NetworkService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_NetworkService getInstance(CIMClient cimClient, CIM_NetworkService dataInstance) {
		...
		return CIM_NetworkServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_NetworkService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_NetworkService dataInstance = null;
		try {
			dataInstance = new CIM_NetworkService(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_NetworkService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_NetworkService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_NetworkService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_NetworkService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Keywords(String[] Keywords) {
		return true;
	}

	public static boolean isValid_ServiceURL(String ServiceURL) {
		return true;
	}

	public static boolean isValid_StartupConditions(String[] StartupConditions) {
		return true;
	}

	public static boolean isValid_StartupParameters(String[] StartupParameters) {
		return true;
	}
