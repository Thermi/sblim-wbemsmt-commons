/** 
 * CIM_StaticForwardingEntryHelper.java
 *
...
public final class CIM_StaticForwardingEntryHelper {

	public final static String CIM_CLASS_NAME = "CIM_StaticForwardingEntry";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_StaticForwardingEntry.Java_Package_List.size(); i++) {
						if (!((String)(CIM_StaticForwardingEntry.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_StaticForwardingEntry.Java_Package_List.get(i))).endsWith(".")) {
							CIM_StaticForwardingEntry.Java_Package_List.setElementAt((String)(CIM_StaticForwardingEntry.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_StaticForwardingEntry.");
						resultArrayList.add(new CIM_StaticForwardingEntry(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_StaticForwardingEntry.");
						resultArrayList.add(new CIM_StaticForwardingEntry(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_StaticForwardingEntry.");
						resultArrayList.add(new CIM_StaticForwardingEntry(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_StaticForwardingEntry getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_StaticForwardingEntryHelper.getInstance(cimClient, new CIM_StaticForwardingEntry(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_StaticForwardingEntry getInstance(CIMClient cimClient, CIM_StaticForwardingEntry dataInstance) {
		...
		return CIM_StaticForwardingEntryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_StaticForwardingEntry getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_StaticForwardingEntry dataInstance = null;
		try {
			dataInstance = new CIM_StaticForwardingEntry(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_StaticForwardingEntry createInstance(CIMClient cimClient, CIM_StaticForwardingEntry dataInstance, boolean notifyDifferencies){ 
		...
		CIM_StaticForwardingEntry newDataInstance = null;
		try {
			newDataInstance = CIM_StaticForwardingEntryHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_StaticForwardingEntry modifyInstance(CIMClient cimClient, CIM_StaticForwardingEntry dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_StaticForwardingEntryHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_StaticForwardingEntry dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_StaticForwardingEntry dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_StaticForwardingEntry.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_StaticForwardingEntry.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_StaticForwardingEntry.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_StaticForwardingEntry.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_StaticForwardingEntry.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AllowedToGo(UnsignedInt16[] AllowedToGo) {
		return true;
	}
	...
	public static boolean isValid_MACAddress(String MACAddress) {
		return true;
	}
	...
	public static boolean isValid_StaticStatus(UnsignedInt16 StaticStatus) {
		return true;
	}
