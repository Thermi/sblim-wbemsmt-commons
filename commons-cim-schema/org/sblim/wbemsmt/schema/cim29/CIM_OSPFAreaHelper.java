...
public final class CIM_OSPFAreaHelper {

	public final static String CIM_CLASS_NAME = "CIM_OSPFArea";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_OSPFArea.Java_Package_List.size(); i++) {
						if (!((String)(CIM_OSPFArea.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_OSPFArea.Java_Package_List.get(i))).endsWith(".")) {
							CIM_OSPFArea.Java_Package_List.setElementAt((String)(CIM_OSPFArea.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_OSPFArea.");
						resultArrayList.add(new CIM_OSPFArea(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_OSPFArea.");
						resultArrayList.add(new CIM_OSPFArea(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_OSPFArea.");
						resultArrayList.add(new CIM_OSPFArea(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_OSPFArea getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_OSPFAreaHelper.getInstance(cimClient, new CIM_OSPFArea(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_OSPFArea getInstance(CIMClient cimClient, CIM_OSPFArea dataInstance) {
		...
		return CIM_OSPFAreaHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_OSPFArea getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_OSPFArea dataInstance = null;
		try {
			dataInstance = new CIM_OSPFArea(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_OSPFArea createInstance(CIMClient cimClient, CIM_OSPFArea dataInstance, boolean notifyDifferencies){ 
		...
		CIM_OSPFArea newDataInstance = null;
		try {
			newDataInstance = CIM_OSPFAreaHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_OSPFArea modifyInstance(CIMClient cimClient, CIM_OSPFArea dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_OSPFAreaHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_OSPFArea dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_OSPFArea dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_OSPFArea.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_OSPFArea.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_OSPFArea.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_OSPFArea.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_OSPFArea.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_AreaID(UnsignedInt32 AreaID) {
		return true;
	}

	public static boolean isValid_AreaType(UnsignedInt16 AreaType) {
		return true;
	}

	public static boolean isValid_ExtLsdbLimit(UnsignedInt32 ExtLsdbLimit) {
		return true;
	}
