...
public final class CIM_ProfileHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					Class clazz = CIM_ProfileHelper.findClass(cimClient, cimInstance);
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_Profile.");
						resultArrayList.add(new CIM_Profile(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_Profile.");
						resultArrayList.add(new CIM_Profile(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_Profile.");
						resultArrayList.add(new CIM_Profile(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_Profile getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ProfileHelper.getInstance(cimClient, new CIM_Profile(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Profile getInstance(CIMClient cimClient, CIM_Profile dataInstance) {
		...
		return CIM_ProfileHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Profile getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_Profile dataInstance = null;
		try {
			dataInstance = new CIM_Profile(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_Profile.getPackages();
	...
	*/	
	public final static CIM_Profile createInstance(CIMClient cimClient, CIM_Profile dataInstance, boolean notifyDifferencies){ 
		...
		CIM_Profile newDataInstance = null;
		try {
			newDataInstance = CIM_ProfileHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_Profile modifyInstance(CIMClient cimClient, CIM_Profile dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_ProfileHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_Profile dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_Profile dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_Profile.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Profile.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Profile.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Profile.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Profile.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
