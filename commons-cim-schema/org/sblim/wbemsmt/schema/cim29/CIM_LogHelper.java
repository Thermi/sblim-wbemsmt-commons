...
public final class CIM_LogHelper {

	public final static String CIM_CLASS_NAME = "CIM_Log";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_Log.Java_Package_List.size(); i++) {
						if (!((String)(CIM_Log.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_Log.Java_Package_List.get(i))).endsWith(".")) {
							CIM_Log.Java_Package_List.setElementAt((String)(CIM_Log.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_Log.");
						resultArrayList.add(new CIM_Log(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_Log.");
						resultArrayList.add(new CIM_Log(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_Log.");
						resultArrayList.add(new CIM_Log(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_Log getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_LogHelper.getInstance(cimClient, new CIM_Log(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Log getInstance(CIMClient cimClient, CIM_Log dataInstance) {
		...
		return CIM_LogHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_Log getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_Log dataInstance = null;
		try {
			dataInstance = new CIM_Log(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_Log.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_Log.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_Log.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_Log.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CurrentNumberOfRecords(UnsignedInt64 CurrentNumberOfRecords) {
		return true;
	}

	public static boolean isValid_MaxNumberOfRecords(UnsignedInt64 MaxNumberOfRecords) {
		return true;
	}
