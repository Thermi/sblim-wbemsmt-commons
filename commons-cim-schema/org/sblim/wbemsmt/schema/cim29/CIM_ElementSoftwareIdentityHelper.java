...
public final class CIM_ElementSoftwareIdentityHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_ElementSoftwareIdentity(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_ElementSoftwareIdentity.Java_Package_List.size(); i++) {
						if (!((String)(CIM_ElementSoftwareIdentity.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_ElementSoftwareIdentity.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_ElementSoftwareIdentity.Java_Package_List.setElementAt((String)(CIM_ElementSoftwareIdentity.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_ElementSoftwareIdentity.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static CIM_ElementSoftwareIdentity getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ElementSoftwareIdentityHelper.getInstance(cimClient, new CIM_ElementSoftwareIdentity(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ElementSoftwareIdentity getInstance(CIMClient cimClient, CIM_ElementSoftwareIdentity dataInstance) {
		...
		return CIM_ElementSoftwareIdentityHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ElementSoftwareIdentity getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_ElementSoftwareIdentity(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ElementSoftwareIdentity createInstance(CIMClient cimClient, CIM_ElementSoftwareIdentity dataInstance, boolean notifyDifferencies){ 
		...
		CIM_ElementSoftwareIdentity newDataInstance = null;
		try {
			newDataInstance = CIM_ElementSoftwareIdentityHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ElementSoftwareIdentity modifyInstance(CIMClient cimClient, CIM_ElementSoftwareIdentity dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_ElementSoftwareIdentityHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_ElementSoftwareIdentity dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_ElementSoftwareIdentity dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_ElementSoftwareIdentity.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_ElementSoftwareIdentity.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_ElementSoftwareIdentity.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_ElementSoftwareIdentity.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_ElementSoftwareIdentity.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_OtherUpgradeCondition(String OtherUpgradeCondition) {
		return true;
	}

	public static boolean isValid_UpgradeCondition(UnsignedInt16 UpgradeCondition) {
		return true;
	}

	public static boolean isValid_CIM_SoftwareIdentity(CIM_SoftwareIdentity CIM_SoftwareIdentity) {
		return true;
	}
