...
public final class CIM_SubProfileRequiresProfileHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_SubProfileRequiresProfile(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_SubProfileRequiresProfile.Java_Package_List.size(); i++) {
						if (!((String)(CIM_SubProfileRequiresProfile.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_SubProfileRequiresProfile.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_SubProfileRequiresProfile.Java_Package_List.setElementAt((String)(CIM_SubProfileRequiresProfile.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_SubProfileRequiresProfile.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static CIM_SubProfileRequiresProfile getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_SubProfileRequiresProfileHelper.getInstance(cimClient, new CIM_SubProfileRequiresProfile(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SubProfileRequiresProfile getInstance(CIMClient cimClient, CIM_SubProfileRequiresProfile dataInstance) {
		...
		return CIM_SubProfileRequiresProfileHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_SubProfileRequiresProfile getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_SubProfileRequiresProfile(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_SubProfileRequiresProfile createInstance(CIMClient cimClient, CIM_SubProfileRequiresProfile dataInstance, boolean notifyDifferencies){ 
		...
		CIM_SubProfileRequiresProfile newDataInstance = null;
		try {
			newDataInstance = CIM_SubProfileRequiresProfileHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_SubProfileRequiresProfile modifyInstance(CIMClient cimClient, CIM_SubProfileRequiresProfile dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_SubProfileRequiresProfileHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_SubProfileRequiresProfile dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_SubProfileRequiresProfile dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_SubProfileRequiresProfile.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_SubProfileRequiresProfile.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_SubProfileRequiresProfile.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_SubProfileRequiresProfile.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_SubProfileRequiresProfile.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CIM_RegisteredProfile(CIM_RegisteredProfile CIM_RegisteredProfile) {
		return true;
	}

	public static boolean isValid_CIM_RegisteredSubProfile(CIM_RegisteredSubProfile CIM_RegisteredSubProfile) {
		return true;
	}
