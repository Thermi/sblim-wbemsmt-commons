...
public final class CIM_TraceLevelTypeHelper {

	public final static String CIM_CLASS_NAME = "CIM_TraceLevelType";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_TraceLevelType.Java_Package_List.size(); i++) {
						if (!((String)(CIM_TraceLevelType.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_TraceLevelType.Java_Package_List.get(i))).endsWith(".")) {
							CIM_TraceLevelType.Java_Package_List.setElementAt((String)(CIM_TraceLevelType.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_TraceLevelType.");
						resultArrayList.add(new CIM_TraceLevelType(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_TraceLevelType.");
						resultArrayList.add(new CIM_TraceLevelType(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_TraceLevelType.");
						resultArrayList.add(new CIM_TraceLevelType(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_TraceLevelType getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_TraceLevelTypeHelper.getInstance(cimClient, new CIM_TraceLevelType(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_TraceLevelType getInstance(CIMClient cimClient, CIM_TraceLevelType dataInstance) {
		...
		return CIM_TraceLevelTypeHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_TraceLevelType getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_TraceLevelType dataInstance = null;
		try {
			dataInstance = new CIM_TraceLevelType(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_TraceLevelType createInstance(CIMClient cimClient, CIM_TraceLevelType dataInstance, boolean notifyDifferencies){ 
		...
		CIM_TraceLevelType newDataInstance = null;
		try {
			newDataInstance = CIM_TraceLevelTypeHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_TraceLevelType modifyInstance(CIMClient cimClient, CIM_TraceLevelType dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_TraceLevelTypeHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_TraceLevelType dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_TraceLevelType dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_TraceLevelType.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_TraceLevelType.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_TraceLevelType.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_TraceLevelType.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_TraceLevelType.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_BaseType(UnsignedInt16 BaseType) {
		return true;
	}
	...
	public static boolean isValid_TraceValues(UnsignedInt32[] TraceValues) {
		return true;
	}

	public static boolean isValid_ValueDescriptions(String[] ValueDescriptions) {
		return true;
	}
