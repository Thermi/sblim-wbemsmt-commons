/** 
 * CIM_ProductSoftwareComponentHelper.java
 *
...
public final class CIM_ProductSoftwareComponentHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_ProductSoftwareComponent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_ProductSoftwareComponent.Java_Package_List.size(); i++) {
						if (!((String)(CIM_ProductSoftwareComponent.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_ProductSoftwareComponent.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_ProductSoftwareComponent.Java_Package_List.setElementAt((String)(CIM_ProductSoftwareComponent.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_ProductSoftwareComponent.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_ProductSoftwareComponent.");
						resultArrayList.add(new CIM_ProductSoftwareComponent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_ProductSoftwareComponent.");
						resultArrayList.add(new CIM_ProductSoftwareComponent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_ProductSoftwareComponent.");
						resultArrayList.add(new CIM_ProductSoftwareComponent(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_ProductSoftwareComponent getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ProductSoftwareComponentHelper.getInstance(cimClient, new CIM_ProductSoftwareComponent(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ProductSoftwareComponent getInstance(CIMClient cimClient, CIM_ProductSoftwareComponent dataInstance) {
		...
		return CIM_ProductSoftwareComponentHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ProductSoftwareComponent getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_ProductSoftwareComponent dataInstance = null;
		try {
			dataInstance = new CIM_ProductSoftwareComponent(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ProductSoftwareComponent createInstance(CIMClient cimClient, CIM_ProductSoftwareComponent dataInstance, boolean notifyDifferencies){ 
		...
		CIM_ProductSoftwareComponent newDataInstance = null;
		try {
			newDataInstance = CIM_ProductSoftwareComponentHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_ProductSoftwareComponent modifyInstance(CIMClient cimClient, CIM_ProductSoftwareComponent dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_ProductSoftwareComponentHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_ProductSoftwareComponent dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_ProductSoftwareComponent dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_ProductSoftwareComponent.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_ProductSoftwareComponent.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_ProductSoftwareComponent.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_ProductSoftwareComponent.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_ProductSoftwareComponent.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CIM_Product(CIM_Product CIM_Product) {
		return true;
	}
