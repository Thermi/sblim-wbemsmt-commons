/** 
 * CIM_UnsignedPublicKeyHelper.java
 *
...
public final class CIM_UnsignedPublicKeyHelper {

	public final static String CIM_CLASS_NAME = "CIM_UnsignedPublicKey";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_UnsignedPublicKey(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					...
					for (int i=0; clazz==null && i<CIM_UnsignedPublicKey.Java_Package_List.size(); i++) {
						if (!((String)(CIM_UnsignedPublicKey.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(CIM_UnsignedPublicKey.Java_Package_List.get(i))).endsWith(".")) {
							CIM_UnsignedPublicKey.Java_Package_List.setElementAt((String)(CIM_UnsignedPublicKey.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_UnsignedPublicKey.");
						resultArrayList.add(new CIM_UnsignedPublicKey(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_UnsignedPublicKey.");
						resultArrayList.add(new CIM_UnsignedPublicKey(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_UnsignedPublicKey.");
						resultArrayList.add(new CIM_UnsignedPublicKey(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_UnsignedPublicKey getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_UnsignedPublicKeyHelper.getInstance(cimClient, new CIM_UnsignedPublicKey(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_UnsignedPublicKey getInstance(CIMClient cimClient, CIM_UnsignedPublicKey dataInstance) {
		...
		return CIM_UnsignedPublicKeyHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_UnsignedPublicKey getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_UnsignedPublicKey dataInstance = null;
		try {
			dataInstance = new CIM_UnsignedPublicKey(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_UnsignedPublicKey createInstance(CIMClient cimClient, CIM_UnsignedPublicKey dataInstance, boolean notifyDifferencies){ 
		...
		CIM_UnsignedPublicKey newDataInstance = null;
		try {
			newDataInstance = CIM_UnsignedPublicKeyHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_UnsignedPublicKey modifyInstance(CIMClient cimClient, CIM_UnsignedPublicKey dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_UnsignedPublicKeyHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_UnsignedPublicKey dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_UnsignedPublicKey dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_UnsignedPublicKey.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_UnsignedPublicKey.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_UnsignedPublicKey.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_UnsignedPublicKey.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_UnsignedPublicKey.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
