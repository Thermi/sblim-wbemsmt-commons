...
public final class CIM_ListenerDestinationHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<CIM_ListenerDestination.Java_Package_List.size(); i++) {
						if (!((String)(CIM_ListenerDestination.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_ListenerDestination.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_ListenerDestination.Java_Package_List.setElementAt((String)(CIM_ListenerDestination.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_ListenerDestination.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static CIM_ListenerDestination getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_ListenerDestinationHelper.getInstance(cimClient, new CIM_ListenerDestination(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ListenerDestination getInstance(CIMClient cimClient, CIM_ListenerDestination dataInstance) {
		...
		return CIM_ListenerDestinationHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_ListenerDestination getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_ListenerDestination(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_ListenerDestination.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_ListenerDestination.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_ListenerDestination.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_ListenerDestination.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
