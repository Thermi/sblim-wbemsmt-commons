...
public final class PRS_ActivityHelper {

	public final static String CIM_CLASS_NAME = "PRS_Activity";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<PRS_Activity.Java_Package_List.size(); i++) {
						if (!((String)(PRS_Activity.Java_Package_List.get(i))).trim().equals("") &&
								!((String)(PRS_Activity.Java_Package_List.get(i))).endsWith(".")) {
							PRS_Activity.Java_Package_List.setElementAt((String)(PRS_Activity.Java_Package_List.get(i)) + ("."), i);
						}
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class PRS_Activity.");
						resultArrayList.add(new PRS_Activity(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class PRS_Activity.");
						resultArrayList.add(new PRS_Activity(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class PRS_Activity.");
						resultArrayList.add(new PRS_Activity(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static PRS_Activity getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return PRS_ActivityHelper.getInstance(cimClient, new PRS_Activity(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Activity getInstance(CIMClient cimClient, PRS_Activity dataInstance) {
		...
		return PRS_ActivityHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static PRS_Activity getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		PRS_Activity dataInstance = null;
		try {
			dataInstance = new PRS_Activity(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Activity createInstance(CIMClient cimClient, PRS_Activity dataInstance, boolean notifyDifferencies){ 
		...
		PRS_Activity newDataInstance = null;
		try {
			newDataInstance = PRS_ActivityHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static PRS_Activity modifyInstance(CIMClient cimClient, PRS_Activity dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = PRS_ActivityHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, PRS_Activity dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, PRS_Activity dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					PRS_Activity.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!PRS_Activity.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < PRS_Activity.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(PRS_Activity.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(PRS_Activity.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ActionLog(String ActionLog) {
		return true;
	}

	public static boolean isValid_Deleted(String Deleted) {
		return true;
	}

	public static boolean isValid_LocalDate(Calendar LocalDate) {
		return true;
	}

	public static boolean isValid_Modified(String Modified) {
		return true;
	}

	public static boolean isValid_NewKeys(String NewKeys) {
		return true;
	}

	public static boolean isValid_ParameterList(String ParameterList) {
		return true;
	}
