...
public final class CIM_CommMechanismForManagerHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new CIM_CommMechanismForManager(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<CIM_CommMechanismForManager.Java_Package_List.size(); i++) {
						if (!((String)(CIM_CommMechanismForManager.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(CIM_CommMechanismForManager.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							CIM_CommMechanismForManager.Java_Package_List.setElementAt((String)(CIM_CommMechanismForManager.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (CIM_CommMechanismForManager.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static CIM_CommMechanismForManager getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_CommMechanismForManagerHelper.getInstance(cimClient, new CIM_CommMechanismForManager(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_CommMechanismForManager getInstance(CIMClient cimClient, CIM_CommMechanismForManager dataInstance) {
		...
		return CIM_CommMechanismForManagerHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_CommMechanismForManager getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new CIM_CommMechanismForManager(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_CommMechanismForManager createInstance(CIMClient cimClient, CIM_CommMechanismForManager dataInstance, boolean notifyDifferencies){ 
		...
		CIM_CommMechanismForManager newDataInstance = null;
		try {
			newDataInstance = CIM_CommMechanismForManagerHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_CommMechanismForManager modifyInstance(CIMClient cimClient, CIM_CommMechanismForManager dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_CommMechanismForManagerHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_CommMechanismForManager dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_CommMechanismForManager dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_CommMechanismForManager.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_CommMechanismForManager.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_CommMechanismForManager.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_CommMechanismForManager.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_CommMechanismForManager.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_CIM_ObjectManager(CIM_ObjectManager CIM_ObjectManager) {
		return true;
	}
