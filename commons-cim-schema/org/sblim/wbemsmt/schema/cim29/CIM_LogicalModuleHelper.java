...
public final class CIM_LogicalModuleHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class CIM_LogicalModule.");
						resultArrayList.add(new CIM_LogicalModule(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class CIM_LogicalModule.");
						resultArrayList.add(new CIM_LogicalModule(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class CIM_LogicalModule.");
						resultArrayList.add(new CIM_LogicalModule(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static CIM_LogicalModule getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return CIM_LogicalModuleHelper.getInstance(cimClient, new CIM_LogicalModule(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LogicalModule getInstance(CIMClient cimClient, CIM_LogicalModule dataInstance) {
		...
		return CIM_LogicalModuleHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static CIM_LogicalModule getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		CIM_LogicalModule dataInstance = null;
		try {
			dataInstance = new CIM_LogicalModule(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = CIM_LogicalModule.getPackages();
	...
	*/	
	public final static CIM_LogicalModule createInstance(CIMClient cimClient, CIM_LogicalModule dataInstance, boolean notifyDifferencies){ 
		...
		CIM_LogicalModule newDataInstance = null;
		try {
			newDataInstance = CIM_LogicalModuleHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static CIM_LogicalModule modifyInstance(CIMClient cimClient, CIM_LogicalModule dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = CIM_LogicalModuleHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, CIM_LogicalModule dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, CIM_LogicalModule dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					CIM_LogicalModule.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!CIM_LogicalModule.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < CIM_LogicalModule.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(CIM_LogicalModule.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(CIM_LogicalModule.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ModuleNumber(UnsignedInt16 ModuleNumber) {
		return true;
	}
